"""업로드 바이트 읽기 어댑터 두 벌 (`kernel/blob_backends.py` · `PLAN-SoT §9 〈342〉-㉴` V-4).
...
def test_drive_uploads_skips_an_upload_whose_bytes_are_gone(tmp_path: Path) -> None:
    ...
    ⭑ **⟨병합 창 8-a⟩ `numpy` 에 걸려 있던 `importorskip` 한 줄을 걷었다.**
    ／ 종전 ~~「`app.worker` 는 geo 스택을 끌어오므로 **의존이 선 곳에서만** 돈다 —
    건너뛰는 것을 통과로 세지 않으려고 이유를 남긴다」~~
    ⛔ **`numpy` 는 선언·핀된 의존이다**(`requirements.in:3` · `requirements.txt` `numpy==2.5.2`).
    선언된 의존에 `importorskip` 을 걸면 그 의존이 이미지에서 빠지는 날 이 시험은
    **red 가 아니라 skip** 이 되고 요약줄은 green 을 찍는다 — `CLAUDE.md §4` 가 막는 그 자리이고
    `test_dependency_gate.py::test_every_importorskip_on_a_declared_dependency_is_gone` 이
    형제까지 잠근다(그 시험이 이 줄을 잡아냈다). **선언된 의존은 그냥 import 한다** —
    없으면 그것이 red 다.
    """
