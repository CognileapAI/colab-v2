...
#   ② db/ai/seed 의 시드 셋을 소유자 롤로 적재한다           (K2 사전 22행 · K2b 그래프 노드 55·엣지 21)
...
# ② 시드 — 소유자 롤로 넣는다. 시험이 세는 수(22 · 55 · 21)의 출처가 여기다.
...
#   넷째 파일은 2026-09-26 자료 지역 확정 1 의 남한 노드·엣지·별칭이다(`0012_region_south_korea` 의 적재물).
#   빼면 test_concept_graph_db.py 가 55·21 을 못 세어 red 다.
psql_owner < "$REPO/db/ai/seed/practitioner_concept_nodes.sql" >/dev/null
psql_owner < "$REPO/db/ai/seed/region_south_korea.sql" >/dev/null
