...
⭑ **⟨개정 2026-08-31 · Ted 판정 ⑬ · `PLAN-SoT §9 〈240〉`⟩ 이 파일이 재는 것은 이제
**스위치를 켠 갈래**다.** ／ 종전 문면 ~~등록 데이터셋의 지도형은 타일 갈래로 간다~~ —
정본(260826 델타 · POL-021)이 축자로 타일을 배제하고 있어 **기본값은 「한 장」**이 됐다.
갈래 선택과 기본값의 시험은 `test_tile_branch_switch.py` 다. 여기 픽스처가 `client` 에서
`tile_client` 로 바뀐 것이 그 개정의 전부다 — **재는 성질은 그대로다.**
...
def test_등록_데이터셋의_지도형_결과는_타일_갈래다(tile_client, put_target, tiny_geotiff):
    """`oneOf` 의 다른 쪽으로 넘어간다 — `tileUrlTemplate` 이 있고 `imageUrl` 이 없다."""
    tid = put_target(copy_from=[tiny_geotiff])
    job = _render(tile_client, {"datasetId": tid})
...
def test_타일_틀은_치환_자리_셋을_그대로_가진다(tile_client, put_target, tiny_geotiff):
    """계약 축자 — 「`{z}`·`{x}`·`{y}` 치환」. 화면은 이 셋만 바꾼다."""
    tid = put_target(copy_from=[tiny_geotiff])
    tpl = _render(tile_client, {"datasetId": tid})["result"]["tileUrlTemplate"]
    assert "{z}" in tpl and "{x}" in tpl and "{y}" in tpl


def test_타일_갈래에도_경계와_동반파일이_함께_간다(tile_client, put_target, tiny_geotiff):
...
def test_타일_틀이_가리키는_주소가_실제로_타일을_낸다(tile_client, put_target, tiny_geotiff):
    """**이음매에 있고 화면에 못 가던** 것이 `#48` 이었다 — 이제 같은 문자열로 닿는다."""
    tid = put_target(copy_from=[tiny_geotiff])
    tpl = _render(tile_client, {"datasetId": tid})["result"]["tileUrlTemplate"]
    path = tpl.replace("{z}", "6").replace("{x}", "54").replace("{y}", "25")
    r = tile_client.get(path[path.index("/viz/v1"):], headers=AUTH)
    assert r.status_code == 200, r.text
    assert r.headers["content-type"] == "image/png"


def test_비지도형은_타일_갈래로_가지_않는다(tile_client, put_target):
...
def test_미등록_업로드는_그대로_단일_이미지다(tile_client, put_target, tiny_geotiff):
