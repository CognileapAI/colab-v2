...
def test_완료_결과는_이미지_경계_범례를_준다(client, put_target, tiny_geotiff):
    ...
    ⭑ **⟨재개정 2026-08-31 · Ted 판정 ⑬ · `〈240〉`⟩ 갈래가 스위치가 됐고 **기본값은
    「한 장」**이라, 주 화면 자리가 다시 `imageUrl` 이다.** ／ 종전 문면 ~~`〈238〉` 으로
    `tileUrlTemplate` 으로 바뀐다~~ — 정본(260826 델타 · POL-021)이 타일을 축자로
    배제하고 있다. 켠 쪽의 키 집합은 `test_tile_branch_switch.py` 가 잰다.
    **키 집합을 등호로 계속 잠근다 — 검사를 줄이지 않는다.**"""
    ...
    assert "tileUrlTemplate" not in res, "`oneOf` 다 — 두 갈래를 함께 내지 않는다"
