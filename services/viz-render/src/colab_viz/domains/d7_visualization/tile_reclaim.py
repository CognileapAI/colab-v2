...
⭑ **⟨증보 2026-09-13 · `DL-2` 회부문 ⓓ6 · 범위 분리⟩ 이 모듈이 말하는 「지우지 않는다」는
  **자동 회수 루프**의 이야기다.** 사용자가 데이터셋 삭제를 누른 경로는 **별도 문**
  (`reclaim_on_delete`)이 지고, 그 문은 지워진 `fileId` 집합만 입력으로 받는다.
  ⛔ **잠금이 약해진 것이 아니다** — 이 루프의 S3 삭제는 **여전히 0건**이고 그 사실을 재는
  시험 2건(`tests/test_s3_tile_reclaim.py`)은 **한 글자도 고치지 않았다.** 잠금이 막으려던
  것은 「**스스로 도는** 판정이 아무도 안 보는 사이에 지우는 것」이고, 새 문의 방아쇠는
  **사람의 삭제 한 번**이다.
...
def run_s3_observation(*, client: Any, source: Any,
    ...
    **자동 회수 루프는 S3 를 지우지 않는다** — 여기에 삭제 문을 아예 연결하지 않는다.
    dev의 첫 관측이 안전하다는 증거를 얻기 전 로컬 볼륨용 자동 삭제를 객체 저장소까지
    넓히면 삭제 범위가 새로 생긴다. 따라서 apply=True도 요청 사실만 경고하고 결과는
    언제나 applied=False다.
    ⭑ 사용자 지시 삭제는 **별도 문**(`reclaim_on_delete`)이 진다 — 방아쇠가 사람의 삭제
    한 번이고 입력이 지워진 `fileId` 집합이라, 스스로 도는 이 루프와 범위가 다르다.
...
@dataclass
class S3ReclaimJob:
    """dev S3 배포에 붙는 주기 job.

    **자동 회수 루프이므로 삭제 포트를 받지 않는다** — 사용자 지시 삭제는 별도 문
    (`reclaim_on_delete`)이 지고, 그 문은 이 job 을 거치지 않는다.
    """
    client: Any
    source: Any
    uploads_prefix: str = "uploads"
