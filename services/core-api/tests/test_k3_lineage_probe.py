...
def test_k3_계보_제안_후보와_대조군_4종을_적어_둔다(p2_client, sql, fake_ai, session_factory):
    ...
    for row in rows:
        ...
        # ⑵ — **후손은 파생 Lv 가 업로드 Lv 보다 크다.** 그래서 적격 필터가 통째로 지운다.
        #     이것이 구조 보장이고, 군이 빈 이유가 「후보에 없었다」가 아님을 여기서 못 박는다.
        for dataset_id in row["groups"]["descendants"]["in_pool_ids"]:
            assert row["eligibility"][dataset_id] > row["upload_level"], \
                f"후손 {dataset_id} 가 적격 Lv 로 걸리지 않는다 — 필터가 새는 자리다"
        assert row["groups"]["descendants"]["survived"] == 0
