...
⛔ **제품 기능이 아니다.** ⭑ ⟨개정 2026-09-12 · `DL-1`⟩ **제품 삭제 op 은 `DL-1` 이 연다**
(`deleteDataset`·`getDatasetDeletionImpact` · 병합 시 `PLAN-SoT §9 〈N〉`) ／ 종전 표기
~~`deleteDataset` 은 `NOT_IMPLEMENTED_P1`~~. **그래도 이 스크립트는 그것과 다른 일을 한다** —
제품 삭제는 **묘비**(행이 남고 계보에 자리가 남는다 · 감사 `dataset.tombstoned`)이고,
여기는 **행 자체를 지운다**(감사 `dataset.deleted`). 대상은 **바이트가 이미 없어진 데이터셋의
원장 행**(＝ 제품이 표현할 수 없는 상태)이고, 그 사유·승인자·대상 목록은 `§9` 행에 남는다.
제품 패키지 밖(`ops/`)이라 배포 이미지에 실리지 않는다.

**왜 스크립트인가** — 제품에는 **되돌릴 수 없는 물리 삭제 표면이 없고, 열 계획도 없다.**
5행을 위해 그 표면을 여는 것은 `CLAUDE.md §5` 「범위 늘리기」다.
