...
def gone(message: str = "이 데이터는 지워졌어요.") -> ApiError:
    """410 — 두 자리에만 쓴다. **인자를 안 주면 묘비 문면이다.**

    ⭑ **⟨병합 창 8-a⟩ 이 함수가 이 파일에 두 번 정의돼 있었다.** 두 줄기가 각자 410 을 세웠고
    뒤의 정의가 앞을 **조용히 덮어써** `gone()` 을 인자 필수로 만들었다 — `catalog.py:696` 의
    무인자 호출이 `TypeError` 로 죽었다(묘비 시험이 그 사실을 냈다). **둘 다 살려 한 벌로 합쳤다.**

    ⑴ **연구실 묘비** — **자기 연구실의 묘비 하나에만** 쓴다 (Ted 판정 2026-09-03 · 17차 해제).
    ...
    ⑵ **수명이 지난 다운로드 티켓** — 「있었는데 창이 닫혔다 — 다시 발급받으라」
    (`〈339〉`-(다) · `getDownloadBytes` 전용 · `message` 를 명시로 넘긴다).
    404 와 갈라 쓰는 이유는 같다: 404 는 「없거나 접근이 회수됐다」, 410 은 서명은 맞는데
    수명이 지난 티켓이다. 둘을 접으면 화면이 「없다」와 「다시 받으면 된다」를 구분 못 한다.
    ⚠ 계약에서도 두 자리는 **다른 응답**이다 — `Gone`(묘비) · `TicketGone`(티켓 만료).
    """
...
def payload_too_large(message: str) -> ApiError:
    """413 — 본문이 저장 백엔드의 단일 쓰기 상한을 넘는다 (`〈339〉` — S3 단일 PUT 5 GiB).

    400 과 갈라 쓴다: 요청이 틀린 것이 아니라 **이 입구로는 못 받는 크기**다. 그 크기의
    정문은 프리사인드 전송(`〈338〉`)이고, 메시지가 그쪽을 가리킨다.
    """
