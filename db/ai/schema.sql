...
CREATE TABLE d9_topic_synonym (
  ...
  -- ⭑ ⟨2026-09-06 · Ted 판정 · `〈360〉` 해소 · `0006_topic_vocab_six`⟩ 주제 어휘 **6값**.
  --   platform 쪽이 먼저 넓혔고(`db/platform/schema.sql` · `0013_topic_vocab_six` · `〈359〉`)
  --   이 체인이 뒤따르지 못한 것을 `〈359〉`-㉶ 가 다음 회차 진입조건으로 적어 두었다.
  --   ⚠ 이 6값은 **dev 에 이미 적용된 리비전**의 결과다(WU-C13 · `0007` 머지가 rc7 갈래와 잇는다).
  topic        text        NOT NULL
...
-- Scoped dataset knowledge is separate from the public ontology guardian ACL.
-- No runtime role is provisioned by this migration.
CREATE SCHEMA knowledge;
REVOKE ALL ON SCHEMA knowledge FROM PUBLIC;
-- Same canonical value definition as contracts/schemas/common.json#/$defs/Ulid.
