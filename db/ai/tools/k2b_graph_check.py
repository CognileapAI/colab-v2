...
# ⭑ 2026-09-26 Ted 판정 「자료 지역 확정」 1 로 한 줄 더해 10 줄이다(남한⊂한반도).
APPROVED_G6 = {
    ...
    # ⭑ 2026-09-26 Ted 판정 「자료 지역 확정」 1 축자 「남한⊂한반도 관계로 연결」 — 10줄째.
    ("p-south-korea", "안에 있다", "p-korea-peninsula"): "2026-09-26 자료 지역 확정 1",
}
...
def main() -> int:
    ...
    for e in sorted(db_g6):
        if e not in APPROVED_G6:
            fail(f"[2 승인 목록 밖] {e[0]} -{e[1]}-> {e[2]} — Ted 승인 목록(§F-A 9줄 + 2026-09-26 1줄) 어디에도 없다")
