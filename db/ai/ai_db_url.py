"""ai 체인의 접속 URL 판독기 (`PLAN-SoT §9 〈121〉-㉯`).

`COLAB_AI_DB_URL` 은 값으로도, **`COLAB_AI_DB_URL_FILE` 로 경로로도** 들어온다.
`docker inspect` 의 환경변수 목록에 접속 문자열이 통째로 들어 있어 그 값이 작업 기록에
남았기 때문이다 — 자격 파일을 `0600` 으로 두고 경로만 넘긴다.

⚠ **platform 체인과 공유하지 않는다.** 두 체인은 마이그레이션 체인이 갈라져 있고(`CLAUDE.md §3-3`)
그 분리를 「같은 헬퍼를 쓰니까」로 도로 붙이지 않는다. 같은 규칙이 `db/platform/platform_db_url.py` 에
따로 산다.
"""
