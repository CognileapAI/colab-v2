#!/usr/bin/env bash
# 접속 URL 파일 참조 시험 — ai 체인 (`PLAN-SoT §9 〈121〉-㉯`).
#
# `COLAB_AI_DB_URL_FILE` 이 다섯 규칙대로 도는지 기계가 증명한다.
...
#   ⑤ **값을 메시지에 싣지 않는다**
#
# 판독기는 `db/ai/ai_db_url.py` 다. **platform 체인과 공유하지 않는다** —
...
red() { echo "::error::db-url-file(ai) red — $*"; exit 1; }

command -v "$PY" >/dev/null 2>&1 || red "python 을 찾지 못했다($PY). COLAB_PYTHON 으로 지정한다."
[ -f "$CHAIN_DIR/ai_db_url.py" ] || red "판독기(ai_db_url.py)가 없다."
...
echo "db-url-file(ai): green — 규칙 5항"
