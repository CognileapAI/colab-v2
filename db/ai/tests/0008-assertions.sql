-- 0008 오라클 — **`d10_model_call` 이 실제로 막는가**를 DB 에게 물어본다.
...
--
-- 0007 까지만 적용된 DB 에서는 첫 문장(표 부재)에서 ERROR 가 나고 psql 이 비영으로 끝난다.
-- 그것이 「되돌리면 red」의 실물이다.
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION '0008 오라클 실패 — d10_model_call 표가 없다';
  END IF;
  ...
  IF n <> 2 THEN
    RAISE EXCEPTION '0008 오라클 실패 — 색인 둘이 아니라 %건이다', n;
  END IF;
  ...
  IF n <> 0 THEN
    RAISE EXCEPTION '0008 오라클 실패 — 리전 칸이 생겼다(%건). 철회된 의무를 되살렸다', n;
  END IF;
  ...
  IF n <> 0 THEN
    RAISE EXCEPTION '0008 오라클 실패 — 파생 칸(%건)이 생겼다. 캐시율은 조회 때 계산한다', n;
  END IF;
  ...
  SELECT count(*) INTO n FROM information_schema.columns
    WHERE table_name = 'd10_model_call';
  IF n <> 15 THEN
    RAISE EXCEPTION '0008 오라클 실패 — 칸이 15 가 아니라 %개다', n;
  END IF;
  ...
  END;
  IF ok THEN RAISE EXCEPTION '0008 오라클 실패 — 목록 밖 call_site 가 들어갔다'; END IF;
  ...
  EXCEPTION WHEN check_violation THEN ok := false;
  END;
  IF ok THEN RAISE EXCEPTION '0008 오라클 실패 — 목록 밖 outcome 이 들어갔다'; END IF;
  ...
  END;
  IF ok THEN
    RAISE EXCEPTION '0008 오라클 실패 — 사유 없는 not_called 가 들어갔다. 미호출 행의 요지가 사라진다';
  END IF;
  ...
  EXCEPTION WHEN check_violation THEN ok := false;
  END;
  IF ok THEN RAISE EXCEPTION '0008 오라클 실패 — not_called 가 아닌 행에 사유가 붙었다'; END IF;
  ...
  END;
  IF ok THEN
    RAISE EXCEPTION '0008 오라클 실패 — 미호출 행에 지연 0 이 붙었다. 평균 지연이 조용히 낮아진다';
  END IF;
  ...
  EXCEPTION WHEN check_violation THEN ok := false;
  END;
  IF ok THEN RAISE EXCEPTION '0008 오라클 실패 — 캐시율이 1 을 넘는 행이 들어갔다'; END IF;
  ...
  EXCEPTION WHEN check_violation THEN ok := false;
  END;
  IF ok THEN RAISE EXCEPTION '0008 오라클 실패 — 정규 ID 가 아닌 id 가 들어갔다'; END IF;
  ...
  EXCEPTION WHEN others THEN
    RAISE EXCEPTION '0008 오라클 실패 — 정상 행이 거절됐다: %', SQLERRM;
  END;
  ...
  IF NOT ok THEN
    RAISE EXCEPTION '0008 오라클 실패 — 조회로 캐시율을 계산하지 못했다';
  END IF;

  RAISE NOTICE '[0008-assertions] 닫힌 어휘 실거절 · 미호출 사유 양방향 · 캐시 상한 · 정규 ID · 정상 3행 통과 · 조회 캐시율 0.80 → OK';
