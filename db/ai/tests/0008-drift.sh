#!/usr/bin/env bash
# 0008 드리프트 시험 — **원장 표를 되돌리면 오라클이 red 를 낸다**를 기계가 증명한다.
# 형태는 db/ai/tests/0006-drift.sh 와 같다 (같은 실패를 두 번 배우지 않는다).
#
#   ㈎ head (0008 적용)        → 오라클 green (닫힌 어휘가 실제로 거절 · 정상 행 통과 · 조회 캐시율)
#   ㈏ 0007 까지만             → 오라클 red     ← 「되돌리면 red」
#   ㈐ head → downgrade 0007   → 오라클 red + pg_dump 로 0007 형태 복원 확인
...
HEAD_REV="0008_d10_model_call_ledger"
PREV_REV="0007_merge_vocab_and_category"

red() { echo "::error::0008-drift red — $*"; exit 1; }
ready() { echo "::error::0008-drift red(준비) — $*"; exit 78; }
...
[ -f "$HERE/0008-assertions.sql" ] || red "오라클 파일(0008-assertions.sql)이 없다."

TMP="$(mktemp -d -p "${TMPDIR:-/tmp}" k1bdb-drift8-XXXXXX)"
PGC=""
...
for token in 'CREATE TABLE d10_model_call' 'd10_model_call_called_at_idx' \
  ...
  grep -q "$token" "$TMP/delta.sql" || red "렌더된 델타 SQL 에 '$token' 이 없다 — 0008 이 체인에 안 붙었거나 갈렸다."
done
...
oracle() {   # $1=DB $2=기대 $3=라벨
  ...
  out="$(docker exec -i "$PGC" psql -q -v ON_ERROR_STOP=1 -U postgres -d "$1" \
          < "$HERE/0008-assertions.sql" 2>&1)"; rc=$?
  got="green"; [ $rc -eq 0 ] || got="red"
  if [ "$got" = "$2" ]; then
    echo "[0008-drift] $3 → $got OK"
    [ "$got" = "red" ] && echo "$out" | grep -m1 "0008 오라클 실패\|ERROR" | sed 's/^/           ↳ /'
  else
    echo "[0008-drift] $3 → $got (기대 $2) ✗"
...
oracle head_db green "㈎ 0008 적용 후 — 닫힌 어휘가 실제로 거절하고 정상 행이 통과하는가"

mkdb prev_db; psql_f prev_db "$TMP/prev.sql" || red "0007 마이그레이션이 적용되지 않았다."
oracle prev_db red "㈏ 0008 없음 — 오라클이 red 를 내는가"
...
# 「지웠다」와 「되돌렸다」는 다르다 — 0007 상태와 스키마가 같은가.
...
if diff -u "$TMP/prev_db.norm" "$TMP/down_db.norm" > "$TMP/shape.diff"; then
  echo "[0008-drift] ㈐ downgrade 결과 = 0007 상태 (pg_dump 동일) → OK"
else
  echo "[0008-drift] ㈐ downgrade 결과가 0007 상태와 다르다 ✗"
  sed 's/^/           /' "$TMP/shape.diff" | head -40
  FAILURES+=("㈐ downgrade = 0007 동일성")
fi
...
if diff -u "$TMP/decl_db.decl" "$TMP/chain_db.decl" > "$TMP/decl.diff"; then
  echo "[0008-drift] ㈑ 선언 정본 schema.sql = 마이그레이션 결과 → OK"
else
  echo "[0008-drift] ㈑ schema.sql 과 마이그레이션 결과가 갈렸다 ✗"
  sed 's/^/           /' "$TMP/decl.diff" | head -60
  FAILURES+=("㈑ schema.sql ↔ 마이그레이션 동일성")
fi

if [ "${#FAILURES[@]}" -gt 0 ]; then
  printf '::error::0008-drift red — 실패 %d건:\n' "${#FAILURES[@]}"
  printf '     - %s\n' "${FAILURES[@]}"
  exit 1
fi
echo "0008-drift green — ㈎ 닫힌 어휘 실거절 ＋ 정상 행 통과 · ㈏㈐ 되돌리면 red · downgrade 실물 동작 · 선언 = 적용 · 리전·캐시율·질의 칸 0."
