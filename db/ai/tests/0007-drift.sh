...
ready() { echo "::error::0007-drift red(준비) — $*"; exit 78; }
...
#   ⭑ ⟨병합 기록 2026-09-25 · D10 실행 원장⟩ develop 은 같은 red(원장 회차가 뒤에 붙음)를
#     `dev_db`·`stg_db` 를 체인 head 까지 올려 견주는 방식으로 고쳤다. 통합본은 위 체인 head
#     별도 DB 방식을 채택한다 — 아래 ㈒ 행 대조가 0007 상태의 `dev_db`·`stg_db` 를 전제로 하기 때문이다.
