#!/usr/bin/env bash
# 0010 오라클 — 2026-09-18 결정 4·5 의 개념 6행. 배치는 `0009-drift.sh` 와 같다
# (같은 실패를 두 번 배우지 않는다). 한 줄 회차라 두 순서 갈래는 없다 — 그 자리는
# `0007-drift.sh` 가 이미 재고 있고, 이 파일은 **이 회차가 무엇을 더하는가**를 잰다.
#
#   ⓪ 델타 정적 판정 — 이 회차의 렌더에 **DDL 이 없고** 개념 그래프를 **실제로 건드린다**
#      (0009 의 ⓪ 와 정확히 반대다 — 그때는 보류라 d9_concept 이 나타나면 red 였다)
#   ㈎ head(0010) 적용 → 오라클 SQL 통과
#   ㈏ prev(0009) 판에서는 같은 오라클이 **실패해야** 한다 — 「되돌리면 red 가 나는가」
#   ㈐ downgrade 실물 동작 — 0009 shape·행 복원 ＋ 그 판에서 오라클이 실패
#   ㈑ 선언 정본(`db/ai/schema.sql`) = **체인 head** (schema-diff 가 보는 것과 같은 사실)
#
# ⚠ ㈑ 이 견주는 상대는 체인 head 이지 이 회차가 아니다 — `schema.sql` 은 체인 전체의 선언
#   정본이다(`0006-drift.sh` · `0007-drift.sh` · `0009-drift.sh` ㈑ 와 같은 배치).
#
...
REV="0010_practitioner_concept"
PREV_REV="0009_practitioner_lexicon"

red(){ echo "::error::0010-drift red — $*"; exit 1; }
ready(){ echo "::error::0010-drift red(준비) — $*"; exit 78; }
...
TMP="$(mktemp -d -p "${TMPDIR:-/tmp}" k1bdb-drift10-XXXXXX)"; PGC=""
...
for token in ERA5 ECMWF 유럽중기예보센터 가뭄 '파일 포맷 예제'; do
...
# 0009 의 ⓪ 와 반대다 — 그때는 보류라 이 글자가 나타나면 red 였고, 이번에는 없으면 red 다.
grep -q 'd9_concept_edge' "$TMP/delta.code" || red "델타가 d9_concept_edge 를 건드리지 않는다 — 엣지 E1-12 가 빠졌다."
# 결정 8 은 relation CHECK 3값을 그대로 두기로 닫았다. 새 관계값이 델타에 나타나면 그 판정이 깨진 것이다.
for banned in '~이 제공한다' '~을 관측한다' '~에서 가공되었다' '~을 보정한다'; do
  if grep -q "$banned" "$TMP/delta.code"; then
    red "델타에 «$banned» 가 있다 — 결정 8 은 relation CHECK 를 열지 않았다."
  fi
done
echo "[0010-drift] ⓪ 델타 = 노드 5 ＋ 엣지 1 · DDL 0 · 새 relation 0 → OK"

docker image inspect "$IMAGE" >/dev/null 2>&1 || docker pull -q "$IMAGE" >/dev/null 2>&1 \
  || ready "이미지 $IMAGE 를 확보하지 못했다."
PGC="k1bdb_0010_$$_${RANDOM}"
...
apply head_db "$HERE/0010-assertions.sql" || red "㈎ head 에서 오라클이 실패했다 — 이 회차가 주장을 지키지 못한다."
echo "[0010-drift] ㈎ head 적용 ＋ 오라클 통과 → OK"

# ── ㈏ 이전 head 에서는 같은 오라클이 **실패해야** 한다 ──────────────────────
mkdb prev_db
apply prev_db "$TMP/prev.sql" || red "$PREV_REV 적용 실패(㈏)."
if apply prev_db "$HERE/0010-assertions.sql" >/dev/null 2>&1; then
  echo "[0010-drift] ㈏ 0009 판이 오라클을 통과했다 — 오라클이 이 회차를 재지 않는다 ✗"
  FAILURES+=("㈏ 0009 에서 오라클 통과")
else
  echo "[0010-drift] ㈏ 0009 판에서 오라클 실패(기대) → OK"
fi

# ── ㈐ downgrade 실물 동작 ＋ 0009 복원 ──────────────────────────────────────
mkdb down_db
apply down_db "$TMP/head.sql" || red "down 판 head 적용 실패(㈐)."
apply down_db "$TMP/down.sql" || red "downgrade 적용 실패(㈐) — 되돌릴 수 없는 회차다."
if apply down_db "$HERE/0010-assertions.sql" >/dev/null 2>&1; then
  echo "[0010-drift] ㈐ downgrade 판이 오라클을 통과했다 — 되돌려도 red 가 안 난다 ✗"
  FAILURES+=("㈐ downgrade 에서 오라클 통과")
fi
norm prev_db; norm down_db
if diff -u "$TMP/prev_db.norm" "$TMP/down_db.norm" > "$TMP/shape.diff"; then
  echo "[0010-drift] ㈐ downgrade 결과 = 0009 shape (pg_dump 차이 0줄) → OK"
else
  echo "[0010-drift] ㈐ downgrade 결과가 0009 shape 와 다르다 ✗"
  sed 's/^/           /' "$TMP/shape.diff" | head -60
  FAILURES+=("㈐ downgrade = 0009 shape")
fi
...
if diff -u "$TMP/prev_db.rows" "$TMP/down_db.rows" > "$TMP/rows.diff"; then
  echo "[0010-drift] ㈐ downgrade 결과 = 0009 그래프 행 집합 ($(wc -l < "$TMP/prev_db.rows")행 동일) → OK"
else
  echo "[0010-drift] ㈐ downgrade 뒤에도 이 회차의 행이 남았다 ✗"
  sed 's/^/           /' "$TMP/rows.diff" | head -40
  FAILURES+=("㈐ downgrade = 0009 그래프 행 집합")
fi
...
if diff -u "$TMP/decl_db.norm" "$TMP/chain_db.norm" > "$TMP/decl.diff"; then
  echo "[0010-drift] ㈑ 선언 정본 schema.sql = 체인 head($CHAIN_HEAD_REV) → OK"
else
  echo "[0010-drift] ㈑ schema.sql 과 체인 head 가 갈렸다 ✗"
  sed 's/^/           /' "$TMP/decl.diff" | head -60
  FAILURES+=("㈑ schema.sql ↔ 체인 head")
fi

if [ "${#FAILURES[@]}" -gt 0 ]; then
  printf '::error::0010-drift red — 실패 %d건:\n' "${#FAILURES[@]}"
  printf '     - %s\n' "${FAILURES[@]}"
  exit 1
fi
echo "0010-drift green — 델타 DDL 0 · head 오라클 통과 · 0009·downgrade 판에서 실패 · 그래프 행 복원 · 선언 = 체인 head."
