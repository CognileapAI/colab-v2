"""alembic 환경 — platform 체인.

접속 URL 은 환경변수 COLAB_PLATFORM_DB_URL 로 들어온다. 이 파일에 적지 않는다.
**값 대신 경로로도 받는다** — COLAB_PLATFORM_DB_URL_FILE (`PLAN-SoT §9 〈121〉-㉯`).
`docker inspect` 의 환경변수 목록에 접속 문자열이 통째로 남는 것을 막는다. 판정 규칙은
`platform_db_url.py` 한 곳에 있다.
