...
command -v "$ALEMBIC" >/dev/null 2>&1 || { echo "::error::0027-operator-audit-drift red(준비) — alembic이 없다"; exit 78; }
...
for token in d2_operator_audit d3_operator_audit d6_operator_audit d2_operator_export d3_operator_export d5_operator_export d6_operator_export d8_operator_export 'ENABLE ROW LEVEL SECURITY' 'receipt_hash IS NULL'; do
  grep -q "$token" "$TMP/up.sql" || { echo "::error::0027 drift red — $token 없음"; exit 1; }
done
echo "[0027-drift] 감사 3표·내보내기 5표·RLS·pending index 확인 → OK"
