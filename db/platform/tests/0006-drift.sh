#!/usr/bin/env bash
# 0006 드리프트 시험 — **0006 를 되돌리면 red 가 난다**를 기계가 증명한다.
# 형태는 0005-drift.sh 와 같다(같은 실패를 두 번 배우지 않는다). 다른 것은 대상 리비전뿐이다.
#
#   ㈎ head (0006 적용)        → 오라클 green
...
HEAD_REV="0006_s1_trgm_matching"
PREV_REV="0005_s1_search_index"
...
grep -q "pg_trgm" "$TMP/head.sql" \
  || red "렌더된 head SQL 에 pg_trgm 가 없다 — 0006 가 체인에 안 붙었거나 리비전 이름이 다르다."
...
oracle head_db green "㈎ 0006 적용 후 — 오라클"
...
echo "0006-drift green — ㈎ 적용 green · ㈏ 0006 없으면 red · ㈐ downgrade 실물 동작 + 0005 복원."
