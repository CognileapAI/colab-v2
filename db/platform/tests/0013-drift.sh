...
#   ㈏ 0012 까지만             → 오라클 red     ← 「되돌리면 red」
#   ㈐ head → downgrade 0012   → 오라클 red + pg_dump 로 0012 형태 복원 확인
#   ㈑ 0012 에 기존 행을 심고 → 0012:0013 델타 적용 → **백필 오라클 green**
...
PREV_REV="0012_merge_lv1_and_transfer"

red() { echo "::error::0013-drift red — $*"; exit 1; }
ready() { echo "::error::0013-drift red(준비) — $*"; exit 78; }
...
for token in file_extension observation_interval_unit period_granularity; do
  grep -q "$token" "$TMP/head.sql" \
    || red "렌더된 head SQL 에 $token 이 없다 — 0013 가 체인에 안 붙었거나 M-9·M-6·M-7 이 한 head 가 아니다."
done
...
# ── ㈏ 0012 까지만 (0013 없음) ──────────────────────────────────────────────
mkdb prev_db;  psql_f prev_db "$TMP/prev.sql" || red "0012 마이그레이션이 적용되지 않았다."
...
# 「지웠다」와 「되돌렸다」는 다르다 — 0012 상태와 스키마가 같은가.
...
if diff -u "$TMP/prev_db.norm" "$TMP/down_db.norm" > "$TMP/shape.diff"; then
  echo "[0013-drift] ㈐ downgrade 결과 = 0012 상태 (pg_dump 동일) → OK"
else
  echo "[0013-drift] ㈐ downgrade 결과가 0012 상태와 다르다 ✗"
  sed 's/^/           /' "$TMP/shape.diff" | head -40
  FAILURES+=("㈐ downgrade = 0012 동일성")
fi

# ── ㈑ 백필 — **적재된 데이터 위에서** 도는 것이 이 회차의 알맹이다 ────────────
# 0012 상태에 기존 행을 심고, 그 위에 0012→0013 델타만 적용한다. 실제 배포와 같은 순서다.
mkdb fill_db;  psql_f fill_db "$TMP/prev.sql"    || red "0012 적용 실패(㈑)."
psql_f fill_db "$HERE/0013-backfill-seed.sql"    || red "백필 재료를 심지 못했다(㈑)."
psql_f fill_db "$TMP/delta.sql"                  || red "0012→0013 델타가 적재 데이터 위에서 돌지 않았다(㈑)."
oracle fill_db green "㈑ 백필 후 — 파일명과 일치하는가" 0013-backfill-assertions.sql

# 오라클이 오라클임의 증명 — **열만 세우고 백필을 안 하면 red** 여야 한다.
mkdb nofill_db; psql_f nofill_db "$TMP/prev.sql" || red "0012 적용 실패(㈑-b)."
...
echo "0013-drift green — ㈎ 적용 green · ㈏ 0013 없으면 red · ㈐ downgrade 실물 동작 + 0012 복원 · ㈑ 백필 일치(대조군 red)."
