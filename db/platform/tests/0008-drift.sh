#!/usr/bin/env bash
# 0008 드리프트 시험 — **0008 을 되돌리면 red 가 난다**를 기계가 증명한다.
# 형태는 0006-drift.sh 와 같다(같은 실패를 두 번 배우지 않는다). 다른 것은 대상 리비전뿐이다.
#
#   ㈎ head (0008 적용)        → 오라클 green
...
HEAD_REV="0008_s3_upload_transfer"
PREV_REV="0007_p2_human_written_meta"
...
grep -q "d5_upload_transfer" "$TMP/head.sql" \
  || red "렌더된 head SQL 에 d5_upload_transfer 가 없다 — 0008 이 체인에 안 붙었거나 리비전 이름이 다르다."
...
oracle head_db green "㈎ 0008 적용 후 — 오라클"
...
echo "0008-drift green — ㈎ 적용 green · ㈏ 0008 없으면 red · ㈐ downgrade 실물 동작 + 0007 복원."
