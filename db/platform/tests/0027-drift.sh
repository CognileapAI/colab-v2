#!/usr/bin/env bash
# 0027 오라클 — 계정 상태 열. 배치는 `0025-drift.sh` 와 같다(head/prev/down 세 DB).
...
red(){ echo "::error::0027-drift red — $*"; exit 1; }
ready(){ echo "::error::0027-drift red(준비) — $*"; exit 78; }
command -v docker >/dev/null || ready "docker가 없다"
command -v "$ALEMBIC" >/dev/null || [ -x "$ALEMBIC" ] || ready "alembic이 없다"
TMP="$(mktemp -d -p "${TMPDIR:-/tmp}" colab-0027-XXXXXX)"; PGC=""
...
render "upgrade 0027_account_status" "$TMP/head.sql"
render "upgrade 0026_login_sessions" "$TMP/prev.sql"
render "downgrade 0027_account_status:0026_login_sessions" "$TMP/down.sql"
...
apply head "$HERE/0027-assertions.sql" || red "head oracle 실패"
apply prev "$TMP/prev.sql" || red "prev 적용 실패"
if apply prev "$HERE/0027-assertions.sql" >/dev/null 2>&1; then red "0026이 oracle을 통과했다"; fi
apply down "$TMP/head.sql" || red "down head 적용 실패"
apply down "$TMP/down.sql" || red "빈 DB downgrade 실패"
if apply down "$HERE/0027-assertions.sql" >/dev/null 2>&1; then red "downgrade가 oracle을 통과했다"; fi
...
echo "0027-drift green — head/previous/downgrade와 빈 상태 shape 복원."
