#!/usr/bin/env bash
# 0024 드리프트 — head green, 0023/downgrade red, downgrade shape는 0023과 동일.
set -uo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CHAIN_DIR="$(cd "$HERE/.." && pwd)"
ALEMBIC="${COLAB_ALEMBIC:-alembic}"
PG_IMAGE="${COLAB_PG_IMAGE:-postgres:16-alpine}"
HEAD_REV="0024_s2_grid_convenience"
PREV_REV="0023_upv_image_grid"
red(){ echo "::error::0024-drift red — $*"; exit 1; }
unready(){ echo "::error::0024-drift red(준비) — $*"; exit 78; }
command -v docker >/dev/null 2>&1 || unready "docker가 없다"
command -v "$ALEMBIC" >/dev/null 2>&1 || [ -x "$ALEMBIC" ] || unready "alembic이 없다"
[ -f "$HERE/0024-assertions.sql" ] || unready "오라클 파일이 없다"
TMP="$(mktemp -d -p "${TMPDIR:-/tmp}" colab-0024-XXXXXX)"; PGC=""
...
for token in d3_dataset_grid_profile d3_lab_default_grid d5_upload_grid_profile early_preview_upload_id 'FORCE ROW LEVEL SECURITY'; do
...
oracle(){
  ...
  docker exec -i "$PGC" psql -q -v ON_ERROR_STOP=1 -U postgres -d "$db" \
    <"$HERE/0024-assertions.sql" >"$TMP/oracle.out" 2>&1 || rc=$?
  if { [ "$expected" = green ] && [ "$rc" -ne 0 ]; } || { [ "$expected" = red ] && [ "$rc" -eq 0 ]; }; then
    sed 's/^/  /' "$TMP/oracle.out" | head -30; red "$label 결과가 $expected가 아니다"
  fi
  echo "[0024-drift] $label → $expected OK"
}
mkdb head_db; apply head_db "$TMP/head.sql" || red "head 적용 실패"; oracle head_db green "head"
mkdb prev_db; apply prev_db "$TMP/prev.sql" || red "prev 적용 실패"; oracle prev_db red "0023"
...
diff -u "$TMP/prev_db.norm" "$TMP/down_db.norm" >"$TMP/shape.diff" || {
  head -50 "$TMP/shape.diff"; red "downgrade가 0023 shape를 복원하지 못했다"; }
echo "0024-drift green — head/previous/downgrade 3갈래와 0023 shape 복원."
