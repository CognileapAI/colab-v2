#!/usr/bin/env bash
# 0043 오라클 — `d3_dataset_variable.missing_rate_percent` 생성 컬럼. 배치는 `0031-drift.sh` 와 같다.
#
#   ㈎ head(0043) 적용 → 오라클 SQL 통과
#   ㈏ prev(0042) 에서는 같은 오라클이 **실패해야** 한다 — 「되돌리면 red 가 나는가」
#   ㈐ downgrade 실물 동작 — 0042 shape 복원 ＋ 그 판에서 오라클이 실패
...
ready(){ echo "::error::0043-drift red(준비) — $*"; exit 78; }
...
TMP="$(mktemp -d -p "${TMPDIR:-/tmp}" colab-0043-XXXXXX)"; PGC=""
...
apply head_db "$HERE/0043-assertions.sql" || red "㈎ head 에서 오라클이 실패했다 — 이 회차가 주장을 지키지 못한다."
echo "[0043-drift] ㈎ head 적용 ＋ 오라클 통과 → OK"

# ── ㈏ 이전 head 에서는 같은 오라클이 실패해야 한다 ──────────────────────────
mkdb prev_db
apply prev_db "$TMP/prev.sql" || red "$PREV_REV 적용 실패(㈏)."
if apply prev_db "$HERE/0043-assertions.sql" >/dev/null 2>&1; then
  echo "[0043-drift] ㈏ 0042 판이 오라클을 통과했다 — 오라클이 이 회차를 재지 않는다 ✗"
  FAILURES+=("㈏ 0042 에서 오라클 통과")
else
  echo "[0043-drift] ㈏ 0042 판에서 오라클 실패(기대) → OK"
fi

# ── ㈐ downgrade 실물 동작 ＋ 0042 shape 복원 ────────────────────────────────
mkdb down_db
apply down_db "$TMP/head.sql" || red "down 판 head 적용 실패(㈐)."
apply down_db "$TMP/down.sql" || red "downgrade 적용 실패(㈐) — 되돌릴 수 없는 회차다."
if apply down_db "$HERE/0043-assertions.sql" >/dev/null 2>&1; then
  echo "[0043-drift] ㈐ downgrade 판이 오라클을 통과했다 — 되돌려도 red 가 안 난다 ✗"
  FAILURES+=("㈐ downgrade 에서 오라클 통과")
fi
norm prev_db; norm down_db
if diff -u "$TMP/prev_db.norm" "$TMP/down_db.norm" > "$TMP/shape.diff"; then
  echo "[0043-drift] ㈐ downgrade 결과 = 0042 상태 (pg_dump 차이 0줄) → OK"
else
  echo "[0043-drift] ㈐ downgrade 결과가 0042 상태와 다르다 ✗"
  sed 's/^/           /' "$TMP/shape.diff" | head -60
  FAILURES+=("㈐ downgrade = 0042 동일성")
fi
...
if diff -u "$TMP/decl_db.norm" "$TMP/chain_db.norm" > "$TMP/decl.diff"; then
  echo "[0043-drift] ㈑ 선언 정본 schema.sql = 체인 head → OK"
else
  echo "[0043-drift] ㈑ schema.sql 과 체인 head 가 갈렸다 ✗"
  sed 's/^/           /' "$TMP/decl.diff" | head -60
  FAILURES+=("㈑ schema.sql ↔ 체인 head")
fi

if [ "${#FAILURES[@]}" -gt 0 ]; then
  printf '::error::0043-drift red — 실패 %d건:\n' "${#FAILURES[@]}"
  printf '     - %s\n' "${FAILURES[@]}"
  exit 1
fi
echo "0043-drift green — head 오라클 통과 · 0042·downgrade 판에서 실패 · 0042 shape 복원 · 선언 = 체인 head."
