#!/usr/bin/env bash
# 0044 오라클 — `d3_search_evidence` 의 운영자 전 연구실 읽기 정책. 배치는 `0043-drift.sh` 와 같다.
#
#   ㈎ head(0044) 적용 → 오라클 SQL 통과
#   ㈏ prev(0043) 에서는 같은 오라클이 **실패해야** 한다 — 「되돌리면 red 가 나는가」
#   ㈐ downgrade 실물 동작 — 0043 shape 복원 ＋ 그 판에서 오라클이 실패
#   ㈑ 선언 정본(`db/platform/schema.sql`) = **체인 head** (schema-diff 가 보는 것과 같은 사실)
#
# ⚠ ㈑ 이 견주는 상대는 체인 head 이지 이 회차가 아니다 — `schema.sql` 은 체인 전체의 선언
#   정본이라 뒤 회차가 서면 이 회차와는 갈린다. 지금은 이 회차가 head 다.
#
# 근거 = intent `dev-package/intent/2026-09-25-operator-search-scope.md` §설계트리 Q7.
...
REV="0044_evidence_operator_read"
PREV_REV="0043_variable_missing_rate"

red(){ echo "::error::0044-drift red — $*"; exit 1; }
ready(){ echo "::error::0044-drift red(준비) — $*"; exit 78; }
...
TMP="$(mktemp -d -p "${TMPDIR:-/tmp}" colab-0044-XXXXXX)"; PGC=""
...
apply head_db "$HERE/0044-assertions.sql" || red "㈎ head 에서 오라클이 실패했다 — 이 회차가 주장을 지키지 못한다."
echo "[0044-drift] ㈎ head 적용 ＋ 오라클 통과 → OK"

# ── ㈏ 이전 head 에서는 같은 오라클이 실패해야 한다 ──────────────────────────
mkdb prev_db
apply prev_db "$TMP/prev.sql" || red "$PREV_REV 적용 실패(㈏)."
if apply prev_db "$HERE/0044-assertions.sql" >/dev/null 2>&1; then
  echo "[0044-drift] ㈏ 0043 판이 오라클을 통과했다 — 오라클이 이 회차를 재지 않는다 ✗"
  FAILURES+=("㈏ 0043 에서 오라클 통과")
else
  echo "[0044-drift] ㈏ 0043 판에서 오라클 실패(기대) → OK"
fi

# ── ㈐ downgrade 실물 동작 ＋ 0043 shape 복원 ────────────────────────────────
mkdb down_db
apply down_db "$TMP/head.sql" || red "down 판 head 적용 실패(㈐)."
apply down_db "$TMP/down.sql" || red "downgrade 적용 실패(㈐) — 되돌릴 수 없는 회차다."
if apply down_db "$HERE/0044-assertions.sql" >/dev/null 2>&1; then
  echo "[0044-drift] ㈐ downgrade 판이 오라클을 통과했다 — 되돌려도 red 가 안 난다 ✗"
  FAILURES+=("㈐ downgrade 에서 오라클 통과")
fi
norm prev_db; norm down_db
if diff -u "$TMP/prev_db.norm" "$TMP/down_db.norm" > "$TMP/shape.diff"; then
  echo "[0044-drift] ㈐ downgrade 결과 = 0043 상태 (pg_dump 차이 0줄) → OK"
else
  echo "[0044-drift] ㈐ downgrade 결과가 0043 상태와 다르다 ✗"
  sed 's/^/           /' "$TMP/shape.diff" | head -60
  FAILURES+=("㈐ downgrade = 0043 동일성")
fi
...
if diff -u "$TMP/decl_db.norm" "$TMP/chain_db.norm" > "$TMP/decl.diff"; then
  echo "[0044-drift] ㈑ 선언 정본 schema.sql = 체인 head → OK"
else
  echo "[0044-drift] ㈑ schema.sql 과 체인 head 가 갈렸다 ✗"
  sed 's/^/           /' "$TMP/decl.diff" | head -60
  FAILURES+=("㈑ schema.sql ↔ 체인 head")
fi

if [ "${#FAILURES[@]}" -gt 0 ]; then
  printf '::error::0044-drift red — 실패 %d건:\n' "${#FAILURES[@]}"
  printf '     - %s\n' "${FAILURES[@]}"
  exit 1
fi
echo "0044-drift green — head 오라클 통과 · 0043·downgrade 판에서 실패 · 0043 shape 복원 · 선언 = 체인 head."
