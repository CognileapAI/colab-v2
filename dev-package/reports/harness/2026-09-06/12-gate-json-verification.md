...
- 자리 찾기 = `COLAB_GATE_REPORT_DIR`(있으면) → 없으면 `dev-package/reports/**/gate-summary.json` 중
  **가장 최근에 쓰인 것**. Bash env 는 도구 호출 간 유지되지 않으므로 실질 경로는 후자다.
  ⭑ **⟨증보 2026-09-06 · 어드바이저 게이트 ② 지적 4⟩ 그 자리에 낡은 JSON 이 놓일 수 있다** —
  아래 §2-6 이 그 구멍과 조치·재실측이다.
...
### 2-6. ⭑ 어드바이저 게이트 ② 반영 — H7 신선도 대조 재실측 (차단 3 / 통과 3 · 불일치 0)

**구멍**(지적 4) = 배출처가 안 실린 실행에서 H7 은 `dev-package/reports/**/gate-summary.json` 중
mtime 최신 하나를 고른다. `.gitignore` 에 그 이름이 없어 **레인이 산출물 규약(`rules §2-2`)대로
그것을 커밋하는 순간**, 이후 모든 워크트리가 체크아웃과 함께 그 파일을 받고(직후 mtime 이 서로
같아 아무거나 뽑힌다) **게이트를 안 돌린 레인이 통과**한다. 도입 시점에는 커밋된 JSON 이 0건이라
잠복 상태였다.

**조치 둘** —
- `.gitignore` 에 `dev-package/reports/**/gate-summary.json`. ⚠ 부활 규칙(`!<경로>`)은 쓰지 않았다 —
  그 파일이 **같은 이름으로** 트리에 남아 위 glob 에 그대로 걸리므로 무시 여부와 무관하게 구멍이
  유지된다. 회차 기록 한 벌은 이름을 바꿨다: `…/2026-09-06/final/gate-summary.record.json`
  (＝ 커밋된 `gate-summary.json` 0건).
- `lane-gate-summary.sh` 가 JSON 의 `commit`·`tree` 를 레인 `cwd` 의 `git rev-parse HEAD` ·
  `HEAD^{tree}` 와 대조하고, **둘 다 어긋나면 부재와 같이 exit 2**. `tree` 를 함께 보는 이유는
  `rules §3-2`(같은 트리면 전수 재실행을 갈음) 와 같다 — ff·amend 로 커밋 해시만 달라진 경우를
  「안 돌렸다」로 몰지 않는다. 값이 없거나 `cwd` 가 체크아웃이 아니면 **대조하지 않는다**(판정 불가 = 통과).

| 케이스 | 기대 | 실측 |
|---|---|---|
| `gate-summary.json` 부재 | 차단 | **2** ✓ |
| `commit == HEAD` ＋ `red_판정 0` | 통과 | **0** ✓ |
| `commit`·`tree` 둘 다 어긋남(낡은 JSON) | 차단 | **2** ✓ (「다른 커밋을 잰 것 · 부재와 같이 취급」 ＋ 양쪽 해시 표시) |
| `commit` 만 어긋나고 `tree` 동일(ff·amend) | 통과 | **0** ✓ |
| `commit`·`tree` 가 `null`(판정 불가) | 통과 | **0** ✓ |
| `commit == HEAD` ＋ `red_판정 1` | 차단 | **2** ✓ |
| 훅 env `COLAB_HOOKS=0` ＋ 낡은 JSON | 통과 | **0** ✓ |

⚠ **부작용 하나** — 게이트를 돌린 뒤 커밋하면 JSON 이 직전 트리를 가리켜 차단된다. 그래서
**게이트는 마지막 커밋 뒤에 한 번 돌리고 끝낸다**를 `lane-worker` 정의와 `gates/README.md` 에
명시했다. 이것은 규율 강화이지 우회 대상이 아니다 — 커밋 전 계수를 커밋 후의 근거로 인용하는
자리가 원래 D11·D12 의 뿌리다.

하네스 = 임시 파이썬 스크립트(레포에 남기지 않는다) · 일회용 `git init -b main` 레포.
H3 25케이스와 같은 실행에서 쟀다 — **계 32건 · 불일치 0**(H3 표는 `11-merge-guards-verification.md §2-5`).
