# CoLAB v2 × Claude Fable 5.1 — 에이전트 개발 환경 설계안 v3

작성 2026-09-06. v2 + 「AI-native SDLC 플레이북」 갭 분석(`playbook-gap.md` 6절 16건 · 5절 템플릿 · J-11/J-12) 반영본.
근거 = brief · inventory · workflow-profile · market-survey · grill-me 조사(mattpocock) · **실측 재계수** · **Anthropic 문서 인용(V1~V4)** · 플레이북 인용. `[미검증]` 은 K 절.

**v2 대비 변경 요약** — ① 파이프라인에 **1.5 grill-me** · **3.5 to-spec** 2단계 신설 ② **intent.md / spec.md** 2 아티팩트 도입, 라운드 파일을 「spec 의 실행 뷰」로 재정의 ③ vendored 스킬 5 → **8**(mattpocock `grilling`·`grill-me`·`to-spec` 추가, superpowers `brainstorming` 미채택) ④ 〈N〉 원장에 `intent:`·`spec:` 역링크 2필드 ⑤ advisor 게이트 ② 체크리스트 3항 확정(intent 대조 포함) ⑥ J-11·J-12·지표 S5 신설. **신규 phase 없음**(P-S·P-G 흡수)이고 **advisor 게이트① 수용분도 불변** — 에이전트 4 · 훅 7(차단 5) · 게이트 JSON 3상태(run.sh 기존 구현) · phase 순서 · J-2/J-6 실측 비활성.

---
## A. 설계 목표와 성공 지표

**목표** — 세션 시작 주입을 CoLAB 개발에 실제로 쓰는 것만 남기고, 사람이 기억해 우회하던 D1~D22 를 훅·게이트로 내려 「메인=오케스트레이터, 실행=격리 레인」이 문장이 아니라 코드로 강제되게 한다.
...
**핵심 함의** — humanize/presentation 자산의 **본진이 곧 세션 루트(`00 CoLAB`)** 다. v1 의 「본진 프로젝트로 옮긴다」는 J-0 이 ⓑ 일 때만 S1 을 움직인다. J-0 이 ⓐ 면 옮길 곳이 없다(이미 거기 있다) → 「형제 폴더로 이동」이 유일한 수단.
...
---
## J-0. 세션 루트 (Ted 판정 1순위 — 이 설계의 모든 경로가 종속)

**실측 사실** — 세션 cwd = `/mnt/f/00_Project/00 CoLAB`(git 레포 아님). 자동메모리 디렉터리 `-mnt-f-00-Project-00-CoLAB`. 레포는 그 아래 `30 CoLAB-v2/`.
...
⇒ 현 상태(루트=`00 CoLAB`)에서 `30 CoLAB-v2/.claude/settings.json` 에 훅을 넣으면 **한 줄도 안 뜬다**. `30 CoLAB-v2/CLAUDE.md` 는 **뜨긴 뜨되 launch 가 아니라 그 아래 파일을 읽는 순간**(현행 동작과 일치).

### 선택지

| | ⓐ 루트 유지 = `00 CoLAB` | ⓑ 루트 이동 = `30 CoLAB-v2` |
|---|---|---|
...
| humanize/presentation 자산 | 같은 루트에 상주 → **S1 을 못 내린다**. 형제 폴더로 이동해야 함 | 자동으로 후보에서 빠짐 → S1 즉시 −21 |
...
**권고 = ⓑ**. 이유 셋 — ① 훅이 레포 이력에 들어가야 「코드로 강제」가 성립(ⓐ 는 Ted 기계에만 존재하는 규칙, D 목록이 반복될 조건 그대로) ② S1 의 21개(humanize+presentation)가 파일 이동 없이 사라짐 ③ `30 CoLAB-v2/CLAUDE.md` 가 지연이 아니라 launch 로딩이 되어 §1 부트스트랩 규칙이 첫 턴부터 걸린다.
**ⓑ 채택 시 완화** — 협업자 영향은 J-9(커밋 vs local) + `COLAB_HOOKS=0` 킬스위치로 처리. 메모리 이관은 P-M 에서 rules 로 흡수하므로 추가 비용 없음.
**ⓐ 채택 시 필수 후속** — humanize/presentation 15+6 을 형제 폴더(예 `/mnt/f/00_Project/10_Humanize`)로 이동해야 S1 이 움직인다. `00 CoLAB/.claude/` 는 gitignore 밖이므로 훅 백업 절차를 별도로 세운다.

**이하 모든 경로는 ⓑ 기준**(`30 CoLAB-v2/` = 레포 루트 = 세션 루트). ⓐ 로 판정되면 `.claude/` 접두어만 `00 CoLAB/.claude/` 로 치환하고, 버전관리·협업자 항목을 「해당 없음」으로 바꾼다.
...
| 경로 | 처리 |
|---|---|
| `CLAUDE.md` | 불변 규칙만 존치(위임 원칙·서브에이전트 언어·HTML 규격). 스킬 3절(graphify/travel-proposal/explain-visually)은 각 스킬 description 이 담당 → 삭제. 5,307B → **≤2,500B** |
| `settings.json` `effortLevel` | **글로벌은 건드리지 않는다**(전 프로젝트 비용 영향 — J-10). `30 CoLAB-v2/.claude/settings.json` 에 프로젝트 스코프로 `high` |
| `agents/advisor.md` | 프로젝트로 이동 → `30 CoLAB-v2/.claude/agents/advisor.md` |
| `agents/` humanize 12 | **archive 이동**(본진 = `00 CoLAB/.claude/agents` 에 6개 이미 존재하는 중복 + 고유 6). 설계자 결정, Ted 판정 불요 |
| `skills/` apple-design·travel-proposal·humanize 3종 | archive 또는 humanize 본진으로. `archify`·`explain-visually`·`graphify` 존치 |
...
.claude/
  ...
  settings.local.json    # Ted 전용 훅(J-9 가 「개인」이면 여기로)
  ...
  hooks/                 # C절 스크립트
dev-package/
  intent/<YYYY-MM-DD>-<주제>.md   # 발의 아티팩트 (J-12 ⓐ = 레포 안). 템플릿 L-1
...
**intent.md 의 집·소유** (J-12 권고 ⓐ)
- 위치 = 레포 안 `dev-package/intent/<YYYY-MM-DD>-<주제>.md`. 커밋·게이트·〈N〉 역링크 대상이라 `40 COLAB-기획` 밖. 기획자 원본(`10_적용전`)은 **무수정**, intent 는 경로로 참조만 한다.
...
- **승인 후 개정 금지 · 신규 발행** — 승인된 intent 는 고치지 않고 새 intent.md 를 낸다. **[추론]** — 플레이북에 명시 규칙 없음(K 미검증 12).
...
| 활성(7) | context7 · typescript-lsp · pyright-lsp · code-review · eli5 · skill-creator · claude-mem(**J-2 판정 대기**) |
|---|---|
| **비활성(7)** | superpowers(5스킬 vendoring 으로 대체 · `brainstorming` 은 **grill-me 가 자리를 차지**해 대체) · understand-anything(에이전트 9+스킬 8, S1 최대 기여자) · ralph-loop · claude-md-management · frontend-design · feature-dev(에이전트 3) · playwright(필요 회차에만 on-demand) |
| 판정 대기 | codex — J-6 |
...
등록처 = `30 CoLAB-v2/.claude/settings.json`(J-9 가 「개인」이면 `settings.local.json`).
**등록 형식은 전부 `bash .claude/hooks/<x>.sh`** — D5(NTFS exec bit)가 훅 자신에게도 적용되기 때문.
**킬스위치** — 모든 스크립트 첫 줄 `[ "${COLAB_HOOKS:-1}" = "0" ] && exit 0`.
...
| F20 | `30_적용완료` 환류 | J-1 판정 대기 + **잔여 결함은 새 intent.md 로 발행**(기존 intent 개정 아님) | |
...
| 3 Ted 판정(개정) | Ted | **`dev-package/intent/<날짜>-<주제>.md`** + ELI5 HTML. `결정서_Ted_*.md` 는 intent 승인 기록으로 흡수(J-11) | F14 eli5-lint. Ted 교정·커밋 = 승인 |
...
| 15 환류(개정) | 오케스트레이터(J-1 활성 시) | `30_적용완료` 이동 + **잔여 결함은 새 intent.md** | 승인된 intent 개정 금지 |
...
- **지금 개통하는 최소분** — 입력 = 전수 `red(판정)` 로그 · `deploy_doctor` 14/14 미달 항목 · 기획자 문의/티켓. 처리 = `researcher` 가 intent 초안 1건(B-3). 승인 = Ted 교정·커밋. 요구 번호 대응 없음(F22 후보), 티어는 J-1 활성 뒤 재판정.
...
| 순서 | Phase | 내용 | 검증(측정) | 세션 | 레인 진행 중 가능? |
|---|---|---|---|---|---|
| 1 | **P0 기준선** | S1/S3/S4 현 상태 실측 → `dev-package/reports/harness/baseline.md` | 3지표 수치 기록 | 0.5 | 가능 |
| 2 | **P-T 플러그인 토글** | 플러그인 7 비활성, 글로벌 에이전트 12·스킬 5 를 archive 로 이동 | 새 세션 후보 목록 **에이전트 42→≤12** | 0.5 | 가능 |
| 3 | **P-B CLAUDE.md §1 + H1** | §1 을 라운드 파일 1개로, `bootstrap-diet.sh` 등록 | 새 세션에서 부트스트랩 읽기 바이트 2.4MB→라운드 1개 | 0.5 | 가능 |
| 4 | **P-E 워크트리 env** | H2(`SubagentStart:lane-worker`) + `worktree-setup.sh`(venv 재생성) + run.sh 자체 source | 레인 1개 스폰 → 첫 전수 red(준비) **16→0** | 1 | 가능(새 레인에서 검증) |
| 5 | **P-M 메모리 이관** | `rules/colab-rules.md` 작성, 메모리 33건에 `MIGRATED` 표기(삭제 아님) | G절 탐침 3문 통과 | 1 | 가능 |
| 6 | **P-A 에이전트** | `.claude/agents/` 4개 작성, advisor 글로벌→프로젝트 | 4개 스폰 각 1회 정상 기동, advisor ② 가 revision 체크리스트 포함 | 0.5 | 가능 |
| 7 | **P-S 스킬 vendoring** | superpowers 5 + **mattpocock 3**(`grilling`·`grill-me`·`to-spec`) 복사·Fable 5.1 문안 교정, to-spec 의 이슈트래커 절 제거, `dev-package/intent/`·`prd/specs/` 신설, `colab-v2-work` 축약 | CLAUDE.md ≤200행, colab-v2-work ≤110행, 스킬 후보 47→≤20(vendored 8 포함) **+ grill-me 1회 실전 실행 → intent.md 1건 산출(확인 문장 원문 포함) + to-spec 1회 → spec.md 1건 및 라운드 파일 첫 줄 링크 존재** | 1.5 | 가능 |
| 8 | **P-G 병합 가드** | H3·H4·H5, `renumber-decisions.sh`(**〈N〉 행의 `intent:`·`spec:` 필드 보존·이동 포함**), merge driver, exec-bit 게이트 | 의도적 위반 5종 시도 → 5/5 차단 **+ 레인 첫 줄 `merge --ff-only` 1회 통과 확인**(오탐 반증) **+ 재번호 후 표본 5건의 `intent:`·`spec:` 경로가 실존 파일을 가리키는지 확인** | 1 | 가능 |
| 9 | **P-J 게이트 JSON** | `run.sh` 요약 끝에 JSON 배출 + H6·H7 | 전수 1회에서 `counts` 가 요약줄과 일치 | 0.5 | 가능(게이트 로직 무변경이라 공유자산 위험 없음) |
| 10 | **P-C 정리** | 메모리 삭제 실행, MEMORY.md 3줄화, `30_적용완료` 배선(J-1 활성 시) | 메모리 33→≤4 | 0.5 | 가능 |
...
v1 과 동일(archive 복사 → 대체물 검증 → 삭제). 변경 2건 —
...
| # | 항목 | 다음 세션에서 무엇이 달라지나 | 권고 |
|---|---|---|---|
| **J-0** | **세션 루트 ⓐ`00 CoLAB` 유지 vs ⓑ`30 CoLAB-v2` 이동** | ⓑ: 훅·규칙이 레포에 커밋되어 리뷰·롤백 가능, humanize/presentation 21개가 후보에서 자동 소거, CLAUDE.md 가 첫 턴부터 로딩. 대신 런처 습관 변경 + 기획 폴더는 `--add-dir` + 협업자에게 훅이 간다. ⓐ: 아무것도 버전관리되지 않고, S1 을 내리려면 humanize/presentation 자산을 형제 폴더로 물리 이동해야 한다 | **ⓑ** |
| **J-1** | `40 COLAB-기획/30_적용완료` 활성 vs 폐지 | 활성: 병합마다 기획 문서 이동 + `planning-freshness` 가 미이동을 red 로. 폐지: D22 드리프트 검출 지점 소멸 | **활성화** |
| **J-2** | claude-mem 유지 vs 비활성 | 비활성: 매 도구 호출 프로세스 1개 + 매 프롬프트 훅 제거. 대신 cross-session 검색 상실 | **먼저 세어보고 판정** — 아래 실행법 |
| **J-3** | humanize/presentation 자산 행선지 | **본진이 곧 세션 루트**로 밝혀졌으므로 v1 질문이 바뀐다. ⓐ 형제 폴더 신설(`/mnt/f/00_Project/10_Humanize`)로 이동 ⓑ 제자리 유지 + J-0=ⓑ 로 후보에서 자연 소거 | J-0=ⓑ 면 **ⓑ(제자리)**, J-0=ⓐ 면 **ⓐ(형제 폴더)** |
| **J-4** | `/code-review ultra` 매 병합 vs 배포 회차당 1회 | 매 병합: 회당 $5~25. 회차당 1회: 비용 1/n, 중간 병합 결함이 회차 끝까지 잔존 | **배포 회차당 1회**(병합 직전) |
| **J-6** | codex 플러그인 유지 vs 비활성 | 유지: 2nd-opinion 축 존치, 에이전트 1+스킬 3+커맨드 7 이 목록에 잔존 | **먼저 세어보고 판정** — 아래 실행법 |
| **J-8** | 설계 문서 위치 | 레포 이력 귀속 여부 | `30 CoLAB-v2/docs/superpowers/specs/2026-09-06-harness-fable51-design.md` |
| **J-9** | 훅을 `settings.json`(커밋·팀 공유) vs `settings.local.json`(Ted 전용) | 문서 인용 — `.claude/settings.json` = "Yes, can be committed to the repo", `.claude/settings.local.json` = "No, gitignored". **커밋하면 박홍진(PR #1) 등 클론한 사람 기계에서 차단 훅 5개가 그대로 돈다.** 훅 항목은 층 간 병합(replace 아님)이라 개인 훅과 공존 가능 | **커밋 + `COLAB_HOOKS=0` 킬스위치 안내를 README 에 명시** |
| **J-10** | `effortLevel: high` 를 글로벌 vs 프로젝트 스코프 | 글로벌: humanize·발표자료 등 **전 프로젝트 비용이 같이 오른다**(현재 글로벌은 `low`, fable 은 `medium`). 프로젝트: CoLAB 세션만 오름 | **프로젝트 스코프**(`30 CoLAB-v2/.claude/settings.json`) |
| **J-11** | `결정서_Ted_*.md` ⓐ **폐지·intent 로 일원화** vs ⓑ 병존 | ⓐ: 판정 근거·확인 문장·재개봉 금지 표식이 intent.md 한 파일에 모이고 〈N〉 이 역링크로 가리킨다. 기존 4건은 그대로 보존, 신규 회차부터 적용. ⓑ: 같은 내용이 두 파일로 갈라져 「왜 이렇게 했나」 재구성 비용이 그대로 남는다 | **ⓐ** |
| **J-12** | `intent/` 위치 ⓐ **레포 안**(`30 CoLAB-v2/dev-package/intent/`) vs ⓑ `40 COLAB-기획` 안 | ⓐ: 커밋·게이트·〈N〉 역링크 대상이 되고 레인이 같은 트리에서 읽는다. 기획자 원본 무수정 규칙은 참조로 유지. ⓑ: 기획자 직접 열람은 쉬우나 레포 밖이라 버전관리·게이트가 안 걸린다 | **ⓐ** |

### J-2 / J-6 을 기억이 아니라 계수로 판정하는 법 (지적 7 수용)
...
| superpowers | brainstorming 3(1 은 오늘), systematic-debugging 1 | 설계대로 플러그인 비활성 + 5스킬 vendoring 유지 |

→ **J-2·J-6 확정: 비활성.** Ted 재실행 불요.
...
| intent 승인 후 취급 | **개정 금지·신규 발행** | 플레이북 명시 규칙 아님 **[추론]**(K 미검증 12). 「한 패치보다 넓은 것은 새 intent 로」 + spec 재작업 후행지표 계측에서 유도 |
| 병렬 레인 정책 | **직렬 1개 유지**, P-G 완료 후 오케스트레이터가 완화안을 제안 | 9/5 에 이미 판정된 사항 — 재개봉하지 않는다 |
| 글로벌 humanize 12 에이전트 | archive 이동 | 본진(`00 CoLAB/.claude/agents`)과 6건 이름 중복 = 순수 중복 |
...
8. J-2/J-6 계수 명령이 실제 세션 로그 포맷(`.jsonl`)에서 유효한지 → Ted 실행 시 0 이 나오면 「미사용」이 아니라 「패턴 불일치」일 수 있으므로, 먼저 아무 문자열(`Bash`)로 grep 해 히트가 나오는지 대조.
...
15. playbook-gap 이 미대조로 남긴 「F 표 수정 ↔ C·D 절 충돌」은 **이번 v3 에서 해소** — 신설 2단계는 훅을 0개 추가하고 게이트 로직을 건드리지 않는다(훅 7 · 게이트 JSON 계약 불변).

### 반론 (advisor 지적 중 부분 반박 1건)

- **지적 1 「하위 폴더 CLAUDE.md 는 파일 접근 시에만 로드」는 맞으나, 「B-2 의 어떤 것도 로드되지 않는다」는 과장이다.** 문서상 `30 CoLAB-v2/CLAUDE.md` 와 `30 CoLAB-v2/.claude/rules/*.md` 는 **그 아래 파일을 읽는 순간 실제로 로드된다**(nested rules 도 on-demand 로 명시). 로드되지 않는 것은 **`settings.json`(훅·permissions·effortLevel)과 `agents/`** 다. 결론(J-0 을 세워야 한다)은 동일하므로 수용하되, ⓐ 를 택할 때 「규칙 문서는 지연이나마 걸린다」를 근거로 삼을 수 있어 선택지 비교표에 반영했다.
