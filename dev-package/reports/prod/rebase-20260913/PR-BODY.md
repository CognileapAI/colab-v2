...
**`main` 규율 안으로 들여놓는 장치**를 마저 넣었다. 커밋 **28**(기존 18 ＋ 정비 A~E ＋ 배포 중 정정 F~H ＋ 실측 등재 1) ／ 종전 ~~22(기존 18 ＋ 신규 4)~~.
...
- **D. 결정 번호 재발급** `〈383〉` → `〈400〉`.
- ⭑ **배포 중 정정 F~H** — `install-cron.sh` 운영자 알림 cron 세 상태(선언 · 명시 면제 · 미선언 exit 2) · `ship.sh` 판정 레포 tar 에서 AppleDouble·`__pycache__` 제외 · `deploy_release.py` 대상 `pr`(prod).

⛔ 이 PR 의 **커밋은 코드·문서만** 바꾼다. ⭑ ⟨2026-09-13 저녁⟩ 다만 그 도구로 **prod 첫 정기 재배포를 실제로 밟았다** — 아래 「배포 실측」. ／ 종전 ~~prod 에 아무것도 적용하지 않았다~~.
...
## 배포 실측 (2026-09-13 · KST)

| 항목 | 값 |
|---|---|
| 대상 | `origin/main` **`aa8bee981ff5`**(브랜치 tip 아님 · 배포 도구만 브랜치 판을 미추적 복사해 빌드) |
| 태그 | **`prod-20260913` → `aa8bee981ff5` · 로컬만**(push 안 함) |
| 백업 | 19:38 GREEN — `_ops/backups/prod/2026-09-13T103842Z-*` |
| 롤 · 시크릿 · 키 | 롤 4 신설(총 8) · `/etc/colab` 4 신설(총 11) · `prod.env` 키 6→9 |
| 반입 | `ship.sh` 2차 성공 — `MAIN_SHA main=aa8bee981ff5 candidate=aa8bee981ff5 ancestor=yes` · 이미지 5 적재 · `COLAB_IMAGE_TAG=prod-aa8bee981ff5` · 판정 레포 동기화(1차 실패 = `/opt/colab-v2` root 소유 → `chown`) |
| 마이그레이션 | `up.sh` exit 0 — platform `0012_merge_lv1_and_transfer` → `0031_search_evidence` **19** · ai `0005_k2b_concept_graph_seed` → `0007_merge_vocab_and_category` **3** |
| 기동 | 4 유닛 healthy · `/healthz` 8000/8100/8200 200 · 실행 이미지 5 전부 `prod-aa8bee981ff5` |
| 롤 권한 | `db-bootstrap.sh account-admin`(**마이그레이션 뒤** — 앞서 돌리면 `account_admin` 스키마 부재로 실패) · `operator` · `app-grants` · `verify` ok |
| cron | ① 백업 ② 만료 전송 정리 ③ 소유권 스냅샷(매시 17분 · `colab-v2-prod` · gid 999 · 수동 1회 `current.json` d3_file 4 · d5_upload_file 4) 설치 · **④ 운영자 알림 명시 면제**(`COLAB_NOTIFICATION_SKIP=1` — prod 에 SQS·Secrets Manager·CloudWatch 자원 없음 · 런타임 venv 미구성) |
| doctor | 1차 **11/15**(✗③ 빈 env-file → IMDS 403 · ✗⑥⑦ AppleDouble `._*.py` 15,353 → 「null bytes」 · ✗⑩ 웹 미배포) → 원인 제거 뒤 **최종 `항목 15 — ✓ 15 · ✗ 0 · ─ 0` · exit 0 · 19:54 · 한 번의 실행**(실행기 기록 `pr.verify.1.log`) |
| 웹 | `deploy_release.py run --plan`(대상 `pr`) → `colab-platform-web-prod` · CloudFront `index-DuCCGbNb.js` 로컬=원격 일치 · 실행기 종료코드 78 = 검증 뒤 Slack 비밀 파일 부재(판정 무관) |
| 스모크(CloudFront) | admin 로그인 201 · `/me` 연구원 `mustChangePassword=false` · 데이터셋 1건 목록 200 · 검색 200 · 미리보기 라우트 400(빈 본문) |

근거 = `dev-package/reports/prod/rebase-20260913/REPORT.md §10` · `docs/DEPLOY.md §4-0b`·`§4-1c`.

## Ted 판정

| # | 항목 | 사실 | 판정할 것 |
|---|---|---|---|
| 1 | **태그 형식화** | `docs/BRANCHING.md` 규칙 6 · `infra/_lib/ship-gate.sh` 주석은 「태그 주체 = Ted」. 이번은 배포 세션이 `prod-20260913` 을 **로컬에서** 찍고 push 하지 않았다(게이트는 로컬 태그로 통과) | ⓐ 문면을 「Ted 또는 Ted 가 위임한 배포 세션」으로 고친다 ⓑ 이번만 예외로 두고 Ted 가 같은 커밋에 태그를 push 한다 |
| 2 | **④ 운영자 알림의 prod AWS 자원** | SQS 큐 2 · Secrets Manager 웹훅 ARN 2 · CloudWatch 알람이 dev 에만 있다. prod 는 명시 면제로 배포했다 | ⓐ prod 에도 만든다(비용 발생 · 알림 런타임 venv 구성 포함) ⓑ 당분간 면제 유지(면제 사실은 `install-cron.sh` 출력에 남는다) |
| 3 | **`〈400〉` 재발급** | 브랜치의 결정 번호는 임시다. 병합 직전 `origin/main` 최대 번호 +1 로 재실측한다(규칙 4-1). 이번 배포 실측 원장 행도 `〈N〉(병합 직전 발급)` 자리 | 병합 시점에 오케스트레이터가 발급 — Ted 확인만 |
