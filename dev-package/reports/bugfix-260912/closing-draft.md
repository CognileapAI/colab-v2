...
- 실측 두 값(병합 직전 오케스트레이터가 **재실측**) —
  - `bash dev-package/prd/tools/max-decision.sh` = **381** ⟹ 이 문서의 `〈382〉`~`〈389〉` 은 임시 번호다(`.claude/rules/colab-rules.md §4-1`).
...
- **번호** = 실측 `max-decision.sh` = **381**. 아래 `〈382〉`~`〈389〉` 은 임시 번호이고 **병합 직전 재실측한다** — 다른 레인이 먼저 들어오면 그만큼 밀린다.
...
1. `bash dev-package/prd/tools/max-decision.sh` **재실측** → `〈382〉`~`〈389〉` 를 실측값+1..+6 으로 일괄 치환(브랜치 전 파일 · `main` 쪽 같은 번호 인용은 무수정).
