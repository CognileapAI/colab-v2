#!/usr/bin/env bash
# 4회차(「1시간 이하」 주기 범위) 측정 한 벌 — 새 일회용 postgres(포트 공개 없음) → setup-db.sh → 측정기 --seed-dev-like →
# 「1시간 이하」 probe 상태표(두 경로 · cadence_probe_states.py). 끝나면 컨테이너를 지운다. DEV·운영 DB 에 닿지 않는다.
#
...
services/core-api/.venv/bin/python eval/k4-search/measure_draft_contribution.py "$URL" --i-know-this-is-disposable \
  --seed-dev-like --payload "$PAYLOAD" --round "4회차 주기 범위(1시간 이하)" --output "$OUT"
echo "measure rc=$?"
services/core-api/.venv/bin/python "$HERE/cadence_probe_states.py" "$URL" --payload "$PAYLOAD" --output "$STATES"
echo "states rc=$?"
