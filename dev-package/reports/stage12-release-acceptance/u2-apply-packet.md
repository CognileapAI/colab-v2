# U2 실제 회수 승인 대상 — 최종 배포 버전

상태: **승인 대기 · 실제 삭제0**. 사용자에게 아래 정확한 대상을 제시한 뒤 승인받아야 실행한다.

## 봉인한 대상

- 코드·배포: `30f5adf6774771c9ca34e2c3a0b92a0648b9cc76`.
- 실행 core config ID: `sha256:002b0cf2e76961de591f328f6a1aec039b9c214c51bcd324e87a59ea96bcfe9c`.
...
- private plan: `/tmp/colab-stage12-u2-approval-30f5/u2-plan.json`, 디렉터리0700·파일0600 실측. 최초 `/mnt/f` 사본은 요청한0600과 달리 실제0777로 보여 private 입력으로 사용하지 않는다. 파일을 만든 것은 실행 승인이 아니다.
...
`2026-09-11T08:48:32.480890Z`, 실제30f5 core에서 DB `REPEATABLE READ, READ ONLY`와 연구실 scope로 확인했다. A dataset25/file448/registered upload40, B dataset1/file2/registered upload1은 보존한다. `u2-observation.json` SHA256은 실제 파일로 확인한다.
...
1. 실행 직전 현재30f5 이미지로 `/opt/colab-v2/backup.sh`를 실행하고 platform/AI 백업 두 객체의 실제 HEAD 크기를 확인해 실행 기록에 봉인한다. 마지막 확인 백업은 `2026-09-11T082022Z` platform104408B/AI5520B지만, 이를 미래 실행 직전 백업으로 대신하지 않는다.
...
```bash
docker exec \
  -e COLAB_RUNNING_IMAGE_CONFIG_ID=sha256:002b0cf2e76961de591f328f6a1aec039b9c214c51bcd324e87a59ea96bcfe9c \
  -e COLAB_RELEASE_SHA=30f5adf67747 \
  ...
  --expected-code-sha 30f5adf6774771c9ca34e2c3a0b92a0648b9cc76 \
  --expected-image-config-id sha256:002b0cf2e76961de591f328f6a1aec039b9c214c51bcd324e87a59ea96bcfe9c
```
...
- f8 앱 rollback 이미지 보존은 데이터 복구를 대신하지 않는다. DB downgrade·옛 migrator 실행0.
