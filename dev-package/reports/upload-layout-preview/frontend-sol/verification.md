...
- 등록 대표 그림은 실제 `File`을 모달 상태로 보존하고 object URL을 교체·해제한다. 데이터셋 생성 요청을 시작하면 동기 잠금 표식으로 일반 입력, 원본·격자·대표 그림 조작과 전역 파일 드롭을 모두 막고 저장 중 안내만 보인다. 생성 실패 때만 보존된 입력을 다시 편집할 수 있다. 데이터셋 등록이 성공한 뒤 대표 그림 PUT이 실패하면 등록 정보와 원본 파일을 잠그고, 같은 dataset ID로 대표 그림 PUT만 재시도하거나 자동 그림으로 완료한다. 전역 파일 드롭은 생성 요청 중과 생성 완료 뒤 모두 동기·비동기 양쪽에서 확인해 두 번째 업로드 접수를 막는다. 등록 또는 PUT 도중 닫힌 모달의 늦은 응답은 상태와 상세 이동을 바꾸지 않는다.
...
- Astra 2차 reject P1 GREEN: 생성 요청 수명의 동기 잠금과 생성 중 전용 화면을 적용한 핵심 2파일 140/140, 전체 frontend 87파일 1,155/1,155 통과. `npm run typecheck` 오류 0, `npm run build` 성공(195 modules transformed). 최신 lifecycle 정본은 `final-review/gate-summary.json`이며 이 파일의 1,155건과 생성물 정합성 결과를 최종 근거로 사용한다.
