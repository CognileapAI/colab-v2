...
| 무엇 | 왜 | 등재 |
|---|---|---|
| `infra/dev/compose.yml` — 볼륨 `events` ＋ 두 단위 env ＋ `volume-init` `chown` | Ted 판정 — dev 트리거 스풀을 켠다 | `〈373〉` |
| `db/ai` — `schema.sql` CHECK 6값 ＋ 시드 `topic_synonym_six.sql`(13행) ＋ 리비전 `0006_topic_vocab_six` | Ted 판정 — 주제 동의어. `〈359〉`-㉶ 가 스스로 적어 둔 다음 회차 | `〈374〉` |
| `infra/staging/manifest-refdata.json` — 뿌리 4건에 `sourceLabel` | ⓔ(`BF-9`)를 세우려면 잴 대상이 있어야 한다 | `〈376〉` |
...
### ⓐ `sourceLabel` ⟹ ⓔ(`BF-9`) — ⭐ **green. `BF-9` `open` → `done`** (`〈376〉`)
...
### ⓑ `COLAB_VIZ_TRIGGER_SPOOL` — ⛔ **켰다. 그런데 `BF-12` ⑶ 은 여전히 안 선다** (`〈377〉`)
...
### ⓓ `load-seed.py` 409 — **등록 그대로. 원인은 규명했다** (`〈379〉`)
...
- ⚠ **위험은 지우지 않는다** — `〈373〉` 로 켠 루프가 **하필 그 단위에 스레드를 하나 더 얹는다**. 실부하 회차의 판정 자리다.
...
2. 전수 green 이면 **`main` fast-forward** ＋ `〈381〉` 로 전수 결과 등재.
...
4. Ted 판정 대기 셋 — `t4g.medium`(`〈378〉`) · s3 회수 주체(`〈377〉`) · `load-seed` op 확장(`〈379〉`).
