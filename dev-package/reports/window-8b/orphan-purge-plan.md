...
전제 = `PLAN-SoT §9 〈354〉`(S3 삭제 · 승인 1회 소진) · `〈363〉`(계수 오류 정정)
...
⟹ `〈354〉` 가 지운 것이 **정확히 이 5건의 바이트**이고 그 밖은 없다.
...
**0행에 조용히 성공**한다. `〈363〉` 함정의 쓰기판) ⑵ **순서 고정** — `d3_file` → 자식·bare → `d3_dataset`
...
**⑴ Ted 의 명시 GO** — `〈354〉` 축자 「승인 범위 = **재적재 전 1회뿐** · 이후 삭제로 연장되지 않는다」이고
