...
| | 조건 | 실측 | 판정 |
|---|---|---|---|
| ㉠ | 창 8-a done(`main` ff ＋ 게이트 전수) | `main` = `87d5b87` · PR #1 MERGED · 전수 green 50 한 실행 | **선다** (`§9 〈348〉`) |
| ㉡ | AWS 접근 인수 | `aws sts get-caller-identity` 성공(`user/colab-platform-s3-uploader-dev`) ＋ 그 키로 ssh 성공 | **선다** |
| ㉢ | 유료 전환 | `aws support describe-*` **AccessDenied**(이 IAM 은 S3 최소권한) | **미확인** (`〈350〉`) |
| ㉣ | `t4g.medium` 승격 | IMDS 실조회 = **`t4g.small`**(vCPU 2 · 1,841 MB) | **완화 판정으로 열었다** (`〈349〉`) |
...
⭑ **⟨최종 집계 2026-09-06 · 재적재 뒤⟩ 위 표는 재적재 **전** 상태다. 최종은 아래다** — 근거 `load-result.md §5` · `〈364〉`.

**계 — green 1(ⓕ) · red 1(ⓐ) · 미측정 2(ⓔ·ⓖ) · Ted 눈 확인 3(ⓑⓒⓓ).**

| | 최종 판정 | 실측 |
|---|---|---|
| ⓐ | **red** | 회수 요약 0줄 — `〈353〉` dev compose 에 `COLAB_VIZ_TRIGGER_SPOOL` 이 없어 루프가 안 뜬다. **재시도로 못 덮는다** |
...
⛔ **개명하지 않는다 · 원천을 지어내지 않는다.** 등재 `§9 〈356〉`-㉲-b. **Ted 확인 목록에서 뺐다.**
...
- ⓓ — 데이터셋 상세의 **스크린샷 버튼**. 실브라우저 크롬에서 내려받기가 **완주**하는가 · 잔존 앵커 0 · 취소 증상 미재현.
  ⭑ **이제 열 데이터셋이 14건 있다**(재적재 완료 · `〈362〉`). 카탈로그 `totalCount` = **14** — 고아 5건은 정리됐다(`〈366〉`).
...
| 무엇 | 값 | 등재 |
|---|---|---|
| 주제 어휘 | 4값 → **6값**(`가뭄`·`파일 포맷 예제`) · 마이그레이션 **`0013_topic_vocab_six`** · 회차 **20차**(등급 ㉯ · **계약 표면 변경 0**) | `〈359〉` |
| 재배포 | sha **`ea036c50c921`** · 4/4 healthy · `deploy_doctor` **14/14 한 실행** · 롤백 사슬 `dev-0045f2233a04` | `〈362〉` |
| 재적재 | **14 데이터셋 / 434 파일(본체 414 ＋ 격자 20) / 계보 6 — 매니페스트와 일치** | `〈362〉` |
| 레벨 | `Lv.0` 9 · `Lv.1` 2 · `Lv.2` 2 · 형제 1 — **계보에서 계산된 값이 화면에 뜬다** | `〈362〉`-㉰ |
| OOM | **신규 0건** · core-api 피크 **286.4 MiB / 384 MiB(74.6%)** — `〈358〉` 예측대로 최대 단일 파일이 뜨는 순간 | `〈362〉`-㉲ |
| ⓕ | ⭐ **green** — 미리보기 실호출 `완료`(변수 `EVI`) ⟹ **`I-D` `partial` → `done`** | `〈364〉`-㉮ |
| ⓔ·ⓖ | **미측정** — 매니페스트에 `sourceLabel` 0건(원천 노드 없음) · 한글 파일명 3건이 전부 **NFC** | `〈364〉`-㉯㉰ |

⛔ **정정 하나가 크다 — 「dev 는 비어 있다」가 틀렸다**(`〈363〉`). 계수를 **`colab_app` 롤**로 재
RLS 가 전 행을 가린 것을 「0」으로 읽었다. 실물은 **데이터셋 5건**이었고 `〈354〉` 의 S3 삭제가
그 바이트를 가져갔다 — **DB 행은 지울 수단이 없어 바이트 없는 고아 5건이 남는다.** Ted 결정 자리.
⭑ **규율** = RLS 아래에서 「없다」를 앱 롤로 판정하지 않는다. 정본은 **API** 다.

---

## 6. `t4g.small` 위험 감시 — 〈349〉-㉱ 규칙
...
⟹ 완화 판정(`〈349〉`)은 이 회차 동안 유지됐다. ⚠ 다만 **부하가 없었다** — 데이터 0 이라 렌더도 업로드도 안 돌았다. 실부하 판정은 재적재 회차 자리다.
...
| `BF-9` | `open` | `open` 무변 | ⓔ — 재적재는 끝났고 **`sourceLabel` 이 0건**이라 원천 노드가 없다(`〈364〉`-㉯) |
| `U-1` | `partial` | `partial` 무변 | **배포와 무관** — 8차 동결 해제 Ted 판정 ㉯ |
| `F-3` | `partial` | `partial` 무변 | **배포와 무관** — 9차 동결 해제 Ted 판정 ㉯ |
| `I-D` | `partial` | ⭐ **`done`** | **없다** — ⓕ 가 섰다(`〈364〉`-㉮). ⚠ 이 항목이 닫혔다고 dev 가 무결한 것은 아니다(`〈353〉`) |
| `V-4` | `done` | `done` 무변 | 없음 |
...
로그 `gates-all-w8b.txt`(exit 0 · `-j 4` · 단독 8 · 병렬 42 · 미선언 0) · 등재 `§9 〈367〉`.
...
그 뒤 `§9 〈367〉` 행과 이 절이 더해지며 트리가 움직였고, **차분은 문서뿐이다**
