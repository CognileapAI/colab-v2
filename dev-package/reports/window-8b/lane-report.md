...
- ⭑ **⟨추가 · 어드바이저 게이트 ② 지적⟩ `LD-DRGH-LV1` 의 이름 — 판정 하나가 남았다.**
  이름은 **「가뭄 — 시군구 주간 SPI/SPEI-4weeks (Lv.1)」** 인데 화면 레벨은 **`Lv.0`** 이다.
  `03.drought` 에 **`Lv.0` 폴더가 없어** 계보 부모가 0 이고, 레벨은 계보에서만 나온다(`〈194〉`).
  **물음 = 「이름 유지(Lv0 표시 수용) vs `Lv.0` 으로 개명」.** 세 번째 길(가뭄의 Lv.0 원천을 지정해
  부모로 함께 적재)은 **그 원천이 정본에 없어** 이 레인이 고를 수 없다. ⛔ 손대지 않았다.
