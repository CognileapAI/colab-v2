...
- 히어로는 **가장자리까지 가는 요소가 아니다** — `max-width: 720px; margin: 0 auto` 로 가운데 선다.
  따라서 뿌리에 최대폭 1200px·여백 20px 을 줘도 **넓은 창에서 히어로의 보이는 폭은 변하지 않는다**(720px 그대로).
  「뿌리 여백이 히어로를 좁힌다」는 설계 문제는 발생하지 않는다.
...
- 대장 완료정의 ⑴ 「**카탈로그**·상세 화면과 같은 최대폭」 — 카탈로그엔 최대폭 선언이 없다(§2). 지시에 따라 `project.css` 값 1200px 을 채택하고 여기 기록한다. 원장 등재 시 이 문구를 「`project.css`·`detail.css` 와 같은 값」으로 고치는 것을 제안한다.
