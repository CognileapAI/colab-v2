...
- 작업 브랜치 **`k3-resume`** · HEAD **`c4cdd5ea`** · origin push 완료 · **develop 미병합**(사용자 「dev 병합은 추후」 2026-09-24)
...
K3 는 **검색이 아니라 자료 등록 화면(E-04)의 「부모 자료 귀띔」 기능**이다. 사용자가 2026-09-24 에
...
사용자 승인 Q1~Q6 + 게이트 ① 조건 5건. **문 세 개** 구조:
...
| 항목 | 결과 |
|---|---|
| dev 플랫폼 DB | ~~0건~~ — ⚠ **정정 2026-09-25: 거짓 0**(앱 롤 `colab_app` · 연구실 경계 없음 · FORCE RLS). 실제(07:40Z BYPASSRLS 백업) = 4,189행 — 계정 8 · 데이터셋 35(살아 있음 33) · 파일 585 · 프로젝트 6 · 계보 22 · 업로드 147 · autometa 35. 전부 9-16 생성(시드 28 ＋ 팀원 QA 계정 3 의 QA 자료) |
| dev AI 사전 DB | 103행 · head `0007` · **재시드해도 손실 0행**(표 전수 6개 확인, 전부 마이그레이션 시드) |
| S3 | `uploads/`·`previews/` 1,778 객체 — ⚠ **정정: 고아가 아니다.** `uploads/` 1,028 중 1,024 를 DB 가 참조(등록 파일 585 전부 · 만료 미등록 업로드 포함) · `previews/` 750 = 재생성 가능 산출물 · 참조 없는 키 4. reset `s3` 단계(08:34:12–17Z)가 1,778 을 지웠다(versioning · delete marker) |
| 선행 ㉮㉯ | `/opt/colab-repo` 를 `ea21d8c2aa54` 트리로 갱신 · 백업 1회(07:40Z) — **완료** |
| 리허설 | 원시동작 **10/10** · doctor **15/15** · preflight **11/11** |
| 게이트 ③ | **조건부 go** — 조건 1~3 충족, 조건 4(동시 사용자 없음)는 사용자 확인 완료. ⚠ go 논거가 위 두 행(DB 0 · 고아)이었고 재측정하지 않았다 |
...
**지금 dev**(9-24 2차 중단 시점 기록 · 현재 상태는 §8) — 정상 가동(컨테이너 4 healthy). 계정 최종화 미실행 — 재시드 재개 전 계정 설계 재검토 필요.
해제가 멱등이라 **`--from seed` 로 같은 run-dir 에서 그대로 잇는다**(reset~prelude 재실행 불요).

### 3-3. 곁가지 — dev DB 가 왜 비었나 — ⚠ 정정 2026-09-25(`corpus-expansion/dev-reseed-rca-2026-09-25.md` §0′)
- 종전 「행만 지워졌다 · 자동 reset 도구는 그때 없었다 · 9-18 이후 재공백화」는 셋 다 틀렸다.
- 9-15 공백화 = 재시드 `a7d9a6d8` 의 reset `DROP SCHEMA`(05:59:27Z · 06:05:22Z) — alembic 0031 → 0032 · 연구실 06:06:17Z 재생성. S3 백업 172,867 B → 20,983 B 는 그 결과다.
- 9-16 회복(168,601 B) = 재시드 `d81e4387`(02:03:32Z reset — 9-15 저녁 사람 테스트를 지웠다)의 시드 28. 이어 팀원 QA 계정 3 의 QA 자료(12:57–15:15Z)가 더해졌다(9-18 백업 219,162 B).
- 9-16 15:15Z → 9-24 07:40Z 데이터 변경 0 — 9-18·9-24 백업의 표별 행 집합이 같다. **dev 는 비어 있지 않았다.**
- 「비었다」는 경계 없는 앱 롤 계수의 거짓 0 이었다(9-22 14:12Z · 9-24 06:43Z · 06:57Z) — `.agents/rules/deploy.md` 10번 증보 〈363〉 재발.
- 9-24 08:33Z reset 이 이 자료를 지웠다(4,189행 · S3 1,778 키) — reset 이 비영 계수를 보고도 기존 자료를 지운 세 번째다.
...
| # | 무엇 | 권고 |
|---|---|---|
| 1 | **develop 병합 시점** | 코퍼스 실측 뒤 한 번에(사용자 「추후」 확인됨) |
...
- **FORCE RLS** — 연구실 경계 없이 NOBYPASSRLS 롤(앱 롤 `colab_app` · 소유자 롤 모두)로 세면 행이 있어도 0 이 나온다. 「연구실 1 · 나머지 0」이 그 표지다. BYPASSRLS 롤 또는 API 로만 센다 — 9-24 사고의 공동 원인 중 하나(RCA §0′-3).
...
- **(b) dev 재시드 — 데이터 적재 끝 · 검증 미달.** 28/28 등록 · 프로젝트 4 · 간선 18 · autometa 28/28(period_start 28 · grid 26 · crs 24).
  재시드 `stage_verify` exit 1(미리보기 성립 25 · 미성립 1 · 판정불가 2) → **계정 최종화 미실행** — 재시드 재개 전 계정 설계 재검토 필요.
