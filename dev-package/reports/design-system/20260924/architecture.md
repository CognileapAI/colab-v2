...
| **P2b 프리미티브 단일 소유** | `primitives.css`·`patterns.css` 신설 · 계열별 기본값(`.btn`·`.field`·`.card`·`.chip`·`.modal`·`.table`·`.tabs`)은 프리미티브 층, 화면별 편차는 **화면 범위 선택자**(`.memgrid .btn`)로 남김(값 통일은 시각 결정이라 범위 밖) · 게이트 e(맨 클래스 정의는 `primitives.css` 에만) | 정의 자리: 모달 115 · 표 106 · 입력 63 · 버튼 40 · 카드 38 · 칩 26 · 탭 17 → 계열 2~3개씩 PR 분할 가능 | 캡처 차이 0 · 게이트 green | 편차를 범위 선택자로 남기므로 시각 무변. 편차 통일은 P5 뒤 별건 |
...
순서는 P0 → P1 → P2a → P2b → P3(+P4) → P5. P2 의 분할은 Q6(단계당 PR 1건 순차)의 형태 안에서 단계 수만 늘린 것이며 원한 결과는 같다. **P0+P1+게이트에서 멈추는 선택지**(Q6)도 성립한다 — 가장 큰 표류원(토큰 산재·죽은 선언)을 막고 새 표류를 게이트가 잡는 상태까지만 가는 안이며, 보정 층과 중복 클래스는 그대로 남는다.
