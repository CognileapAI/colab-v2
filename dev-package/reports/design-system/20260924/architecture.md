...
| **P2 프리미티브 단일 소유** | `primitives.css`·`base.css`·`patterns.css` 신설 · `.btn/.chip/.card/.modal/.field/.table` 을 한 곳으로 · 중복 제거 · **`@layer` 를 전 파일에 한 번에** · `design-system.css` 223행 해체 · 게이트 e | 전 CSS 19파일 접촉 · 보정 규칙 약 190 셀렉터 재배치 | 캡처 차이 0(예외 표) · 게이트 green · vitest 원문 시험 green | 특이도 변화로 미세 차이 → 캡처 대조가 잡는다 · 중간 상태 금지 |
...
순서는 P0 → P1 → P2 → P3(+P4) → P5. **P0+P1+게이트에서 멈추는 선택지**(Q6)도 성립한다 — 가장 큰 표류원(토큰 산재·죽은 선언)을 막고 새 표류를 게이트가 잡는 상태까지만 가는 안이며, 보정 층과 중복 클래스는 그대로 남는다.
