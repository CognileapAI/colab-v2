...
- 등록 **뒤** 사람 값을 고치는 화면 칸도 없다 — `frontend/src/components/detail/editFields.ts` 의 편집 칸은 7개(`name`·`summary`·`sourceLabel`·`sourceUrl`·`sourceDownloadedOn`·`crs`·`gridDescription`)이고 가공 단계·분류 3축이 없다. `DatasetEditForm.tsx`·`useDatasetEdit.ts` 에 `category`·`dataType`·`processingLevelUserSet` 참조 0건.
  ⟹ **[추론] 잘못 고른 Lv 를 화면에서 되돌리는 경로가 없다** — 계보(부모)를 고쳐 파생값을 움직이는 길만 남는다. [미확인 — 계약 `DatasetUpdate` 에는 `processingLevelUserSet` 이 있다(`contracts/schemas/common.json` 설명 축자); 다른 화면에 그 칸이 있는지는 업로드·상세 밖 전수 grep 으로 닫힌다.]
...
- ⑶ 「이유가 안 보인다」 = **문면**(현재 문장은 두 값만 말하고 왜 갈렸는지는 말하지 않는다).
- ⑷ 「부모 0건 등록에서 경고가 안 뜬다」 = **결함** — 상세에서는 뜨고 등록 화면에서는 안 뜬다. 같은 판정 안에서도 경고가 나야 하는 자리.
...
| I-1 ⑷ 부모 0건 등록에 경고 없음 | 확인 — `derivedPreview` 가 부모 0건에서 `null` ⟹ 비교 불성립 | 결함 | 저촉 없음(판정은 경고를 요구) | S | 불필요 |
