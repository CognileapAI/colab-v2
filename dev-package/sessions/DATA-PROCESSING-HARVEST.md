...
| **HSR 반사도** | G1 `binary/processor.py:72` 는 `crs="EPSG:4326"` 로 **선언만** 하고 bbox 도 근사 고정(`lat_range=(32.0,44.0) lon_range=(120.0,136.0)`, `:69-70`). 실제 좌표는 외부 짝 파일 `rdr_500m_latlon.nc` 에서 온다 **[읽음]**. ⚠ **v2 는 이 파일을 정본으로 쓰지 않는다** — 표준위도가 없어 불완전하다. HSR 격자 정본 = `04.Lat_Lon_info/Lat_HSR.npy`·`Lon_HSR.npy`(`DATA-REFERENCE §1.1`) |
