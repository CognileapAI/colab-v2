...
| # | 항목 | 판정 갈래 | 왜 판정인가 |
|---|---|---|---|
| ① | core-viz `createRender` 의 **`422`** (`core-viz.yaml:98`) | **철회** vs 집행 | 생산자 0. `〈151〉` 의 `202` 와 **동형** — 그때 세운 규칙을 그대로 적용하면 철회다 |
| ② | core-ai `searchDatasets` 의 **`422`** (`core-ai.yaml:142`) | **철회** vs 집행 | 동상. 검증 실패를 `ai-service` 는 전부 `400` 으로 낸다 |
