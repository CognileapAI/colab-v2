...
4. **`DR-8`·`STOP-2` 는 닫혔다** — 정본 갱신 완료(계약 무변경, `§2-12` 처방대로). 개정 시 `core-pipeline.json` 의 포맷 표기는 **갱신된 정본**(`NetCDF·Binary·HDF4·GeoTIFF`) 기준으로 정합을 본다. ⚠ **정정 2026-08-23** — 이 줄이 적힐 때의 「정본 갱신 완료」는 사실이 아니었다. 고쳐진 것은 `planning/README §2` 가 배제한 옛 사본이었고, **260818 정본은 2026-08-23 WU-P2 재개정에서야 실제로 고쳐졌다**(7자리). 결론(닫힘)은 유지되지만 근거는 재개정이다 — `03-HANDOFF §5.5 DR-8` · `sessions/P2-NB-2-canon-amendment-applied.md`.
...
**판정 문구(§8 · 추기-3 반영, ㉢ 이후 갱신)** — **「`STOP-1` 은 닫혔다. `DR-13` 은 닫혔다. ㉢ 승인·동결 성립(2026-08-23). P2 의 계약측 유보는 전부 해소됐다 — staging 실측만 남는다.」**
...
| 7-8 ㉢ Ted 승인 | ✅ **승인 (2026-08-23)** | **6건 일괄 승인 + 동결 명시 지시** — 판정 기록은 `sessions/D2c-ted-approval.md` ㉢ 판정 기록 절. **계약 동결 성립** |
