...
- **⚠ `RenderTarget` 주소지정은 `〈58〉` 로 전제가 바뀌었다 — 판정만 하고 편집은 멈춤 규칙을 탄다.**
  - `RenderTarget` 은 `uploadId` **하나**로 업로드를 가리키고, 그릴 조각은 `fileIds` 로 고른다(`core-viz.yaml:260-265`).
  ...
  - **`[추론]` — 따라서 현행 `RenderTarget` 은 `〈58〉` 아래서도 그대로 성립할 가능성이 높다**(격자는 대상이 아니라 부속). **그러나 확정하지 않는다** — `〈58〉-③` 이 「프런트가 **격자 파일로 지도를 그린다**」를 요구하므로, 격자 자체를 그리는 요구가 있으면 `fileIds` 의 본체 한정이 부족해진다.
...
| **NB-D** | **`〈58〉` 아래서 렌더 대상을 무엇으로 주소지정하는가** | `〈58〉-③` 은 「프런트가 격자 파일로 지도를 그린다」까지만 말한다. `core-viz.yaml:260-265` 의 `fileIds` 는 이미 「**본체 파일**」로 한정한다 | **`[추론]` — 현행 모양이 그대로 성립할 가능성이 높다. 확정하지 않는다.** `core-viz` 편집이 필요하다는 결론이면 **멈추고 보고**(`§5-6`) |
