...
> ⭑ **2026-08-28 갱신 — 기구는 전건 섰고, 야간 무인 실행으로 넘어갔다.** 등재 = `PLAN-SoT §9 〈173〉`.
> **닫힌 것** = crontab 재설치(03:30 = `backup-full.sh` · 본 체크아웃 · Ted 실행) ＋ 운영 데이터 대상 **유인 백업 1회 GREEN**.
> **남은 것 = 크론 환경에서의 첫 회차 로그 하나와, 차단성 아닌 셋**(아래 12번 · `§8`).
> ／ 이전 ⭑ **2026-08-27 갱신 — 이 표는 이제 「남은 것」이 아니라 「무엇이 섰고 무엇이 안 섰는가」다.** 기구는 대부분 섰고, **남은 것은 리허설 1건과 그 리허설이 낳는 실측 몇 개**다.
...
| 12 | **야간 무인 실행 — 스케줄이 실제로 도는가** | 🟧 **절반 닫힘 (2026-08-28 · `PLAN-SoT §9 〈173〉-㉰`)** — **crontab 재설치 ✅**: 03:30 대상이 `backup.sh` → **`backup-full.sh`**, 경로는 **본 체크아웃**(임시 폴더 아님), 월요 04:10 `latest-check.sh` 유지 — 전부 실측. 본 체크아웃 실측 = `infra/staging/backup/` **22 파일** · `infra/staging/restore/` **11 파일**. 운영 데이터 대상 **유인 백업 1회 GREEN**(2026-08-28 10:09~10:10 · 36초 · `uploads` 341,527,870 B · 매니페스트 135 = 아카이브 135 · **원장 129건 전건 포함** · 초과 6건 = 덤프 이후 접수분 · `previews` 39건은 **명시 면제 SKIP** · 보관처 규약 밖 파일 0건). ⛔ **남은 것 = 크론 환경 첫 회차 로그** — **오늘 밤 03:30 이 첫 회차다.** 확인 = 로그 끝에 **전범위 백업 성공 출력**이 있는가 · 보관처에 **실패 표식이 없는가**. **손으로 돌린 GREEN 은 기구가 아니다** |
