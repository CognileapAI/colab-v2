...
| `P2.md` | 업로드·계보 확정·S-08 — **확정 설계 정본** | ✅ 진입 전건 전부 닫힘(`STOP-1`~`STOP-4`·`NB-A`). 실행은 `P2-EXEC.md` 가 지시한다 |
| `P2-EXEC.md` | **P2 실행 지시** — 레인·물결·착수 전 판정·완료 판정 | 🟧 **다음 WU · 착수 가능.** 2026-08-23 계획 세션 · advisor 게이트 ① `go-with-fixes` · Ted 판정 4건(`〈63〉`) 반영 — `P2-viz` 레인 신설, 물결 5단계, 레인 7 |
| `TED-RULINGS-20260830-CANON.md` | **Ted 판정 셋 집행** — `〈230〉` `P5` 완료 정의 순환 제거·닫기 · `〈231〉` 11차 동결 해제(스크린샷 중계 op) · `〈232〉` 확대(줌) 정본 근거·완료 기준 | ✅ 닫힘 — 게이트 before/after 동일(green 31 / red 2) |
...
| `F2.md` | **F-3 파일 관리**(구 `F-2` — 병합에서 개명 · 파일명은 그대로 둔다) — 목록·폴더 구조·다운로드·본체 변경 (`〈278〉〈280〉`, 브랜치 `feature/rtf400_s3_upload`) | 🟦 진행 (2026-08-29) — **Ted 판정 대기 ㉯**(8차+9차 한 번에) |
