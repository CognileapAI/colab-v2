...
- (수정 라운드 정정) 브라우저 여정 스크립트 `scripts/upload-preview-journey.py:161` 은 위험이 아니다 — 두 분기 앞의 `:156` 이 `up-style-palette` 의 `options.length > 0` 을 기다리고, 제품은 같은 `.then` 에서 `setPalettes`·`setPalette` 를 함께 세우므로 선택지가 서면 `palette` 도 서 있다. 따라서 `:161` 클릭 시점에 버튼은 활성이다. `scripts/issue-73-82-journey.py:159` 는 팔레트를 고른 뒤 누르므로 같다. 두 스크립트가 어떤 게이트에도 걸리지 않는다는 사실은 그대로다.
- UI 동작 변경(버튼 비활성)의 실제 브라우저 검증은 이 레인에서 하지 않았다(agent-browser 기동 0). jsdom 시험만으로 판정.

## 9. 수정 라운드 (Fable advisor ② 요구)

- 지적: 「짝 파일 없이 그려 보기」(`up-preview-without-grid`)도 `draw(true)` 로 같은 `draw()` 를 부르는데 `disabled` 가 없어 팔레트 도착 전 클릭이 같은 자리에서 버려진다. `upload.test.tsx` 의 그 버튼 단언·클릭도 활성 대기 없이 수행.
- 기준 HEAD `eda730f6` · lifecycle task `72d7fba331a148eb8a2d404aacf1f4ac`(role lane-worker · 선언 게이트 4 동일).

| 단계 | 커밋 | 변경 |
|---|---|---|
| 시험(RED) | `a63baa13` | 헬퍼 `clickPreviewDrawWhenReady` 에 `testId?`(기본 `up-preview-draw`) · F-ci 시험 ⑤ 3건 · `upload.test.tsx` 2곳 |
| 구현 | `5b51a00e` | `frontend/src/components/upload/PreviewPanel.tsx` 1파일 +3 — `up-preview-without-grid` 에 `disabled={!palette}`. 새 색·토큰·문구 0 |
| 보고 | 이 파일의 커밋 | 이 파일 — §8 `:161` 줄 정정 ＋ 이 절 |

- F-ci 시험 ⑤(`hasReferenceGrid={false}` 마운트): ⑤-1 팔레트 전 disabled · 그 사이 클릭은 `createRender` 0회 ⑤-2 `palettes()` 실패 → disabled ＋ 기존 `up-preview-error`「지금 미리보기를 만들 수 없어요.」 ⑤-3 팔레트 도착 뒤 enabled · 헬퍼(`testId: 'up-preview-without-grid'`)로 누르면 `withoutReferenceGrid: true`·`style.palette='viridis'` 로 1회.
- `upload.test.tsx` — `:559` 활성 단언을 `waitFor` 로, `:567` 클릭을 `clickPreviewDrawWhenReady({ testId: 'up-preview-without-grid', click })` 로. `grep -rn "up-preview-without-grid\|짝 파일 없이" frontend/test scripts` 로 다른 클릭 자리 0 확인(`upload.test.tsx:2625` 는 부재 단언).
- RED(시험 커밋 트리 · 제품 변경 전): `design-fix-20260924-F-ci.test.tsx` `Tests 3 failed | 5 passed (8)` — ⑤ 3건 모두 `expect(element).toBeDisabled()` · `Received element is not disabled`. 같은 트리에서 `upload.test.tsx` `Tests 139 passed (139)` · `tsc --noEmit` 종료 0.
- GREEN(구현 커밋 뒤): F-ci ＋ `upload.test.tsx` `Tests 147 passed (147)`. 게이트 run 은 레인 최종 메시지의 `COLAB_HANDOFF` 줄에 적는다.
- 초과 0. 실제 브라우저 검증은 이 라운드에서도 하지 않았다(agent-browser 기동 0).
