...
- **미달** — ⑴ 위 규칙 1 의 `up.sh` 이중 검사 1건(확정으로 제외) ⑵ 라운드 §2 수용 기준 「`deploy.sh` 드라이런 → 원장 행 끝 `브랜치=integration/r-d`」는 드라이런 부재로 정적 grep 대체 · 값 실측은 다음 staging 회차로 이월. 그 밖 0건.
...
## 10. advisor ② 반영 (2026-09-08 · 브랜치 `lane/wu-d2d3-fix`)

1. **낡은 「14」·「없다」 문면 → 구현 상태.** `CLAUDE.md` §0 완료 조건(14/14 → 15/15 · 개정 표식 1회)·§9(14 항목 → 15 항목) · `.claude/rules/deploy.md`(14 항목 → 15 항목) · `infra/dev/README.md`(손으로 재는 한 줄 → 「게이트는 `ship.sh` 에 있다」 · 항목 15 행 추가) · `docs/BRANCHING.md`(§1 「아직 코드에 없다」 · §2 「신설 예정」 · §5 「우회(계획)」·⚠ 「게이트도 우회 경로도 없다 · 14항목」·「그때까지의 대체」) · `deploy_doctor.py:115` 주석 「나머지 13」 → 14. 규칙 6 축자 블록 무수정 — §2-1 diff **0행 유지**.
2. **⑮ 보장 범위 1줄** — `docs/DEPLOY.md §6-1` ＋ `infra/dev/README.md` 되돌리기 절. 축자 = 대조 대상은 마지막 `ship.sh` 반입 sha · `dev.env` 롤백(`COLAB_IMAGE_TAG` ＋ `up.sh`)은 `CURRENT_SHA`·`MAIN_SHA` 를 갱신하지 않으므로 롤백 뒤 ⑮ 는 직전 반입 판정을 유지한다.
3. **§7 미달 +1** — 원장 `브랜치=` 실측 이월(위).

**운영 메모 (advisor 지적 2건 · 코드 변경 0)**
- **detached HEAD 에서 `브랜치=` 공란** — `ledger_append` 의 `git branch --show-current` 는 detached 에서 빈 문자열이다. staging 리허설을 태그·sha 체크아웃으로 돌리면 규칙 2 의 브랜치 기재가 빈 값으로 남는다. 리허설은 `integration/*` 브랜치 체크아웃 상태에서 돈다.
- **`tag-release.sh dev` 의 N 충돌** — 같은 날 태그 수＋1 로 N 을 정하므로 태그가 손으로 지워졌거나 원격에만 있으면 기존 이름과 겹쳐 exit 65 다. 손 조치 = 원격 태그를 fetch 한 뒤 재실행하거나 이름을 직접 지정한다.
