...
⭑ **red 5건의 뿌리 = 4종**(실측) — ⟨정정 2026-09-08 · advisor ② WU-D6⟩ 종전 ~~「(다) 4건 한 뿌리(exit 78)」~~ 는 회차 로그 대조 전 추정.
- ⓐ **`78` 상수 부재** — H14 · H18. `SKILL.md §4` 문안에도 픽스처에도 숫자가 없고 상태 **이름**만 있다.
- ⓑ **자유 서술 칸에 낱말 요구** — H15.1 · H16.2. 판정은 옳은데 expect 가 `red`·`실패`·「건수」 **낱말**을 요구한다.
- ⓒ **음성 정규식이 부정문 오탐** — H16.1. 「통과시키지 않는다」 안의 `통과` 를 음성 단언이 잡는다.
- ⓓ **판정 라벨 모호** — H09.2. 영어 응답 ＋ 「없음」 오독 · 과제문이 「있음」의 뜻을 못 박지 않았다.
...
**미달 2** — ⑴ 2/2 green **15/20**(막는 것 = §3 뿌리 4종 ⓐ~ⓓ) ⑵ 승격 **3회 연속** 조건에서 이 회차가 red 라 **연속 계수 0**(다음도 1회째).
**초과 2**(근거 병기 · ⟨정정 2026-09-08 · advisor ② WU-D6⟩ 종전 ~~3~~ — `results/.gitignore` 부분집합은 지시 ⑸ 집행이라 초과가 아니다) — ⑴ README 실행 예시 값 `180/0.50` → `93/2.01` 갱신 ＋ 초안 예산 경고(그대로 두면 복사한 사람이 20건 전부 red(준비)) ⑵ README 「결과」 에 `H??.expect.{1,2}.txt` 추가 기재(`run.sh:190` 이 쓰는데 D5 목록에 빠져 있었다).
