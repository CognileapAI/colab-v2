...
| # | 판정 대상 | 종류 | 열리는 것 |
|---|---|---|---|
| ① | ~~`F-1` 구간 조절 select — 코드를 걷을지 완료 정의를 고칠지~~ **판정 완료 `PLAN-SoT §9 〈210〉`** | 제품 판정 | `F-1` 닫기 |
| ② | ~~`K3` 완료 정의의 합격선 수치~~ **판정 완료 `PLAN-SoT §9 〈211〉`** | 제품 판정 | `K3` 닫기 오라클 |
| ③ | `R-1` 상태를 하나로 확정 (3문서 갈라짐 · 잔여 셋 실측 후) | 실측 판정 | `I4` · `#22` · `전범위백업` · `W-1` |
| ④ | `S2` 계수 외 잔여 유무 · `S2b` 평가셋 재판정 | 실측 판정 | `W-1` red 해소 · `S3` |
| ⑤ | ~~삭제할 고아 행 id 목록 확정~~ **판정 완료 — 삭제 철회 `PLAN-SoT §9 〈216〉`** | 판정 | `#22` **종결** |
| ⑥ | **부분 판정 완료** — `전범위백업` 기재 `〈213〉` · `J-1` **범위만** `〈214〉` · `V-1`·`V-2` 는 **타일 단계 뒤로 보류**(`완료 정의 미작성` 유지 · `〈213〉`-㉲) | 정의 판정 | 해당 4건의 닫기 기준 |
| ⑦ | ~~`unknown` 5건 단계 배정~~ **판정 완료 `PLAN-SoT §9 〈212〉` — `unknown` 잔여 0건** | 범위 판정 | 사슬 B 착수 순서 |
| ⑧ | `PV-1` 순서표 자리(몇 단인지) · 미리보기 산출물 수신 창구의 형태 · `layout.json` 렌더 슬롯 재사용 여부 | 설계 판정 | `PV-1` 집행 세부 |
| ⑨ | ~~`I4` 복원 리허설이 `R-1` 을 흡수하는지~~ **판정 완료 — 분리 유지 `PLAN-SoT §9 〈215〉`** | 범위 판정 | `I4` 범위 확정 |
...
- ⭑ **2026-08-29 — 위 10건 중 7건이 닫혔다**(① ② ⑤ ⑥부분 ⑦ ⑨ ＋ 검색 품질 합격선 `〈217〉` · ⑩ 은 이미 끝난 판정). **남은 것 = ③ ④ ⑧ ＋ ⑥ 의 지도 관련 둘.** 값·근거는 `PLAN-SoT §9 〈210〉`~`〈217〉`
- ⚠ **이 문서의 §1·§4·§5 표는 2026-08-29 오전 시점 값이다** — 위 판정으로 `#22`(종결)·단계 배정 5건·완료 정의 2건이 달라졌다. **다시 재기 전까지 그 표의 값을 현재로 읽지 않는다**
