...
메타 — 발의자: 사례 제공자 #1·#2(실무자 사례) · 정리: Claude(researcher, 대조 조사) · 작성 2026-09-18 · 승인 **승인 2026-09-18 (Ted, 「다 권고대로」)**

**판정 결과(2026-09-18, Ted):** 아래 8개 결정 전부 권고대로. (v) 픽스처 선행. (ii)·(iii)·(iv) 는 이 회차에서 열지 않는다. `PLAN-SoT §9-㊴-②` — 「정본에 없으면 만들지 않는다」.

⚠ **(i) 19행은 두 회차에 나누어 전부 적재됐다.** 1회차(`0009_practitioner_lexicon`) 13행 = 동의어 9 + 지명 4. 2회차(`0010_practitioner_concept`) 6행 = 원천표기 3노드 + 주제 2노드 + 엣지 1. 2회차를 연 것은 **2026-09-18 Ted 판정**이고, 그 판정이 결정 4·5 의 「보류 — 재검토 대기」를 닫았다(아래 4·5).

## 사례 출처 (원본)
- **#1 검색 상황 6건 — 사례 제공자 #1 제공** (적합 데이터셋 / 특성 차이 / 기간·지역 매칭 / 해상도 조건 / 관측-보조자료 매칭 / 원본-가공 구분)
- **#2 질문·기대답변 8건 — 사례 제공자 #2 제공** (기대답변에 `XXXX` 자리표시자 포함)
...
⭑ **2회차 동반 개정 (결정 4·5 채택 뒤).** `db/ai/seed/k2b-graph-standard.tsv` 노드 **49→54** · 엣지 **19→20**, `db/ai/tools/k2b_graph_check.py` `CANON_TOPICS` **4→6**(`SYNONYM_TOPICS` 를 같은 값으로 합쳤다). 둘 다 **손으로** 옮겨 적었다. `APPROVED_G6` 9줄은 **무변경** — 새 엣지가 등급 ⑤(기획 정본 어휘)라 승인 목록이 흔들리지 않고, 등급 ⑥ 노드도 그대로 0건이다.

⚠ **`k2-coverage-standard.tsv` 와 `k2_coverage_check.py::EXPECTED` 는 2회차에 손대지 않았다.** 그 오라클의 `topic` 사실은 `k2-coverage-check.sh` 의 `SELECT 'topic', topic FROM d9_topic_synonym` 이지 개념 노드가 아니다. 개념 주제 노드를 4→6 으로 늘려도 그 열은 움직이지 않으므로 기준 4행은 그대로가 맞다(실측 — 2회차 뒤 `k2-coverage-check.sh` exit 0 · 주제 4/4).

⚠ **출처 등급 문제.** `K1b-ONTOLOGY-CONTENT.md:28` 의 등급 ①~⑥ 어디에도 「실무자 사례」가 없고 `source_grade` CHECK 는 `BETWEEN 1 AND 6` 이다. 9행 동의어만 이 문제에 걸린다(지명 4행·ERA5 3노드는 등급 ②·⑤ 근거가 따로 있다). 권고 = `topic_synonym_six.sql` 선례대로 **Ted 가 값을 직접 판정해 내려보내고** `source_note` 를 「Ted 판정 2026-MM-DD — 실무자 사례 #1-6(사례 제공자 #1)·#2-1(사례 제공자 #2) 검토」로 적는다. 스키마·등급 무변경.
...
2. Ted 가 (i) 19행 각각에 채택/기각/보류를 판정하고, 채택분만 새 시드 SQL + 새 alembic 리비전 + 기준 TSV 동시 개정으로 들어간다. **⭑ 2026-09-18 로 19행 전부 채택·적재 완료다** — 1회차 13행(`0009`) + 2회차 6행(`0010`). 보류로 남은 행은 0건이다.
...
4. **ERA5 · ECMWF · 유럽중기예보센터를 원천표기 노드로 실을 것인가.** 2026-08-25 F-12 ㈎ 의 보류 사유(「15 데이터셋에 그 원천이 0건이라 결과를 안 바꾼다」)가 사례 6건에서 무너진다.
   → ~~보류 — 재검토 대기~~ → **채택 2026-09-18 (Ted).** 판정 축자 — 「**없어도 앞으로 추가될거니까 넣자**」.
   **이것은 재개봉이다.** `dev-package/sessions/K1b-ONTOLOGY-CONTENT.md:346` 이 F-12 를 ㈎(뺀다)로 적었고 그 근거 규칙은 같은 줄의 축자 「결과를 안 바꾸는 엣지는 시드하지 않는다」였다. 그 규칙이 딛고 선 사실은 「15 데이터셋에 ECMWF·ERA5 원천이 0 건」이고, 이번 판정은 그 사실 대신 **앞으로 들어올 자료**를 근거로 세운다. 그래서 **결정 4 가 F-12 ㈎ 를 대체한다** — F-12 ㈎ 를 틀렸다고 말하는 것이 아니라 그 전제가 바뀌었다고 말하는 것이다. 적재 = `0010_practitioner_concept` · `db/ai/seed/practitioner_concept_nodes.sql`.
   ⚠ **어휘가 열린 것은 `ECMWF ↔ 유럽중기예보센터` 뿐이다.** `ERA5` 노드는 들어왔으나 **닿는 엣지가 0행**이라 그래프가 「ERA5」 질의를 넓히지 않는다. `ECMWF → ERA5` 를 적을 관계값 `~이 제공한다` 는 결정 8 이 닫아 둔 자리다. 오라클 `eval/k4-search/practitioner-lexical.json` 의 `LEX-15` 를 blocked 로 남긴 이유가 이것이다.
5. **개념 그래프 주제 노드를 4→6 으로 맞출 것인가.** `k2b_graph_check.py:42 CANON_TOPICS` 상수 개정이 따라오고, 그 상수는 2026-08-25 판정을 박제한 자리다.
   → ~~보류 — 재검토 대기~~ → **채택 2026-09-18 (Ted).** 판정 축자 — 「**가뭄 파일포멧 예제 넣자. 데이터가 들어오면 검색이 되어야 한다**」.
   보류 사유였던 「사례 요구 0건」을 판정이 **앞으로 들어올 데이터**로 바꿔 세웠다. `CANON_TOPICS` 주석이 내걸었던 조건은 축자 「노드를 늘리려면 `k2b-graph-standard.tsv` 라는 별도 기준을 함께 고쳐야 한다」였고, 2회차가 그 둘을 **같은 커밋에서 손으로** 고쳐 조건을 충족했다. 사전(`d9_topic_synonym`)은 `0006_topic_vocab_six` 로 이미 6값이었으므로 이것은 새 주제 값을 만드는 것이 아니라 **사전-그래프 비대칭을 없애는 것**이다.
...
- Q4 ERA5·ECMWF·유럽중기예보센터를 원천표기 노드로 싣는가 → A ~~보류~~ → **채택 2026-09-18 (Ted)** 「없어도 앞으로 추가될거니까 넣자」 (반대 관점은 남는다: F-12 ㈎ 의 재개봉이고, 오늘도 그 원천을 가진 자료는 0건이라 **오늘의 검색 결과는 하나도 바뀌지 않는다**. 판정은 그것을 알고 내려졌다)
  - Q4a 그러면 「ERA5」 질의는 열리는가 → A **아니다.** 노드는 들어오지만 엣지가 0행이고, `ECMWF → ERA5` 를 적을 `~이 제공한다` 는 결정 8 이 닫았다. 열린 것은 `ECMWF ↔ 유럽중기예보센터` 한 쌍뿐이다
- Q5 그래프 주제 노드를 4→6 으로 맞추는가 → A ~~보류~~ → **채택 2026-09-18 (Ted)** 「가뭄 파일포멧 예제 넣자. 데이터가 들어오면 검색이 되어야 한다」 (반대 관점: 사례 요구는 여전히 0건이다. 근거는 사례가 아니라 **사전이 이미 6값인데 그래프만 4개이던 비대칭**이다)
...
- ~~`s-ecmwf` / `s-ecmwf-ko` 의 `src < dst` 정규형 정렬 순서~~ → **닫혔다.** `s-ecmwf` < `s-ecmwf-ko` 라 `('s-ecmwf','같은 말이다','s-ecmwf-ko')` 한 행이 정규형이다(2회차 적재 실측 · `k2b-graph-check.sh` exit 0).
- **`ERA5` 어휘는 여전히 0건이다.** 노드는 있으나 닿는 엣지가 없고, 그것을 이을 `~이 제공한다` 는 결정 8 이 닫아 두었다. 사례 6건이 ERA5 를 **명시로** 부르므로 이 자리는 결정 8 을 다시 열 때의 첫 후보다.
...
- Ted 확인 문장: **「다 권고대로」(2026-09-18)** — 결정 1=㈐, 2=9행 전부, 3=채택, 4=채택, 5=채택, 6=이번 회차 미개방, 7=이번 회차 미개방, 8=후자(관계 CHECK 무변경).
  ⚠ **이행 범위는 그 뒤 한 번 좁혀졌다가 다시 열렸다.** 승인 문장은 그대로 둔다(사용자 발화를 고쳐 적지 않는다). 1회차의 실제 적재는 결정 1·2·3 의 **13행뿐**이었고 결정 4·5 는 「보류 — 재검토 대기」로 내려갔다. **2026-09-18 Ted 가 그 둘을 각각 축자로 닫았다** — 결정 4 「없어도 앞으로 추가될거니까 넣자」 · 결정 5 「가뭄 파일포멧 예제 넣자. 데이터가 들어오면 검색이 되어야 한다」. 2회차(`0010_practitioner_concept`)가 남은 6행을 적재했다.
- 프론티어 공집합 확인: 미수행.
- 재개봉 금지: **해당 있음 — 결정 4 다.** 결정 4 는 2026-08-25 `F-12 ㈎`(「뺀다」)의 **재개봉**이고(`K1b-ONTOLOGY-CONTENT.md:346`), 1회차는 그 때문에 적재를 미뤘다. **2026-09-18 Ted 판정이 재개봉을 승인했다** — 축자 「없어도 앞으로 추가될거니까 넣자」. **이 판정이 `F-12 ㈎` 를 대체한다.** 대체의 내용은 「F-12 ㈎ 가 틀렸다」가 아니라 **그 판정이 딛고 있던 사실(「15 데이터셋에 ECMWF·ERA5 원천이 0 건」)을 근거로 삼지 않기로 했다**는 것이다. 기록은 여기에만 둔다 — `K1b-ONTOLOGY-CONTENT.md` 는 2026-08-25 저작 시점의 세션 문서이고 그 뒤 어느 회차도(주제 6값을 연 `0006` 포함) 그 파일에 덧붙이지 않았다. 나머지 결정에는 재개봉이 없다.
