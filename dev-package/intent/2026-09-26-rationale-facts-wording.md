...
메타 — 발의자: Ted · 작성 2026-09-26 · 승인 미승인 · 선행 intent `dev-package/intent/2026-09-25-search-rationale-separation.md`(승인 Ted 2026-09-26 · 패널·근거 종류·사실 규칙의 정의처)
...
## 미해결 질문
- Q2b 라벨만 맞은 카드의 문형(「‘강우·강수’가 이름·주제·요약에 맞았어요」 유지 / 「주제 ‘강우·강수’에 맞았어요」). 구현 때 정하고 PR 요약에 적는다.
- 관찰(이 intent의 결정 대상 아님): `hsr_sample` 카드 「낱말 일치」 항목의 자리 이름에 「확인한 파일 근거」가 섞인다(「…이름·주제·요약·확인한 파일 근거에 맞았어요」). `dataset_search.py:104`가 `match.where` 전체를 잇고, `where`에 `hit_evidence` 이름이 들어가기 때문이다(`d3_catalog.py:1078`). 같은 카드에 「파일 근거」 종류가 따로 서지 않았다. 다룰지 여부는 Ted 판정 대상이다.
...
- 앵커 중복 생성기 수정과 dev 데이터 재적용(Q1b · 후속 항목).
- 필드별 일치 계산(Q2c) · ai-service 사전 확장 변경(Q2ⓑ).
- 정본(`40 COLAB-기획/…`) 파일 수정.

## 확인
- 프론티어 공집합 확인: 미실시(미승인 초안)
- Ted 확인 문장(원문 그대로): 발의 「1,2 둘다 intent로 올리자」(2026-09-26). 승인 문장 없음.
- 재개봉 금지: 아니오
