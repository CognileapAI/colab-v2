...
메타 — 발의자: Ted(2026-09-24 「권고대로 하자」) · 정리: Claude(researcher) · 작성 2026-09-24 · 승인 **㈎-1·㈏ 「권고대로 하자」(2026-09-24 Ted) · 게이트 ① 결정 5건 「좋아 권고대로」(2026-09-24 Ted) — 아래 「판정 기록」 절**
...
## 판정 기록 (2026-09-24 Ted · 「좋아 권고대로」) — 게이트 ① 결정 5건
1. **후보 필터** = `filtered`(주제 ∪ 이름·파일명 토큰)를 **잠정 기본값**으로. 근거 J1(엣지 6: recall@5 6/6 vs recent 2/6, `dev-package/reports/k3-lineage-probe/README.md`). dev 28건 코퍼스 재측정을 후속으로 남긴다.
2. **서버 선행 허용** — E-04 가 제안을 부르지 않는 상태에서 계약·생산자를 먼저 세운다. **플래그 기본 `off`**. 화면 복원(㉮)은 생산자가 참인 제안을 낸 뒤 별도 판정.
3. **계약 개정 서명** — `LineageSuggestionRequest.candidates` 선택 필드 + `LineageParentCandidate` 신설(필드 집합은 아래 미해결 2 의 권고 그대로). optional 추가라 파괴 아님.
4. **계약 산문 정정** — `core-ai.yaml:69-71` 「D9 지식 그래프 조회를 근거로」를 같은 개정에서 삭제.
5. **D10 실행 원장**(㊷ 공급자·처리 리전 기록)은 이 intent 범위 밖 · **K3·K4 공통 별건 intent** 로 뺀다.
6. k=20 · 플래그 기본 off — 권고 수용.

## 판정 기록 2회차 (2026-09-24 Ted · 「좋아 권고대로 하고, 결정4 로 하는데 …」) — luna 실측 뒤 결정 4건
7. **자식 자신 제외** — 업로드의 파일명과 이름 초안이 **모두** 같은 데이터셋만 후보에서 뺀다(이름만 같은 다른 판본은 남긴다). 수정 뒤 J5 대조군 재실측.
8. **0건 사유 분리** — 0건의 사유를 `degraded` 와 분리해 항상 싣고, 「살펴봤는데 후보가 없다」는 `degraded: false`(〈148〉 규율). 계약 응답 스키마에 손이 가면 선택 필드 추가로 한정하고 그 사실을 보고한다.
9. **develop 병합** — 7·8 수정과 재실측 뒤 한 번에.
10. **실행 원장** — 별건 intent(`2026-09-24-d10-model-call-ledger.md`) 승인. 리전 없이 모델·캐시율·토큰·지연·결과를 적재.

## 미해결 질문 (2026-09-24 판정으로 닫힌 것은 취소선 대신 위 「판정 기록」이 정본이다)
