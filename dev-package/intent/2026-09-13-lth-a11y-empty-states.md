...
- Q1 `D-2` 를 카드만 고칠 것인가 → A 판정 대상이다. 사용자가 보는 것 = 키보드만 쓰는 사용자가 ⓐ 카드 보기에서는 상세로 들어가고 표 보기에서는 못 들어간다 ⓑ 네 자리 모두 들어간다.
  ...
  - Q1b 권고 → A **ⓐ 카드 한 자리만 이번 회차** — 표 3자리(`ProjectTable`·`CatalogTable`·`ProjectDatasetTable`)는 같은 항목의 후속으로 등재한다. **(권고안 수용 · Ted 2026-09-13 — 카드 ⑥ 권고 · Ted 2026-09-13)**
- Q2 `D-6` 의 범위 → A 판정 대상이다. `/lab` 도 같은 상태다(`frontend/src/routes/LabPage.tsx` 에 `<h1` 0건 · 카드 제목이 h2). h1 을 두는 화면은 4곳뿐이다(기준 = `grep -rn "<h1" frontend/src/routes` — `DatasetsPage`·`ProjectsPage`·`ProjectDetailPage`·`SearchResultsPage`).
  ...
  - Q2b 권고 → A **ⓑ 로 규약을 정하고 이번 회차는 ⓐ 범위로 집행**. 나머지 화면은 같은 항목의 후속. **(권고안 수용 · Ted 2026-09-13 — 카드 ⑦ ⓑ 규약 확정 ＋ 이번 회차 ⓐ 집행 · 게이트 신설은 후보 등재)**
...
- Q4 `D-7` 은 오탈자인가 → A 아니다. 사용자에게 보이는 자리 2건이 전부 목업 축자이고(`frontend/src/components/dashboard/TodoInbox.tsx` 앵커 `<h2>할 일 함</h2>` · `frontend/src/components/members/MemberPermissionGrid.tsx` 앵커 `승인 위임을 켜면 그 연구원의 할 일 함에 접근 요청이 들어오고`) 같은 표기가 계약·도메인·권한 원칙·정본 v1.4 에 전부 쓰였다.
  ...
  - Q4b 권고 → A **ⓐ 유지 ＋ 기획자 회신**. 바꿀 이유가 생기면 기획 정본에서 먼저 정한다. **(권고안 수용 · Ted 2026-09-13 — 현 표기 유지 ＋ 기획자 회신 · 작업 0)**
- Q5 `I-7` 은 재현되는가 → A 코드에서 재현되지 않는다. `frontend/src/auth/LoginPage.tsx` 앵커 `if (response?.status === 401) {` 안은 `setPassword('');` 하나이고 `setAccountName('')` 호출은 레포에 없다. 로그인 op 은 전역 401 처리에서 제외된다(`frontend/src/api/client.ts` 앵커 `if (response.status === 401 && !isPublicSessionOp(request.url)) {`).
  - Q5a 그러면 무엇을 하는가 → A 코드 변경 0. 기획자에게 관측 1건을 요청한다 — 실패 직후 ⑴ DevTools Network 의 `POST /sessions` 상태 코드 ⑵ `#accountName` 의 `value` ⑶ `performance.getEntriesByType('navigation').length` ⑷ 비밀번호 관리 도구를 끈 상태의 재시도 결과. **(권고안 수용 · Ted 2026-09-13 — 코드 미수정 ＋ 관측 1건 요청 · 작업 0)**

## 미해결 질문
- 해소(Ted 2026-09-13 · 카드 ⑥) — `D-2` 범위 = **ⓐ 카드 한 자리만 이번 회차 집행**. 표 3자리(`ProjectTable`·`CatalogTable`·`ProjectDatasetTable`)는 범위 밖으로 내려 같은 항목의 후속으로 등재한다.
- 해소(Ted 2026-09-13 · 카드 ⑦) — `D-6` = **ⓑ 로 규약을 세우고 이번 회차는 ⓐ 범위(`/lab-settings`)로 집행**. 나머지 화면은 같은 항목의 후속.
- 해소(Ted 2026-09-13 · 권고안 수용) — `I-6` 문면 = 후보 축자 채택 「연결된 데이터셋이 없어요. 데이터셋 상세에서 이 프로젝트를 고르면 여기에 보여요.」 연결 방법 문장의 실제 경로 축자 `[미확인]` 은 구현 레인에서 확인한다.
- 해소(Ted 2026-09-13 · SUMMARY §5) — `D-7` = 현 표기 유지 ＋ 기획자에게 「정한 용어」로 회신. 작업 0.
- 해소(Ted 2026-09-13 · SUMMARY §5) — `I-7` = 코드 미수정 ＋ 기획자 관측 1건 요청(Q5a 의 4항목). 작업 0. 관측 결과는 `[미확인]`.
- 해소(Ted 2026-09-13 · 카드 ⑦) — 접근성 검사 게이트 = 새 대장 항목 후보로만 등재. 이 intent 범위 밖.
- `/lab` 의 h1 부재를 같은 항목으로 묶는지 `[미확인]` — `D-6` 범위 판정에 종속.

## 범위 밖 (명시 제외)
- 표 행 3자리(`ProjectTable`·`CatalogTable`·`ProjectDatasetTable`)의 영역 전체 클릭 대화형화 — `D-2` 와 같은 항목의 후속(카드 ⑥ 권고 · Ted 2026-09-13).
