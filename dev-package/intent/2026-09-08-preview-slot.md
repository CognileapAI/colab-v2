...
⛔ **미승인 초안 · R-C 후보 · 발의 2026-09-08 Ted**
...
1. (①) 자리 선점을 **두 화면 모두**에 적용하는가, 아니면 이미 컨테이너가 즉시 서는 상세만 대상인가 — 업로드는 장면1(파일 없음)에도 자리를 만들 것인가, 아니면 장면2 진입 즉시(파일 선택 직후)부터로 충분한가?
2. (①) 자리 크기 기준 — 고정 `aspect-ratio`인가, 최소 `min-height`(px)인가, 데이터 종류(이미지 vs 타일)에 따라 달라지는가?
3. (①) 못 그림 문면의 정본 텍스트 — 현재 `UNAVAILABLE`(업로드)·`UnavailableNotice`(상세) 두 문구가 다르다. Ted 발의의 「지금 못그린다는 표현」이 이 둘을 그대로 쓰는 것인지, 새 통일 문구가 필요한지.
4. (①) 진행 단계 문구가 「지금 띄우는 표현」(`파일 읽는 중…`→`지도 그리는 중…`→`범례 만드는 중…`) 그대로인지, 자리 선점 목적에 맞춰 문구를 더 추가(예: 「자리 확보됨」류 0단계)할지.
5. (②) 상세 페이지 좌우 배치의 반응형 폴백 — 좁은 화면에서 세로로 접히는 분기점(px)과 순서(이미지 먼저인가 정보 먼저인가).
6. (③) 500MB 상한을 **철폐**할지 **상향(구체 값)**할지, 대용량 렌더 방식(ⓐⓑⓒ) 중 무엇을 쓸지, 서버 자원(디스크·메모리·시간) 재실측 없이 결정 가능한지.
7. (④) 선택 축이 파일·변수·시점 중 무엇까지인지 — 계약에 `variable`은 있으나 시점(time) 파라미터는 이번 조사로 못 찾았다. 시점 선택이 필요하면 계약 신설 대상.
8. (④) 기본 선택 규칙 — 대표 파일/대표 변수를 무엇으로 정하는가(첫 파일? 대표 지정값?).
9. (④) 선택을 업로드·상세 양쪽에 다 두는지, 선택 상태를 URL/컴포넌트 상태 중 어디에 유지하는지.
10. (③④) 계약 변경이 필요하면 21차 동결 해제 대상으로 올릴지, 이번 라운드에서 함께 처리할지.
11. (⑤) 기본 배율의 정확한 값 — km/px 또는 지리 폭(경도·위도 범위)을 시스템 상수로 어떻게 못박을지. Ted 의 「대한민국·중국 우측·일본 간사이」 서술에서 「중국 우측」이 한반도를 기준으로 화면 좌측(서쪽, 중국 동해안)을 뜻하는지, 다른 배치를 뜻하는지 확인 필요 — 「그냥 이정도의 축척」으로 크기 등급만 확정되고 정확한 배치·수치는 열려 있다.
12. (⑤) 확대/축소/기본 배율 컨트롤의 위치·형태를 세 화면(상세·업로드·확장보기)이 공유할지, 화면별로 다른 UI를 둘지.
13. (⑤) 배경 지도 레이어의 상세도 — 해안선만인지, 국경·주요 도시 표기까지 포함하는지.
14. (⑤) 자립형 벡터 해안선 반입 시 라이선스·레포 용량 제약을 통과하는지(Natural Earth 는 Public Domain 이나 실물 파일 크기·간략화 수준 확인 필요).
15. (⑤) 래스터가 화면상 작은 유역 하나뿐일 때, 배경 지도 위에서 「지금 어디를 보고 있는지」를 표시하는 방식(하이라이트 사각형? 화살표?).
16. (⑤) `bounds` 응답 열쇠는 이미 있으나 배경 지도 합성/렌더링을 FE 만으로 끝낼지 서버(viz-render)가 관여할지에 따라 21차 계약 변경 여부가 갈린다.
