...
- 수용한 권고 — 이 intent 해당분:
  - ⑴ 실패 시험명은 **공용 표식** 통로로 낸다(`::gate-failure::`). 기존
    `::gate-readiness-failure::` 선례를 따르고 게이트 **행**에 싣는다. `counts` 를 바꾸지 않으므로
    `colab-gate-summary/1` 그대로이며 ADR-0004 재검토 조건에 걸리지 않는다.
    **위치 규약 주의** — `gates/run.sh:77-79` 고정 4필드와 `gate_summary_json.py` 의
    `(parts + [""] * 5)[1:5]` 를 함께 고친다. 이 변경은 #56 과 같은 함수를 건드린다.
  - ⑵-a `upload-transfer.test.tsx:326`·`:348`·`:351` 은 `act` ＋ 동기 조회로 전환한다.
  - ⑵-b `lineage-unknown-20260907.test.tsx:282-303` 은 실측 예산 명시 ＋ 스캔 축소 **둘 다** 한다.
    예산 N 을 정하는 1회 실측을 레인에 허가한다.
  - ⑶ red fixture 는 **vitest 스텁**으로 세운다. 진짜 시간초과 재현은 그 자체가 부하 의존 시험이
    되므로 배제한다.
  - ⑷ 호스트 단독 전수 `-j 4` 를 **3회** 배정한다. #47 의 수집계수 확인과 같은 예산이다.
  - 근거 문면 정정: 「기본값이라 예산이 없다」가 아니라 **「관행 예산이 있고 이번엔 2건만 손댄다」**
    로 적는다. `{ timeout: 4000/5000 }` 이 레포에 이미 널리 깔려 있다.
- PR 단위: **#56 과 한 묶음**이다. 같은 `summary_gate_row()` 위치 규약을 건드리므로
  한 레인에서 순차로 간다. 병렬 레인 금지.
- 재개봉 금지: 예. 재시도로 green 을 만드는 선택지는 이슈가 이미 배제했고 여기서도 배제한다.
