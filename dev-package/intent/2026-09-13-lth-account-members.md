...
- Q3 `I-5` 확장(계정명 재입력 · 활성 관리자 잔존 셈)을 같이 넣는가 → A 별 판정으로 분리한다. 본인 비활성화가 서버에서 막히므로 화면 가드만으로 관측된 불편이 사라지고, 관리자 A 가 다른 관리자 B 를 비활성화하는 경로의 위험은 별도 측정이 선행한다.

## 미해결 질문
- `D-4` 판정 — ⓐ 표에 「비활성」 칩을 붙이고 그 줄의 권한 편집을 막는다(계약 필드 ＋ core-api 읽기 경로 ＋ 스키마 경계 한 칸 개방 · **M~L**) ⓑ 칩만 붙이고 편집은 그대로 둔다(퇴소자 권한을 미리 내릴 수 있음 · 비활성 계정에 권한이 켜진 상태가 남음) ⓒ 후일 묶음으로 유예(작업 0 · 두 화면이 같은 사람을 다르게 말하는 상태 유지). **권고 = ⓐ** — 계정 관리 화면이 이미 「비활성」을 판정 기준으로 쓰고 권한 편집은 되살아난 뒤가 순서에 맞다.
- `D-4` ⓐ 채택 시 경계 처리 — ⓐ-1 상태를 `d1_account` 로 투영(마이그레이션 1건 · 앱 롤 권한 무변) ⓐ-2 `account_admin` 읽기를 `GET /lab/members` 경로에 개방(마이그레이션 0 · 격리해 둔 자리를 한 칸 개방). **권고 = ⓐ-1** — `CLAUDE.md §3` 규칙 1·5 를 건드리지 않는다. `[미확인]` 투영 비용.
- `I-5` 확장 판정 — ⓐ 화면 가드만(**S**) ⓑ 계정명 재입력 ＋ `set_status` 에 잠금 아래 활성 관리자 셈 추가(**M**). **권고 = ⓐ 를 이번에**, ⓑ 는 관리자 2명 환경 실측 후 별 항목. ⚠ 현재 구조가 활성 관리자 0명을 보장한다고 적지 않는다 `[미확인]`.
