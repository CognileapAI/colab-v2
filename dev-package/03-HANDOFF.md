...
**최종 갱신** 2026-08-22 (**G3·G4·G7 닫힘 · D1 배정표 · 정본 이전·통합 · PoC 잔재 정리 · 결정 ㉕~㉞**)
**현재 단계** R1·G5·G1·IS1·**G3·G4·G7 완료** · **기획 정본이 한 폴더로 완결됐고 레포와 같은 작업공간에 있다** · staging 200 유지
**다음 WU** → **P0 의 잔여 차단은 D 트랙 하나이고, D1 은 이름 한 개에 걸려 있다.**
／ **사람 결정 대기(Ted) — 임계경로**: **`HYD`(수문학 도메인 소유자) 실명.** 이것만 들어오면 `DOMAINS.md §7` 에 적고 **D1 을 닫는다** → D2 → D3 → P0. 같은 이름이 G8·K 트랙(블로커 #3)도 푼다
／ **Ted 가 줄 것 (작업 차단)**: **IS2 apply 용 Cloudflare API 토큰** — Custom token, `Account → Cloudflare Tunnel : Edit` 하나면 충분. `~/.colab-v2-staging.env`(0600)에 `CF_API_TOKEN`·`CF_ACCOUNT_ID`·`CF_TUNNEL_ID`
／ Claude 착수 가능: **IS3** staging 백업 체계 (단 데이터가 없어 실효는 P0 이후)
／ 그 외 사람 결정: **G2** 검색 엔진 · **G8** 온톨로지 범위 · **I0** AWS(prod 전용) · `㉝` 스토리 상한 폐기 비토 여부 · G7 이 남긴 `[미확인]`(S-07 소유 에픽) · 정본 `E-00 README §4` 낡은 서술 정정 여부
