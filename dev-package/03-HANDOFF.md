...
**최종 갱신** 2026-08-23 (**P1 착수 — 3/4 레인 닫힘.** 결정 `㊻` AWS 보류 · `㊼` fileCount 메타 · `㊽` 백필 FORCE · **`㊾` 초기 데이터 원천·T-S 트랙 신설 · `㊿` tif 5번째 포맷·npy 제외**)
...
／ **Ted 가 줄 것**: **① GRIB 원파일** (원천 `03 Reference-Data` 에 0건 — S3 이 이것 없이는 안 닫힌다, `SEED-DATA §0-F-1`) · **② 초기 데이터를 받을 연구실** (한 줄이면 S1 이 ✅ 로 닫힌다, `SEED-DATA §9`) · **IS2 apply 용 Cloudflare API 토큰**(`Account → Cloudflare Tunnel : Edit`) · 터널 ID 히스토리 처리(`§4-8`). **OpenAI API 키는 투입 완료 — K3·K4 차단 해제**
...
최근 확정 — ① 신규 구축 ~~② AWS 배포까지~~(**㊻ 로 개정 — staging 까지**) ③ 정본 260818 ④ AI 도메인 분리 ⑤ WU 단위 ⑥ Assistant BC 미계승 ⑦ `colab-v2` 모노레포 ⑯ 레포 public *(전부 2026-08-22 Ted)* · **⑰ G5 대조 상대 = v1 스키마 ⑱ v1 저장 형태 계승 0 ⑲ 값 집합은 DB가 강제 ⑳ 파생값 2종 미저장** *(2026-08-22 G5)*. **㉑ D4→D3 읽기 Port 시그니처 확정 ㉒ "도메인 소유자" = 변경 승인권자 ㉓ staging=터널 호스트·prod=AWS ㉔ PoC 철거·데이터 폐기** *(2026-08-22 Ted)*. **㊵ 승격 게이트(staging 확인 → Ted 승인 → AWS) ㊶ 검색 = Postgres 단독 ㊷ 모델 = GPT 전면 통일(luna/terra, API 키 · Claude 어댑터 휴면) ㊸ 온톨로지 §④ 4항목 확정 — G8 승인, K1 진입조건 충족** *(2026-08-23 Ted)*. **㊹ 미구현 엔드포인트 = 501 + code 두 종(404 아님, 계약 무수정) ㊺ staging postgres 2 DB · 앱 롤 비소유자·NOBYPASSRLS** *(2026-08-23 Claude 판단 · 비토 가능)*. **㊻ AWS 보류 — v2 완주 판정을 `staging 배포 green` 까지로 내리고 prod 는 출시 결정 뒤에 연다** *(2026-08-23 Ted)*. **㊼ `fileCount` 는 메타다 — 조각 수를 `d3_dataset` 메타 층으로 올린다**(잠긴 행에도 `조각 N`) *(2026-08-23 Ted)*. **㊽ 마이그레이션 백필은 `FORCE RLS` 를 한 트랜잭션 동안만 내린다 — 런타임 무변경, 게이트가 복구를 강제** *(2026-08-23 Claude 판단 · 비토 가능)*. **㊿ `tif` 5번째 입력 포맷 · `.npy` 제외 — D5 범위 +1** *(2026-08-23 Ted)* — **미결 부채 2건을 D5 로 넘겼다**(아래 §5.5). **㊾ v2 초기 데이터 원천 = `03 Reference-Data` · 적재는 화면 경로로만 · T-S 트랙 신설 · 완료 조건 8번(「초기 데이터가 화면에서 보인다」) 추가** *(2026-08-23 Ted 요청 · Claude 정리, 비토 가능)*. 열린 결정 **⑧⑨⑩ 해소**(⑪~⑮ 그대로 · **⑯ 즉시 해소** — `㊿`). **㉖(RLS 실효 증명) 은 D3b 로 닫혔다.** 새로 열린 것 — 없음
