...
**최종 갱신** 2026-08-23 (**P1 착수 — 3/4 레인 닫힘.** 결정 `㊻` AWS 보류 · `㊼` fileCount 메타 · `㊽` 백필 FORCE)
...
**다음 WU** → **P1 이어서** — 남은 레인은 **P1-api**(4 op 이 아직 501: `getDataset`·`listDatasetFacets`·`listLabMembers`·`saveLabMemberPermissions`)와 **S-05 상단**. 작업지시서 `sessions/P1.md` 그대로. **I2 를 지났으므로 이제부터 각 WU 의 완료 판정에 `staging 배포 green` 이 붙는다.** 병렬 착수 가능 = **I3**(배포 자동화) · `generated-up-to-date` 게이트 · **IS4 잔여**. **K3·K4 는 키 투입으로 차단 해제됐다.**
...
| P1 카탈로그 + 상세 헤더 | 1 | 🟦 | **착수 · 3/4 레인 닫힘** (2026-08-23). 작업지시서 `sessions/P1.md`. ✅ **P1-db** `d3_dataset.file_count` 메타 열 + 문장 트리거(증분) + 백필 + 드리프트 시험 3종 — `㊼`·`㊽` ✅ **P1-fe-catalog** S-03 8열·헤더 조건·잠긴 행(`조각 N` 포함)·0건, 시험 24 ✅ **P1-fe-members** `구성원·권한` 4스위치·재위임 금지(서버 배열 준수)·확인 모달 1회=요청 1회, 시험 13. **남은 것 = P1-api**(`getDataset`·`listDatasetFacets`·`listLabMembers`·`saveLabMemberPermissions` — 지금은 501) **+ S-05 상단**. 두 화면은 501 이면 픽스처로 그리고 **구현되는 순간 화면 코드 0줄 변경**으로 실데이터 전환된다. 게이트 전 종 green · selftest 120 red-OK · staging `0002_p1_file_count` 적용 green |
...
최근 확정 — ① 신규 구축 ~~② AWS 배포까지~~(**㊻ 로 개정 — staging 까지**) ③ 정본 260818 ④ AI 도메인 분리 ⑤ WU 단위 ⑥ Assistant BC 미계승 ⑦ `colab-v2` 모노레포 ⑯ 레포 public *(전부 2026-08-22 Ted)* · **⑰ G5 대조 상대 = v1 스키마 ⑱ v1 저장 형태 계승 0 ⑲ 값 집합은 DB가 강제 ⑳ 파생값 2종 미저장** *(2026-08-22 G5)*. **㉑ D4→D3 읽기 Port 시그니처 확정 ㉒ "도메인 소유자" = 변경 승인권자 ㉓ staging=터널 호스트·prod=AWS ㉔ PoC 철거·데이터 폐기** *(2026-08-22 Ted)*. **㊵ 승격 게이트(staging 확인 → Ted 승인 → AWS) ㊶ 검색 = Postgres 단독 ㊷ 모델 = GPT 전면 통일(luna/terra, API 키 · Claude 어댑터 휴면) ㊸ 온톨로지 §④ 4항목 확정 — G8 승인, K1 진입조건 충족** *(2026-08-23 Ted)*. **㊹ 미구현 엔드포인트 = 501 + code 두 종(404 아님, 계약 무수정) ㊺ staging postgres 2 DB · 앱 롤 비소유자·NOBYPASSRLS** *(2026-08-23 Claude 판단 · 비토 가능)*. **㊻ AWS 보류 — v2 완주 판정을 `staging 배포 green` 까지로 내리고 prod 는 출시 결정 뒤에 연다** *(2026-08-23 Ted)*. **㊼ `fileCount` 는 메타다 — 조각 수를 `d3_dataset` 메타 층으로 올린다**(잠긴 행에도 `조각 N`) *(2026-08-23 Ted)*. **㊽ 마이그레이션 백필은 `FORCE RLS` 를 한 트랜잭션 동안만 내린다 — 런타임 무변경, 게이트가 복구를 강제** *(2026-08-23 Claude 판단 · 비토 가능)*. 열린 결정 **⑧⑨⑩ 해소**(⑪~⑮는 그대로 열려 있다). **㉖(RLS 실효 증명) 은 D3b 로 닫혔다.** 새로 열린 것 — 없음
