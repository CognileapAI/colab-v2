...
**최종 갱신** 2026-08-22 (**R1 닫힘. 이후 개발은 WSL에서 현재 레포 그대로 계속 · I0/AWS 착수 보류(Ted)**)
**현재 단계** R1 완료 · 개발환경 WSL로 전환(현재 레포 그대로) · T-C/T-G 병렬 착수 가능
**다음 WU (택1, 전부 병렬 가능)** → Claude 착수 가능: **G7** 기획 SSOT 통합 · **C3/C4** PoC·v1 지식 추출 ／ 사람 결정 필요(Ted): **I0** AWS 계정·결제(**이번엔 보류** — 완료 조건상 AWS 배포는 정본 유지) · **G2** 검색 엔진 결정 · **G8** 온톨로지 범위 합의
