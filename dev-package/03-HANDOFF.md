...
| 9 | **OpenAI API 키** (`㊷`) — `ModelPort` 뒤 런타임 자격증명. 헬퍼 `gpt-5.6-luna` · 오케스트레이션 `gpt-5.6-terra` | **K3**(계보 제안) · **K4**(자연어 검색). K1·K2 는 키 없이 닫혔으므로 KG 토대는 이미 서 있다 |
