...
**최종 갱신** 2026-08-22 (**G: 마운트 영구화 — 반복 블로커 #6 해소**)
**현재 단계** R1·G5 완료 · 정본 접근 **영구 확보** · **P0은 아직 G1·D3에 막혀 있다**
**다음 WU** → Claude 착수 가능: **G1** E-00 패키지 재빌드(P0을 막는 나머지 하나) · **G7** 기획 SSOT 통합 · **G3/G4** ／ 사람 결정 대기(Ted): **D1** 도메인 소유자 배정(→D2 계약 동결이 `㉑` Port를 확정해야 한다) · **I0** AWS(**보류**) · **G2** 검색 엔진 · **G8** 온톨로지 범위
...
최근 확정 — ① 신규 구축 ② AWS 배포까지 ③ 정본 260818 ④ AI 도메인 분리 ⑤ WU 단위 ⑥ Assistant BC 미계승 ⑦ `colab-v2` 모노레포 ⑯ 레포 public *(전부 2026-08-22 Ted)* · **⑰ G5 대조 상대 = v1 스키마 ⑱ v1 저장 형태 계승 0 ⑲ 값 집합은 DB가 강제 ⑳ 파생값 2종 미저장** *(2026-08-22 G5)*. 새로 열린 것 — **㉑ D4→D3 읽기 Port (D2에서 확정)**
