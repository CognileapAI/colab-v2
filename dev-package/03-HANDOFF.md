...
／ ⛔ **Ted 결정 대기 — P2·S2 가 함께 막혀 있다**: **계약의 두 seam 이 모순**이다(`§5.5 DR-7`). HTTP seam 에 업로드 입구가 없는데 이벤트 seam 은 `upload.accepted` 를 core-api 의 필수 발행으로 못 박았다. **어느 seam 이 정본인가**를 먼저 정해야 한다
...
| **DR-7** ⛔ | **계약이 서로 다른 세계를 가정한 채 동결됐고, 그 사이를 보는 게이트가 없다.** ⚠ **「두 seam」이라는 틀 자체가 아직 확정이 아니다** — `core-viz.yaml` 의 `RenderTarget.uploadId` 가 viz 를 D5 업로드 세계에 묶어 두고 있어, viz 도 같은 전제라면 **「두 seam 이 갈렸다」가 아니라 「HTTP seam 만 다른 세계를 그렸다」**가 맞는 서술이 된다. **seam 전수 감사가 도는 중이고**(`sessions/SEAM-AUDIT.md`) 결과에 따라 이 항목의 진단 문장이 바뀐다. **아래 증거 3건은 실측이고, 그것을 묶는 틀이 잠정이다.** `contracts/seams/fe-core.yaml` 은 **34 op 중 업로드 진입점도 `createDataset` 도 없이** 데이터셋이 **이미 존재하는 세계**만 그린다. 그런데 `contracts/events/core-pipeline.json` 은 `upload.accepted` 를 **「core-api 가 내는 유일한 이벤트이며 파이프라인의 입구」**로 선언하고 `source` 를 **`core-api` 상수**로 못 박았다 — **core-api 가 반드시 내야 하는 이벤트인데 그것을 촉발할 HTTP 입구가 없다.** 게이트 19 종을 확인했다: `contract-lint`·`contract-breaking` 은 **`contracts/seams` 만**, `event-lint`·`event-breaking` 은 **`contracts/events` 만** 본다. **둘 사이를 보는 게이트가 하나도 없다.** ⚠ **증거가 셋이다** — ① 업로드 입구 부재 ② `source: core-api` 상수 ③ **`link`/`unlink` 비대칭**: `/projects/{projectId}/datasets/{datasetId}` 에 **`unlinkProjectDataset` 은 있는데 `linkProjectDataset` 이 없다**(`grep` 0건). **끊는 길은 있는데 잇는 길이 없다.** 게다가 같은 계약이 `createProject` 는 준다 — **프로젝트는 만들 수 있는데 데이터셋은 만들 수도 이을 수도 없다.** 두 엔티티가 같은 계약 안에서 다른 대우를 받는다 | **D2 · D2b** (seam 간 정합 게이트) · **정본**(어느 seam 이 정본인가) | **⛔ P2 · S2 를 막고, P5 도 잠재적으로 막는다** — `㊾-③` 이 적재를 업로드 화면 경로로만 가도록 못 박았으므로 **S2 는 P2 를 기다리는 게 아니라 이 결정을 기다린다.** **P5(프로젝트)는 아직 안 열려서 안 터졌을 뿐이다** — ③ 의 `link` 가 없으면 프로젝트에 데이터셋을 **붙일 수 없다**. ⚠ **다른 여섯과 성격이 다르다** — `DR-2`~`DR-6` 은 **검사가 틀린 신호를 봤다.** 이건 **아무도 안 본 자리**다. 게이트를 통과해서가 아니라 **게이트가 없어서** 살아남았다. 그래서 「op 을 하나 더한다」로 끝나지 않을 수 있다 — **어느 seam 을 정본으로 볼지가 먼저다.** 그리고 **고칠 범위가 업로드보다 넓다** — ③ 은 업로드와 무관한 동작이고, `link` 는 `DATAMODEL` 상 **N:N 이며 연결마다 활용 의미 문장이 붙으므로** 본문 있는 op 이다. **「op 하나 더한다」로 끝나지 않는다.** *(colab-v2-dev 세션 P2 착수 중 발견, 이 세션이 계약·게이트로 확인)* |
