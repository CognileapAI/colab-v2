...
**최종 갱신** 2026-08-22 (**현행 PoC 배포 실물 파악 · 백업 사건 복구 · G1 재정의 · 결정 ㉑㉒㉓**)
**현재 단계** R1·G5 완료 · 정본 접근 영구 확보 · **G1은 "재빌드"가 아니라 "신선도 게이트"로 밝혀짐** · staging 경로 확보(터널 호스트)
**다음 WU** → Claude 착수 가능: **G1** 신선도 검사기+게이트(지시서 `sessions/G1.md`) · **G7** 기획 SSOT 통합 · **G3/G4** ／ 사람 결정 대기(Ted): **D1** 10개 도메인에 승인권자 배정(정의는 `㉒` 로 확정됨) · **I0** AWS(**prod에만 필요** — `㉓`) · **G2** 검색 엔진 · **G8** 온톨로지 범위
...
> 현행 PoC 배포 실물은 `DEPLOY-CURRENT.md`. **AWS에는 아무것도 없다**(`~/.aws` 미설정).

| WU | 상태 | 비고 |
|---|---|---|
| I0 계정·결제·리전·예산 알람 | ⬜ | **prod 임계경로.** 단 staging은 I0 없이 진행 가능해져 I2 분수령이 앞당겨진다 |
...
최근 확정 — ① 신규 구축 ② AWS 배포까지 ③ 정본 260818 ④ AI 도메인 분리 ⑤ WU 단위 ⑥ Assistant BC 미계승 ⑦ `colab-v2` 모노레포 ⑯ 레포 public *(전부 2026-08-22 Ted)* · **⑰ G5 대조 상대 = v1 스키마 ⑱ v1 저장 형태 계승 0 ⑲ 값 집합은 DB가 강제 ⑳ 파생값 2종 미저장** *(2026-08-22 G5)*. **㉑ D4→D3 읽기 Port 시그니처 확정 ㉒ "도메인 소유자" = 변경 승인권자 ㉓ staging=터널 호스트·prod=AWS** *(2026-08-22 Ted)*. 새로 열린 것 — 없음
