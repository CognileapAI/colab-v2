...
| **DR-2** | **`tif` 는 입력이자 출력이다** — 감지가 *이미 COG 인 tif* 와 *일반 GeoTIFF* 를 구분하지 못하면 **사람이 올린 파일이 우리 산출물로 계보에 기록된다.** 표시 오류가 아니라 **계보 오염**이라 조용히 틀리고 되돌릴 수 없다 | **D5** (구분 규칙 + **음성 시험**) · **S1** (원천의 tif 27 건 중 이미 COG 인 것을 표시) | 지금 코드로 막을 자리가 P1 이 아니다. `㊿` 근거란·`WORK-UNITS §8.5`·`SEED-DATA.md` 셋에 남겨 뒀다 |
