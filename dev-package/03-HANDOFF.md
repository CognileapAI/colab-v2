...
**최종 갱신** 2026-08-23 (**D2 닫힘 · D3 게이트 6종 · G3·G4·G7 닫힘 · 정본 이전·통합 · 결정 ㉕~㊲**)
**현재 단계** 게이트 트랙(T-G) 전부 닫혔고 **계약이 타입으로 동결됐다.** 게이트 **11종 중 10종 구현**, 자기 증명 **83 케이스**(계약 15 · 경계 30 · DB 38) 전부 green · staging 200 유지
**다음 WU** → **P0 의 잔여 차단은 `D3` 하나이고, `D3` 의 잔여는 D3b(RLS 실효 증명)인데 그건 P0 스키마가 있어야 쓴다.** 즉 **지금 코드를 시작하지 못하게 막는 것은 사실상 사람 결정 하나뿐이다.**
／ **사람 결정 대기(Ted) — 임계경로**: **`HYD`(수문학 도메인 소유자) 실명.** `DOMAINS.md §7` 의 빈칸 하나. 들어오면 **D1 을 닫는다** → D9·G8·K 트랙(블로커 #3)도 함께 풀린다
／ **Ted 가 줄 것 (작업 차단)**: **IS2 apply 용 Cloudflare API 토큰** — Custom token, `Account → Cloudflare Tunnel : Edit`. `~/.colab-v2-staging.env`(0600)에 `CF_API_TOKEN`·`CF_ACCOUNT_ID`·`CF_TUNNEL_ID`
／ **Ted 비토 대기 (기본은 그대로 간다)**: `㉝` 스토리 6개 상한 폐기(정본 내용 변경) · `㉟` 표기 규약 · `㊱` 잠김 200·경계 404·권한 403 · `㊲` 멱등 키 둘. **PLAN 이탈 1건 = D1 이 열린 채 D2·D3 진행**(`§5`)
／ Claude 착수 가능: **P0**(경계·계약이 다 섰다) · **D2b** 이벤트 계약 게이트 · **IS3** · `generated-up-to-date` 게이트
／ 그 외 사람 결정: **G2** 검색 엔진 · **G8** 온톨로지 범위 · **I0** AWS(prod 전용) · 터널 ID 히스토리 노출 처리(§4-8)
...
| IS2 터널 라우팅 IaC화 | 🟧 | **선언 배치 완료 · apply 미실행**(Ted 가 Cloudflare API 토큰을 줘야 한다). 산출 `infra/staging/tunnel/`(versions·variables·tunnel.tf·tfvars.example·.gitignore·README). ingress 선언 = `www→nginx:80` + catch-all 404 **둘뿐이고 ssh 규칙 없음**(`§9-㉜`) — 없는 이유를 파일 주석에 남겼다. 비밀은 전부 변수(default 없음)이고 `.gitignore` 가 `*.tfvars`·상태파일 차단(`git check-ignore` 확인). **terraform 미설치라 `validate` 미실행 — 선언은 파서 검증 전이다.** 터널은 **원격 관리형**(토큰만 · ingress 를 Cloudflare 가 push, 커넥터 로그 `version=6` 으로 확증). 3안 비교 후 **Terraform `cloudflare_zero_trust_tunnel_cloudflared_config`** 채택 — 무중단이고 `terraform plan` 이 "레포에서 재적용해 동일 상태 재현"을 그대로 만족시킨다. `.tf` 초안·apply/verify/rollback 시퀀스 = `sessions/IS2.md`(아직 `infra/` 에 배치하지 않았다). 블로커 #7 은 `§9-㉜` 로 **삭제 확정** — IS2 선언에서 빼면 apply 시 사라진다 |
...
| D2 계약 동결 | ✅ | **완료 판정(spectral + oasdiff green) 충족.** 산출 = `contracts/schemas/common.json`(정의 21종 · 값 집합 enum 9종을 DataModel v1.8 에서 전수 대조) · seam 3종 `fe-core`(op 34) `core-ai`(op 2) `core-viz`(op 5) · async 봉투 `contracts/events/`(이벤트 7종) · 게이트 `contract-lint`(spectral 6.16.3 핀) `contract-breaking`(oasdiff 도커 다이제스트 고정) · `contract-selftest` **15 케이스 green**. **진입조건 이탈 1건** — `D1` 이 `HYD` 실명으로 열려 있으나 계약이 그 값을 소비하지 않아 "경계·배정 구조는 확정, 이름 한 칸만 미기재" 전제로 진행했다(`§5 PLAN 이탈 기록`). 결정 ㉟㊱㊲ |
