...
**최종 갱신** 2026-08-22 (**WSL 이관 점검 — 레포·gh는 정상 승계, 기획 정본(G:) 미마운트가 새 블로커**)
**현재 단계** R1 완료 · 개발환경 WSL 전환 완료 · **T-G 전 항목이 정본 접근에 막힘(블로커 #6)**
**다음 WU** → 차단 해제가 먼저다: **블로커 #6 G: 마운트**(Ted, 한 줄). 풀리면 **G5** DataModel v1.8 ↔ 스키마 대조 착수 — P0을 막는 둘 중 하나이자 D2 계약 동결의 직접 입력이다.
／ 사람 결정 대기(Ted): **I0** AWS 계정·결제(**보류** — 완료 조건상 AWS 배포는 정본 유지) · **G2** 검색 엔진 결정 · **G8** 온톨로지 범위 합의 · **D1** 도메인 소유자 배정
...
- **기획 정본**: `CoLab_ver2_1차마일스톤_목업패키지_260818_이태헌` (G:). 인덱스 5종은 260808판에만 있음 → G7. **WSL에서 G: 미마운트 — 현재 읽을 수 없다(블로커 #6).** 260808판만 F: `01 CoLAB-Plan/reference/` 에서 읽힌다.
...
| 6 | **기획 정본(G:) 미마운트** — WSL에 `G:`(Google Drive)가 안 붙어 260818 정본을 읽을 수 없다. F:에는 260808판만 있다. 해제: `sudo mkdir -p /mnt/g && sudo mount -t drvfs G: /mnt/g` (영구화는 `/etc/wsl.conf` 또는 `/etc/fstab`) | **G1 · G1b · G3 · G4 · G5 · G7** — T-G 전체. G5가 막히면 D2·P0까지 연쇄 |
