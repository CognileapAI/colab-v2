...
**최종 갱신** 2026-08-22 (**PoC 스택 철거 · 배포 실물 파악 · 백업 사건 복구 · G1 재정의 · 결정 ㉑㉒㉓㉔**)
**현재 단계** R1·G5 완료 · 정본 접근 영구 확보 · **G1은 "재빌드"가 아니라 "신선도 게이트"로 밝혀짐** · **PoC 철거 완료 — 터널이 빈 채로 v2 를 기다린다**
**다음 WU** → Claude 착수 가능: **G1** 신선도 검사기+게이트(지시서 `sessions/G1.md`) · **IS1** staging 호스트 구성(빈 터널에 v2 오리진) · **G7** 기획 SSOT 통합 · **G3/G4** ／ 사람 결정 대기(Ted): **D1** 10개 도메인에 승인권자 배정(정의는 `㉒` 로 확정됨) · **I0** AWS(**prod에만 필요** — `㉓`) · **G2** 검색 엔진 · **G8** 온톨로지 범위
...
| **IS1 staging 호스트 구성** | ⬜ | **지금 착수 가능 — 진입조건 없음.** 빈 터널에 v2 오리진을 붙인다 |
| IS2 터널 라우팅 IaC화 | ⬜ | IS1 후. 현재 ingress 정본이 Cloudflare 대시보드에만 있다 |
| IS3 staging 백업 체계 | ⬜ | IS1 후. **복원 리허설 필수** — 빈 백업 8주 전례 |
...
| WU | 상태 | 비고 |
|---|---|---|
| G1 E-00 패키지 재빌드 | 🟦 | **재빌드는 필요 없었다** — 패키지 임베드 md가 `DataModel v1.8`·`Policy v1.4`와 sha256 동일(E-00 직접 검증, 8개 에픽 전부 동일). 낡은 것은 *내용*이 아니라 "미반영"이라 적어둔 **서술**이다. 실질 작업 = 신선도 검사기 + `planning-freshness` 게이트 + `planning/README §3` 정정. 파일명(`260817`)과 mtime(Drive 동기 시각으로 전부 동일)이 둘 다 거짓말이라 **오라클은 content-hash여야 한다.** 지시서 `sessions/G1.md` |
