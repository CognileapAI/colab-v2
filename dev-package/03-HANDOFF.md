...
**다음 WU** → **D2c**(계약 개정 — `〈54〉` 이행 · `§5.5 DR-7`, 지시서 `sessions/D2c.md`+추기 · **선행 `DR-13` ✅ 해소됨(2026-08-23) — 착수 가능**). ⛔ **P2 는 열려 있지 않다** — `sessions/P2.md` 의 두 ⛔ 가 각각 독립적으로 막는다: **`STOP-1`**(동결 계약에 업로드 경로 없음 → **D2c 가 닫는다**) · ~~`STOP-4`~~(`DR-13` ✅ 2026-08-23 해소 — D5 WU 승격). **남은 것은 `STOP-1` 하나** — 닫혀야 P2 가 열리고, **P2 가 닫혀야 S2(초기 데이터 적재)가 열린다**(적재는 DB 직접 INSERT 가 아니라 업로드 모달 경로로 간다 — `㊾-③`). 각 WU 의 완료 판정에 **`staging 배포 green` 이 붙는다**(I2 이후). 병렬 착수 가능 = **I3**(배포 자동화 · 작업지시서 초안 `sessions/I3.md` 있음) · `generated-up-to-date` 게이트 · **IS4 잔여** · **D5**(포맷 처리 — `DR-1`·`DR-2`·`DR-3` 이 전부 여기 소유다)
...
| 결정 | 이행 | 실물 |
|---|:--:|---|
| `〈61〉` 계약 동결 전 옳음 판정 | ⬜ **미이행 · D2c 완료 정의** | ㉠ 정본 근거 대조(기계) · ㉡ **흐름 완주 검사**(기계) · ㉢ **Ted 승인**(사람). **㉠·㉡ green 이어도 ㉢ 없이 동결하지 않는다.** ⚠ **㉡ 은 사람이 만든 「단계↔op 대응표」에 기대므로 그 표를 세우는 일이 게이트 구현보다 먼저다** — 표가 틀리면 ㉡ 은 **틀린 흐름을 완주로 판정한다** |
...
| `〈58〉`·`〈59〉` 격자 **추가·교체·삭제** | ⛔ **막힘** | 계약에 **파일 조작 경로가 없다**(`DR-7`). `D2c` 선행. ⚠ **추가만 열면 결정의 절반 이행이다** — `〈59〉` 는 「격자는 **조정하는** 대상」이라 **교체·삭제까지가 정상 동작**이다(잘못 붙인 격자를 바로잡는 것 포함). 판정은 `업로드·편집` 스위치(`⑳`) |
