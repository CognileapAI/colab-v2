...
| **DR-7** ⛔ | **두 seam 이 서로 다른 세계를 가정한 채 동결됐고, 그 사이를 보는 게이트가 없다.** `contracts/seams/fe-core.yaml` 은 **34 op 중 업로드 진입점도 `createDataset` 도 없이** 데이터셋이 **이미 존재하는 세계**만 그린다. 그런데 `contracts/events/core-pipeline.json` 은 `upload.accepted` 를 **「core-api 가 내는 유일한 이벤트이며 파이프라인의 입구」**로 선언하고 `source` 를 **`core-api` 상수**로 못 박았다 — **core-api 가 반드시 내야 하는 이벤트인데 그것을 촉발할 HTTP 입구가 없다.** 게이트 19 종을 확인했다: `contract-lint`·`contract-breaking` 은 **`contracts/seams` 만**, `event-lint`·`event-breaking` 은 **`contracts/events` 만** 본다. **둘 사이를 보는 게이트가 하나도 없다** | **D2 · D2b** (seam 간 정합 게이트) · **정본**(어느 seam 이 정본인가) | **⛔ P2 와 S2 를 함께 막는다** — `㊾-③` 이 적재를 업로드 화면 경로로만 가도록 못 박았으므로 **S2 는 P2 를 기다리는 게 아니라 이 결정을 기다린다.** ⚠ **다른 여섯과 성격이 다르다** — `DR-2`~`DR-6` 은 **검사가 틀린 신호를 봤다.** 이건 **아무도 안 본 자리**다. 게이트를 통과해서가 아니라 **게이트가 없어서** 살아남았다. 그래서 「op 을 하나 더한다」로 끝나지 않을 수 있다 — **어느 seam 을 정본으로 볼지가 먼저다.** *(colab-v2-dev 세션 P2 착수 중 발견, 이 세션이 계약·게이트로 확인)* |
