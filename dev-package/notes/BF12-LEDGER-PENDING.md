# `BF-12` 원장 등재 초안 — 앱 로그 설정 (~~`§9 〈N〉` 자리 미정~~ ⭑ **등재 완료 → `§9 〈325〉`**)

> ⭑ **⟨등재 완료 2026-09-05⟩ 이 초안은 `PLAN-SoT §9 〈325〉` 로 들어갔다.** 번호는 병합 직전
> `origin/main` 의 `§9` 최대(`〈324〉`) ＋1 로 재실측해 박았다. **아래 본문은 그때의 초안 그대로 남긴다**
> — 지우지 않는다. ⚠ **게이트 계수는 초안과 등재문이 다르다** — 초안의 「green 34 · red(판정) 1 ·
> red(준비) 15」는 기준 `main` `59b1c13` 의 워크트리 실측이고, **등재문 `〈325〉`-㉴ 의 「두 실행에
> 걸쳐 50/50」이 병합 트리 `ed8ceb3` 의 값이다.** 등재문이 이긴다.
