...
**상태:** **완료 2026-09-12.** 작업 1~6 ＋ 6-b 집행 · 체크 **66/78**(미이행 12 — 전수 게이트 2 · 작업 1 게이트 종료코드 기록 1 · 마이그레이션 적용 전 `deploy_doctor` 재관찰 1 · 종합 보고 1 · 특정 연구실 좁히기 1(범위 판정 필요) · 계획 자체 점검 6). dev 실적용 `71ee15757737` · 태그 `dev-20260912-2`(로컬·미push) · `deploy_doctor` **15/15 한 번의 실행**(14:25:03Z · exit 0) · 대장 `BO-1`·`BO-2`·`BO-3` 전건 `done` · 원장 `PLAN-SoT §9 〈383〉`·`〈384〉`(임시 번호). **전수 게이트 3계수는 `[미확인]`** — 지목 보고서 부재(작업 5 해당 칸). 남은 것 = **Ted 실사용 확인**(백오피스 목록·재설정·비활성화·관리자 토글·전 연구실 열람) ＋ 후속 7건.
...
- [ ] 전수 전에 홈의 `.colab-v2-test.env` 를 export 로 선언한다. 워크트리는 `node_modules`·`.venv` 와 core-api 의 편집 가능 설치를 먼저 구성한다. — **마무리 레인 미실행.** 이 레인의 필수 게이트 3종은 좁은 게이트라 전수 준비가 필요 없었다.
- [ ] 전수 게이트를 `-j 4` 로 백그라운드 1회 실행하고 green / red(판정) / red(준비) 3계수를 나눠 기록한다. — **미이행.** 마무리 레인은 전수를 돌리지 않았고, 지시문이 근거로 지목한 보고서 `dev-package/reports/r-login-backoffice/task7-full/run2/gate-summary.md` 는 `origin/main` `d6aca34e` 트리에 **부재**다(`git ls-tree -r origin/main` 0건 · 작업 사본 탐색 0건). 잰 것은 게이트 구성원 수 **64종**(`gates/run.sh` `ALL_GATES`)뿐이고 3계수는 `[미확인]` 이다 — 다른 실행의 계수를 옮겨 적지 않았다.
- [x] dev 에 platform 체인 `0027_operator_audit` 위의 `0028_account_status`·`0029_operator_read_policy`·`0030_merge_audit_and_backoffice`(머지) 를 적용하고 core/frontend 이미지를 교체한 뒤 `deploy_doctor --env dev` 를 **한 번** 실행해 15/15 를 받는다. 재시도로 모은 계수를 15 라 적지 않는다. — 2차 배포(`71ee15757737` · 2026-09-12 14:14~14:25Z). **적용 전 head 는 이미 `0027_operator_audit` 이었고 적용 후는 `0030_merge_audit_and_backoffice` 다**(경로 = `0028_account_status`·`0029_operator_read_policy`·`0030` 머지) — 계획 당시 이 줄의 표기는 「`0027` 을 적용하고」였고 실물에 맞춰 고쳤다. 4 단위 전부 `:dev-71ee15757737` healthy · `CURRENT_SHA`/`MAIN_SHA` = `71ee15757737`(`ancestor=yes`) · 프런트 96파일 재업로드 후 CloudFront `index.html` md5 `f59709d8…` 로컬 일치. `deploy_doctor` **`항목 15 — ✓ 15 · ✗ 0 · ─ 0` · exit 0 · 재시도 0회**(14:25:03Z). 마이그레이션 전 백업 `_ops/backups/dev/2026-09-12T141450Z-*`. 근거 `dev-package/reports/r-login-backoffice/task5/deploy-2.md`.
- [x] dev 반영 시 `services/core-api/ops/account-admin-role.sql` 재적용(신규 GRANT: `d2_member_role SELECT` · `service_operator INSERT/DELETE`) — 미적용 시 목록·지정 500. — 적용 완료: `has_table_privilege('colab_account_admin', …)` 전→후 = `service_operator` INSERT `f`→**`t`** · DELETE `f`→**`t`** · `d2_member_role` SELECT `f`→**`t`**. ⚠ **문서 경로(`db-bootstrap.sh account-admin`)는 dev 에서 그대로는 성립하지 않는다** — 파일 안의 `ALTER ROLE … NOSUPERUSER … PASSWORD` 가 `permission denied to alter role`(RDS 마스터는 superuser 가 아니다 · 실측 `rolsuper=f`)로 죽고 `ON_ERROR_STOP` 이라 뒤의 GRANT 가 한 줄도 안 돈다. 그 한 문장만 빼고 적용했으며(속성 판정은 파일 꼬리 자기 점검이 그대로 수행) **이 결함은 어느 게이트·`deploy_doctor` 항목에도 걸리지 않는다** — 후속 항목.
...
- [x] 대장 `BO-1` 을 `done` 으로 갱신한다(intent Q7). `completion_def`·`evidence` 를 채운다. — `6e99a157` 회차에 완료(`work-items.yaml` `BO-1` `status: done` · `evidence` = 계정 추가 화면·`POST /admin/accounts`·계약 경로 ＋ 판정 근거 Ted Q7).
- [x] 확장분을 새 항목으로 등재한다 — 계정 목록·운영자 비밀번호 재설정·비활성화/재활성화. `stage: after_stage2` 면 `CLAUDE.md` 의 `after_stage2` 괄호 목록도 같은 커밋에서 갱신한다. — `BO-2`(목록·재설정·비활성화) ＋ `BO-3`(관리자 지정·해제와 전 연구실 읽기) 등재 후 **둘 다 `done`**(이 마무리 회차 · `evidence` 기재). 괄호 목록은 19항목으로 이미 동기화돼 있고 이 회차는 **바꾸지 않았다** — 게이트 ㈕ 는 `status` 를 보지 않고 `stage: after_stage2` 집합만 대조한다(`gates/tools/work_item_consistency.py:600`).
...
- [ ] 실제 종료코드·3계수·미실행·잔여 제한을 보고한다. 전체 체크 완료 시에만 구현 완료로 표시한다. — **부분.** 마무리 레인의 필수 게이트 3종(`work-item-consistency`·`planning-freshness`·`service-tests-core-api`) 계수는 보고했으나 **전수 3계수가 없다**(위 항목). 그러므로 이 칸은 열어 둔다.
