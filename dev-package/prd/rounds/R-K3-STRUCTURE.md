...
## 판정 기록 (2026-09-24 Ted 「전부 권고대로 할게」 · 게이트 ① advisor 조건 반영)
- S0 += 후보 원메타 4필드(`crs`·`grid`·`variables`·`fileName`) · rationale/confidence 「잠정값」 산문 · 문자열→정수 Lv 파서 위치 명시 · `seam-consistency` 를 먼저 돌려 인라인 integer 선례 유무 확인.
- S1 → **S1a 비교기(순수 함수·시험)** / **S1b 적격 필터+라우트**. 필터 = `max(파생, 사람값)`. Lv3 상한 구멍 주석·시험.
- S2: 「정답 없으면 맞는 축 없다」 문장 삭제 · 「참인 인용의 비부모는 살아남는다」 시험 추가 · 파생 확신도 계산은 여기 한 곳.
- S3: 잠정 confidence 계산 제거 → 상수 자리채움. rationale 도 자리채움(core-api 가 다시 쓴다).
- S5: 규칙 팔 상위 k 절단 = 모델 팔과 동일.
- S6: ⑴ red 규칙을 「구조 누수(후손·자기·후보 밖·인용 오류) → WU 반려」 vs 「참인 인용의 비부모 → 기록」으로 가름 · ⑴′ 「정답+형제 제거」군 추가(구조 보장 판정용) · ⑶ 형제만 군은 기록.
- 열린 권고 ①(신호 미탑재·원메타 탑재) ②(코퍼스 별건) ③(`max` 필터) ④(브라우저 주장 없음) 전부 채택.
- 인용 줄 어긋남 2건(`d3_catalog.py:490-503` → `:525-533` · `config.py:91-97` → 선례 재확인) 은 S1a 착수 시 수정.
