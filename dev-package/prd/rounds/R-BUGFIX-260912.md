...
- 착수 순서 4단(직렬 · 같은 파일군이라 병렬 금지) —
  ...
  4. `#24`㉯ 분석 중 고지 — `다음 →`(대상은 `reg-open` · `reg-next` 아님) 은 **비활성 유지**하고 사유 한 줄과 `title`·`aria-describedby` 연결을 추가한다. 정책 6(분석 완료 전 비활성)과 기존 회귀 기준을 바꾸지 않는다. red seam = `frontend/test/upload.test.tsx` 5건(구현 전 실행에서 실패 5건 로그 보존 · 수집 0건이 아님). **Ted 판정 ⑧ 확정(2026-09-12) = 소요 수용 ＋ 「동작 중」 표시 보강** — 분석 중 칩(`ANALYZING_CHIP` · `analyzeStage`)에 ㉠ 활동 표시(CSS 애니메이션 · `prefers-reduced-motion` 존중) ㉡ 클라이언트 경과 시간(`N초 경과` · 상태 응답에 진행 수치가 없어 조각 수 진행률은 표시하지 않는다 · 계약 변경 0) ㉢ 사유 줄을 같은 묶음으로 구현한다. `#24`㉮·`#25`⑴ 은 이 묶음으로 닫는다(성능 작업 없음).
