...
| ADR-0005 후속 ② 호스트 뮤텍스 | 로컬 구현·단독 게이트 완료 | spec `dev-package/prd/specs/2026-09-18-gate-host-mutex.md` · 라운드 `dev-package/prd/rounds/R-GATE-HOST-MUTEX.md`. `serial` 선언을 호스트 전역 `flock` 하나로 집행 · 면제 키 `COLAB_GATE_MUTEX_HELD` 분리 · 세 갈래 78. `gate-host-mutex-selftest` 6케이스 green(ⓐ 실경과 2초 · ⓑ waited 3). **전수 1회(완료 조건 4 「잠금 17건」)와 PR 게시는 미실행** |
