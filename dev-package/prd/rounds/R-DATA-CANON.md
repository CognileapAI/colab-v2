...
결정 번호 = 〈398〉 — 2026-09-14 병합 직전 재실측(`dev-package/prd/tools/max-decision.sh` · `origin/main` 최대 〈397〉 ＋ 1 · 착수 시점 기대값 397 은 R-LTH-1 회차가 선점). 등재 행은 `PLAN-SoT §9` 〈398〉 · 실측은 §11.
...
> **〈398〉 R-DATA-CANON 회차 — 자료 설명 정본화 ＋ dev 반복 재생성 · 판정 8건 ＋ dev 초기화 상시 승인 (2026-09-14)**
...
## 11. 병합 시점 실측 (2026-09-14)

- 결정 번호 = 〈398〉. `origin/main` 최대 〈397〉(R-LTH-1 회차 · `a257a2d3`) ＋ 1. 착수 시점 기대값 397 은 선점되어 무효.
- 통합 tip `cd274a40` 전수 게이트 1회 = 「green 66 / red(판정) 1 / red(준비) 0」. 당시 `origin/main` = `3fc6ea01`.
- red 1 = `migration-drift` `0015-drift` 「postgres 60초 기동 한도」. 마이그레이션 파일 무변경(`git diff 3fc6ea01..cd274a40 -- db` 0건). 직렬 단독 재현 green 26/26. 판정 = 병렬 부하성 타임아웃(#46 계열).
...
### 13-1. 결과 계수 (5회차 verify · `dev-package/sessions/DR-4-run-20260914T050442Z.md §3`)

| 계수 | 정본 | 실측 | 판정 |
|---|---|---|---|
| dev 배포 sha · 태그 | — | `d5cd6ca9` · `dev-20260914-3`(push) | green |
| `deploy_doctor` | 15/15 한 번의 실행 | 15/15 · 한 번의 실행 | 성립 |
| 데이터셋 | 28 | **28** | 성립 |
| 프로젝트 | 4(`drought` 2) | **4**(`drought` 2) | 성립 |
| 계보 간선 | 18 | **18** | 성립 |
| 「미지정」 가공 단계 | 0 | **0** | 성립 |
| 프로젝트 미연결 | 0 | **0** | 성립 |
| 가공 단계 분포 | Lv0 10 · Lv1 16 · Lv2 2 | Lv0 10 · Lv1 16 · Lv2 2 | 성립 |
| 계정 | 2(PI ＋ Ted 운영자 · 계정 관리 화면 생성) | 2(4회차 측정값 · 5회차 재측정 없음) | `[미확인 — 5회차 재측정 없음]` |
| S3 | 옛 객체 0 → 새 28건 파일 | 4회차 옛 객체 1526 삭제 · 5회차 등록분 2건 추가 | 성립 |
| 미리보기 판정 표 | 28건 · 이름 | 「성립 28」 — **측정 결함** | 값으로 쓰지 않는다 |
| 「한 번의 실행」 | 성립 | **불성립** — 실행 5회 · 배포는 release executor(도구 밖) · `--from s3`·`prelude`·`seed`·`verify` 재개 · 계정은 러너 별도 명령 | 미달 |

- 미리보기 표 결함 = 순회기가 렌더 완료가 아니라 `dataset-preview` 등장 직후(`preview-unavailable` 계수 0)를 「성립」으로 적는다(같은 기록 §8-1). 렌더 실성립은 이 회차에서 측정되지 않았고, 같은 dev 의 DR-3 실측은 2/5 였다. 후속 = `PV-2` note(측정 방식) ＋ `DR-4-TOOLING`.

### 13-2. intent 대조 (`dev-package/intent/2026-09-14-data-canon-and-reseed.md` 원한 결과 ⓐ~ⓕ)

| 항목 | 원한 결과 요지 | 판정 | 근거 |
|---|---|---|---|
| ⓐ | 유형별 설명 정본이 참조자료 폴더의 `DATASETS.md` 4건에 있고 Ted 확정 표시가 문서 안에 있다 | 충족 | `dev-package/reports/reference-data/datasets-md/{01.level-data/01.precipitation,01.level-data/02.vegetation,01.level-data/03.drought,02.File-format}/DATASETS.md` · 커밋 `c49a1e40`·`2d5d6422` |
| ⓑ | 실행 계획이 정본 md 의 fenced yaml 블록에서 생성되고 어긋나면 비영 종료 | 충족 | `dev-package/tools/dev-seed/build_plan.py` · 게이트 `seed-plan-drift`·`seed-plan-drift-selftest` |
| ⓒ | 명령 하나가 10단계를 사람 입력 0회로 끝까지 돈다 | **미달** | 단일 프로세스 완주 5회 중 0회(§13-1 「한 번의 실행」 행) · 도구 결함 고침으로 재개 지점은 좁아졌으나 재개 횟수 자체가 판정 대상 |
| ⓓ | 데이터셋 28 · 프로젝트 4 · 간선 18 · 가공 단계 정본 일치(「미지정」 0) | 충족(5회차 확정) | §13-1 |
| ⓔ | 가뭄 자료가 「미리보기 없음」으로 등록되고 안내 문면이 「등록은 됩니다」로 바뀐다 | 충족(5회차 확정) | seq 13·14 `.gpkg` `registered_no_preview`(WU-C2a·C2c · 커밋 `57314fcb`·`d5cd6ca9` · 기록 §5) |
| ⓕ | 투입 실적과 정본의 차이가 항목명·사유로 결과 JSON·보고에 남는다 | **부분** | 데이터·간선 차이는 0 으로 소멸 · 미리보기 칸은 측정 결함으로 계수 신뢰도 미확보(§13-1) |

### 13-3. `DR-4` 판정

- 상태 = **`partial`(🟧)**. 완료 정의 ⑴(진입점 하나가 초기화와 재투입을 사람 손조작 없이 이어 부른다) 미달 · ⑵ 재개(`--from`·`--from-seq` 4회 실증) · ⑶ 승인 게이트(dev 한정 상시 · 〈398〉) · ⑷ 화면단 원칙 · ⑸ 계수표 1장 — 충족.
- 실행 5회 = 1회차 `preflight` `secrets` 정지 → 2회차 `reset` ①″ 따옴표 결함 정지 → 3회차 `s3` ② 소유자 단언 정지 → 4회차 `--from s3` 재개(26/28) → 5회차 배포 뒤 `.gpkg` 2건 화면 등록(28/28). 정지 3건 전부 「실모드로 돈 적 없는 원격 줄」이고 `--rehearse` 원시동작 10종이 그 반영.
- 닫는 항목 = `DR-4b`. 진입조건 = 블로커 75 해소 ＋ 도구 결함 2건(`project_index` 앵커 의존 · verify 순회기 대기 조건) 픽스처.

### 13-4. 후속 등재 (대장 `dev-package/work-items.yaml` · 전부 등재만)

| id | 이름 | stage | 흡수 이슈 |
|---|---|---|---|
| `DR-4b` | reseed 배포·계정 단계 통합 ＋ 단일 실행 증명 | after_stage2 | — (DR-4 파생) |
| `DR-4-TOOLING` | reseed 도구 잔여 결함 묶음 7건(산출물 워크트리 밖 보존 포함) | after_stage2 | #49 · #50 |
| `OPS-PI-PW` | PI 계정 비밀번호 재설정 — 유일 사본 유실 복구 | after_stage2 | — (블로커 75) |
| `S3-PROC` | S3 계획 검토 절차 정비 | after_stage2 | #49 |
| `DEPLOY-PRIM` | `--rehearse` 원시동작 10종 배포 문서 승격 | after_stage2 | — |

- 기존 항목 note 에 합친 것 3 = `GT-1`(#46 `migration-drift` 오라클 정비 ＋ 신규 게이트 병렬 안전성) · `LF-13`(#48 EC2 잔존물 판정 — `dev.env.bak-*` · 이미지 tar · web-backup) · `PV-2`(미리보기 「성립」 측정 결함 ＋ 4회차 `viz-render` `RestartCount` 5 관측). 초안의 `GATE-46`·`OPS-EC2`·`PV-2-EXT` 는 이 셋으로 갈음 — 별도 항목을 만들지 않는다.
- `CLAUDE.md` stage 3 표지 = 35항목(종전 30) · `work-item-consistency` ㈕ 대조.

### 13-5. 이슈 대응 (닫기는 Ted)

| 이슈 | 대응 | 근거 |
|---|---|---|
| #40 `.gpkg` 등록 차단 | 해소 | WU-C2a(등록 활성 `ready \|\| failure`) ＋ WU-C2c(단계 이동도 같은 판정) · 5회차 seq 13·14 등록 실사 |
| #44 가공 단계 전건 Lv2 | 해소 | 〈397〉 편집 칸(R-LTH-1) ＋ 러너 명시 지정(C1b) ＋ 재생성(C4) · 분포 Lv0 10·Lv1 16·Lv2 2 |
| #50 러너 실패 처리 | 해소 | 러너 레벨·실패·계정(C1b) · `registered_no_preview` |
| #48 EC2 잔존물 | 부분 | preflight 디스크·잔존물 검사 · 처분 판정은 `LF-13` |
| #49 S3 절차 | 부분 | `stage_s3` ② 소유자 단언 제거 · 문서화는 `S3-PROC`·`DR-4-TOOLING` |
| #46 `migration-drift` 오라클 | 그대로 | `GT-1` note |
| #42 미리보기 뒷단 | 그대로 | `PV-2` note |

- GitHub 등재 2026-09-14 — 신설 #52~#59 · 댓글 #40 #44 #50 #48 #49 #46 #42 · 닫기는 Ted. 대장 연결 = `DR-4b` #52 · `DR-4-TOOLING` #53 · `S3-PROC` #59 · `DEPLOY-PRIM` #52/#48 · `GT-1` #55 #56 #57 · `LF-13` #48 부분 · `PV-2` #42 댓글.

### 13-6. 새 블로커

- `03-HANDOFF.md §4` **75** = 4회차 실행 산출물(러너 상태 파일 · 회전된 PI 비밀번호 유일 사본)이 `lane/wu-c4-run4` 워크트리 삭제로 유실. 푸는 것 = Ted 가 운영자 계정(초기 비밀번호 = 실행 기계의 0600 파일)으로 계정 관리 화면에서 PI 비밀번호 재설정(대안 prelude ③ 재실행) · 대장 `OPS-PI-PW`.
- 재발 방지 = `CLAUDE.md §5-b` 2줄(원격 원시동작 리허설 선증명 · 실행 산출물 워크트리 밖) ＋ `DR-4-TOOLING` ⑺.
