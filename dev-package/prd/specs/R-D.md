...
- **WU 7건 · 순서 고정 D4 → D1 → D2 → D3 → D5 → D6 → D7**(D4 가 먼저여야 D2 의 조상 검사가 죽은 브랜치에 흔들리지 않고, D2 가 `MAIN_SHA` 를 실어야 D3 가 읽는다). D5 는 D1 과 파일 겹침 0 이라 병렬 가능(표시만 · 기본 직렬).
  ...
  - **WU-D2 반입 게이트 ＋ 원장 필드 ＋ MAIN_SHA** — `infra/dev/ship.sh:14`(`SHA=` 읽기) 직후·`:20` 첫 ssh 전에: `git -C "$REPO" fetch -q origin main && git -C "$REPO" merge-base --is-ancestor "$SHA" origin/main || { echo "sha 가 origin/main 조상이 아니다: $SHA" >&2; exit 65; }` ＋ `MAIN_SHA="$(git -C "$REPO" rev-parse --short=12 origin/main)"` 를 `:25` 의 `CURRENT_SHA` 기록 줄과 같은 ssh 에서 `/opt/colab-v2/MAIN_SHA` 로 기록. 명시 우회 = `COLAB_SHIP_ALLOW_NONMAIN=1` 이면 **거절하지 않되 출력에 「비조상 반입 · 우회 선언」 한 줄** ＋ `MAIN_SHA` 에 `ancestor=bypass` 기록(세 상태). ⭑ advisor ① 정정 — 종전 `nonmain:<sha>` 표기는 폐기 · `ancestor=yes|no|bypass` 한 형식으로 통일(D3 판정이 갈리지 않도록). `infra/staging/deploy.sh:266` `ledger_append deploy … green "…"` 비고 끝에 `브랜치=$(git -C "$REPO" branch --show-current)` 추가(`:71`·`:87` red 행도 동일). `rollback.sh` 는 무접촉(브랜치 무관). 태그: `infra/dev/` 에 `tag-release.sh` 신설 — doctor 전건 뒤 사람이 호출 · `dev-YYYYMMDD-N`(N = 같은 날 기존 태그 수＋1) · `prod-YYYYMMDD` 는 기존 규약 유지.
  - **WU-D3 doctor 15항목** — `deploy_doctor.py:62` `MARKS` 에 `⑮` 추가 · `check_main_ancestry(ctx, rep)` 신설 · `:700` `check_backups` 뒤 호출. 판정 = `/opt/colab-v2/CURRENT_SHA` 와 `/opt/colab-v2/MAIN_SHA` 를 읽어 ⑴ 둘 다 있고 `ancestor` 가 `bypass` 아님 ∧ `CURRENT_SHA` 가 `--repo` 트리의 `git`… — ⛔ EC2 에 git 없음 → 대조는 **문자열**: `MAIN_SHA` 파일에는 ship.sh 가 「`origin/main` sha ＋ 후보 sha 가 조상임을 로컬에서 확인했다」는 두 값(`main=<sha> candidate=<sha> ancestor=yes|no|bypass`)을 적고 doctor 는 `candidate == CURRENT_SHA ∧ ancestor == yes` 를 ✓ 로 · `bypass` 는 ✗ ＋ 사유 · 파일 부재는 **─(준비) 아닌 ✗**(우려 4 권고). 요약줄 `항목 15 — ✓ N · ✗ N · ─ N`.
  - **WU-D5 eval 러너** — `eval/harness/run.sh`: 과제 디렉터리 전수(`eval/harness/H??-*/`) · 각 과제 = `task.md`(지시) · `fixture/`(심은 조각) · `expect.sh`(exit 0 = green) · 실행 = `timeout "$COLAB_EVAL_TIMEOUT"` ＋ `claude -p --output-format json --allowedTools "Read,Grep,Glob,Bash(<read-only 목록>)" --no-session-persistence --add-dir <fixture> --max-budget-usd "$COLAB_EVAL_BUDGET" < task.md` → 출력을 `expect.sh` 에 stdin 으로 · 과제당 **2회** · 2/2 green 만 통과 · 1/2 = red(판정 · 「불안정」) · timeout/예산 초과 = red(준비 · exit 78) · 과제 0건 = red · `fixture/`·`expect.sh` 부재 = red(준비). 요약줄 `과제 N · 실행 2N · green N · 불안정 N · 준비 N`. `COLAB_EVAL_TIMEOUT`(초안 180)·`COLAB_EVAL_BUDGET`(초안 0.50)는 **미선언이면 red(준비)**(관대한 기본값 금지 · `${VAR:-}` 없음). `eval/README.md` 하네스 표에 `harness/` 한 행. read-only Bash 목록 = `git log`·`git show`·`git diff`·`sed -n`·`grep`·`ls`·`cat`·`wc`·`python3 <계측기>` — 과제 디렉터리 `README` 에 축자. ⭑ ⟨정정 2026-09-08 · advisor ② WU-D5⟩ 종전 ~~`text`~~ — 요약줄 `USD 합` 이 `text` 출력으로는 측정 불가 · 러너는 `result` 본문만 `expect.sh` stdin 으로 넘겨 대조 조건 유지 · `is_error`/`subtype≠success` 는 red(준비).
...
| # | 항목 | ⓐ | ⓑ | 권고 |
|---|---|---|---|---|
| 1 | 모델 호출 상한을 실측 전에 어떻게 두나 — 첫 20건 실측이 상한 자체를 정한다 | 초안 180초·0.50달러로 돌리고 초과는 red(준비)로 기록만, 상한은 p95×2 로 갱신. ⭑ ⟨정정 2026-09-08 · advisor ② WU-D6⟩ 초안 ~~`0.50`~~ 은 스모크 `results/20260908-161309` 에서 1턴 0.713 USD(`error_max_budget_usd`)로 바닥 미달 — 첫 실측은 `3.00` 으로 실행 · 확정값 `2.01` = 1회 최대 1.0067×2 · 초안값은 호출당 컨텍스트 적재(34,840 토큰)를 셈하지 않았다. | 실측 없이 초안값 고정 | **ⓐ** — 판정 축자. 초안값 초과가 나오면 그 과제는 「상한 미확정」으로 표에 남긴다 |
...
| 9 | 배포 점검기 ⑮ 가 `/opt/colab-v2/MAIN_SHA` 를 읽을 수 있나 — 점검기는 컨테이너 안(`docs/DEPLOY.md:262`)이고 마운트는 `/repo` ＋ 시크릿 2 파일뿐 · `--repo` 인자 부재(`deploy_doctor.py:714-731` · `REPO_ROOT` `:51-52`) | ⓐ `docker run` 에 `-v /opt/colab-v2:/state:ro` 추가 ＋ `--state-dir`(기본 `/state`) 인자 ＋ 「마운트 없음」·「파일 없음」을 구분해 둘 다 ✗(사유 상이) | 현 설계 유지 | **ⓐ** — advisor ① 조건 · 셋 미충족 시 **D3 폐기**(영구 ✗ 점검기는 다음 라운드를 무르게 한다) |
| 10 | `--allowedTools` 의 `Bash(python3 …css_audit.py:*)` 는 인자 경로가 자유면 읽기 전용이 깨진다 | ⓐ 계측기 인자를 fixture 안 사본 경로로 고정 | 자유 인자 | **ⓐ** — advisor ① |
| 11 | 원격 브랜치 삭제·태그 push 는 비가역 원격 행위 | ⓐ 레인은 표(＋로컬 태그)까지 · 원격 삭제·push 는 오케스트레이터가 게이트 ③ 뒤 집행 · `git push origin --tags` 금지 → `git push origin archive/<이름>` 개별 | 레인이 직접 | **ⓐ** — advisor ① · 잡태그 유출 방지 |
...
- 예상 레인 수: **5**(⭑ advisor ① 개정 · 종전 7) = D4 → {D1 ‖ D5} → **D2＋D3 한 레인 `rd-ship-gate`**(`MAIN_SHA` 생산자·소비자 — 두 레인이면 형식 불일치가 병합 뒤에야 보인다) → D6 → D7. **D1 ‖ D5 병행 조건** = 대장(`work-items.yaml`)·`dev-package/sessions/` 갱신은 오케스트레이터 단독(파일 겹침이 0 이 아니라 대장 1건이다). 레인 = `lane-worker` `Agent(isolation:"worktree")` · advisor ①(fan-out 전) ②(산출 수용) ③(병합 전).
...
- **판정 = 조건부 승인**(조건 5 · D3 는 조건 미충족 시 폐기).
- ⑴ 순서 승인 · 「D5 ‖ D1 파일 겹침 0」은 오류 → 대장·sessions 는 오케스트레이터 단독(산출 계획 개정).
- ⑵ 태그 보존 적절 · `git-guard.sh:227-239`·`:148-190` 은 기능 브랜치 삭제·`archive/*` push 를 막지 않는다 · `--tags` 금지 → 개별 push · 원격 삭제는 게이트 ③ 뒤 오케스트레이터.
- ⑶ doctor ⑮ — `--repo` 인자 부재 · `/opt/colab-v2` 미마운트 · `ctx` 상수 실물 0건 → 조건 ⓐ`-v /opt/colab-v2:/state:ro` ⓑ`--state-dir` ⓒ마운트 없음/파일 없음 구분(우려 9).
- ⑷ `harness-eval` 세 상태 = CLAUDE.md §4 정합(깨끗) · 선례 `frontend-visual`.
- ⑸ `--allowedTools` 선례 0 · 계측기 경로 fixture 고정(우려 10).
- ⑹ 레인 7 → 5(D2＋D3 합침 · D1+D2+D3 는 반대 — 게이트 집합이 다르다).
- spec 내부 모순 정정 = 우회 표기 `ancestor=bypass` 로 통일 · `tag-release.sh` 는 EC2 미접촉 · 로컬 `dist/colab-v2-dev.sha` 대조만.
