...
- 등록: `.claude/settings.json` `SubagentStart` 에 matcher `researcher` 항목 추가 · `.codex/hooks.json` `SubagentStart` 에 matcher `researcher` 항목 추가(기존 `lane-worker` 항목과 같은 명령) · `.agents/harness.yaml` `hook_names`·어댑터 목록 · `scripts/tests/test_harness_source_layout.py` 이름 목록 · 새 `.sh` 2개 `git update-index --chmod=+x`(규칙 `4-3`).
