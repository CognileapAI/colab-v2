...
- V7 팔레트 안내는 목록이 1개 이상이면서 3개가 아닐 때만. 0개 · 조회 실패는 `UNAVAILABLE` 문장만(안내 없음) ＋ 조회 실패 때 「다시 시도」 단추가 팔레트를 다시 조회(0개에도 둘지는 우려 4) — 확인 방법: RTL(0 · 1 · 2 · 3 · 4 · 실패 → 재시도).
...
- 모듈 · 인터페이스:
  ...
  - 팔레트: 안내 조건 = 목록 1개 이상이면서 3개가 아님. 0개 = 조회 실패와 같은 오류 상태. 「다시 시도」 = 팔레트 조회 실패일 때(0개 포함 여부는 우려 4 판정) 오류 문장 옆 작은 단추 · 누르면 팔레트 재조회 카운터 증가 ＋ 오류 해제. 오류 상태는 그리기와 공유하므로 단추 표시는 팔레트 상태로만 정한다. 같은 컴포넌트의 기존 「다시 불러오기」 카운터 형태를 재사용한다.
...
## 시험 결정
- seam 구성은 **Ted 확인 대기**(to-spec 2단계) — 권고 ⓐ = 아래 기존 seam 5종 ＋ 신설 1. 다른 구성이면 이 절과 부록 C · E 를 고친다.
...
| V7 | 0 → 안내 없음 · `up-preview-error` = `UNAVAILABLE` · 「다시 시도」 1(우려 4 ⓐ · ⓑ 면 0) · 그리기 비활성 / 1 · 2 · 4 → 안내 있음 · 「다시 시도」 0 / 3 → 안내 0 / 실패 → 「다시 시도」 → 조회 2회 · 성공(3) 뒤 오류 0 · 그리기 활성 / 그리기 실패 오류에는 「다시 시도」 0 |
