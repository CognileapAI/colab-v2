...
## 2. 원한 결과 (V-id)
- V-A1 러너가 `eval/harness/model.txt`(1줄 · 해시 집합 안)를 읽어 `claude -p --model <값>` 으로 호출하고, 정본 부재·빈 값·형식 위반(`^claude-[a-z0-9.-]+$` 밖)이면 78(기본 모델로 재지 않는다) · `config-hash.json` 에 `model`(정본) · `model_usage`(실측 `canonicalModel` 정렬 목록) · `summary.md` 요약줄 끝에 ` · 모델 <정본>` · 별도 줄 `- 모델 — 정본 <값> · 실측 <목록>` · `record-usage` 실패 = 준비 +1 · `summary.md` 를 끝까지 쓴 뒤 78 — 확인: `eval/harness/tests/run-selftest.sh` 새 케이스 ⓝ·ⓞ·ⓟ·ⓡ.
...
- V-A7 `harness.yaml hygiene.home_path_roots` 9개 · 현 트리 홈 경로 판정 0건(results 6줄 `<repo>` · prd 4줄 `~/` 치환 · `/home/user/` 는 allow) · `.github/workflows/agent-bridge.yml` `paths` 에 새 뿌리 3줄(CI 에서 `harness-contract` 를 도는 유일한 자리 `:62` 가 새 뿌리 변경에 돈다) · `harness-contract` green(green 줄 `home-path roots 9`) — 확인: 게이트 red→green 로그 · `test_harness_config.py:204-228` fixture 에 roots 반영 · `grep -c "dev-package/prd/\*\*" .github/workflows/agent-bridge.yml` = 1.
- V-A8 `harness-eval` 면제 게이트 green(PR A head 회차 · 정본 모델 · `hash(head)=hash(회차)`) · 회차 뒤 집합 파일 push 0 · 회차 = 새 기준선(직전 없음 · 참고 `20260926-143218`).
- V-A9 문서: `README.md:108`(「호출 환경의 기본값」) · `:150-151`(집합 목록) · `:164`(「러너는 모델을 지정하지 않아」) · `check.py:79-81` 주석 · `gates/README.md:48` · `gates/tools/harness-eval-selftest.sh:36`(「필터 전용 3줄」) · `gates/README.md:295` · `README.md:171`(건수) 가 현 동작으로 교체 · 옛 문장 0건.
- V-A10 Codex 영향 없음: `scripts/agent-bridge.py` · `.codex/**` · `scripts/codex-harness-eval.py` diff 0 · `agent-bridge check` green.

## 3. 해법 개요
- 파일: `eval/harness/model.txt`(신설) · `run.sh`(정본 읽기 · `--model` · `record-usage` 호출 · 요약줄 · `date -u`) · `config_hash.py`(`compute --model` · `record-usage` 하위 명령 · `verify` 모델 조건 · 직전 모델 조건 · dirty 상세) · `config-paths.txt`(집합) · `gates/tools/harness-eval.sh:79`(요약 정규식) · `ci-filter-check.py:44`(FILTER_ONLY) · `.github/workflows/ci.yml:123-134`(필터 3줄) · `scripts/harness/check.py`(UTC) · `.agents/harness.yaml:100`(뿌리 3개) · `.github/workflows/agent-bridge.yml:5-34` `paths`(뿌리 3줄) · 증거·레거시 10줄 치환 · 시험 4벌 · 문서 3곳. 훅 정의 · 등록부(`.agents/ci-producers.json` 내용) · `parallelism.toml` 무변경.
...
- 요약 정규식 3곳: `harness-eval.sh:79` 끝 `판정실패 관측 과제 0$` → `판정실패 관측 과제 0 · 모델 [^[:cntrl:]]+$` · `harness-eval-selftest.sh:198,201` 같은 갱신 · 같은 파일 가짜 러너 요약줄 `:150,153,156,159`(`:144` red 기대 케이스 `zero_summary`·`wrong_*`) 끝에도 ` · 모델 stub-model` 추가(안 하면 계수 불일치가 아니라 정규식 불일치로 red 가 되어 계수 검사를 더는 증명하지 못한다) · `config_hash.py:37-38` SUMMARY_RE 는 앞부분만 읽으므로 불변 · `run-selftest.sh:130,185` grep 은 접두 대조라 불변.
...
- 뿌리만 더하면 `harness-contract` 는 red(판정) 16건(10줄 · results 줄마다 2건)이다(§1 실측). 처리 ⓐ = 치환(채택): results 6줄의 `<checkout 절대경로>` → `<repo>`(S-6a 와 같은 토큰 · `results/**` 는 집합 밖이라 회차 무관 · 모델 응답 본문의 의미 불변 · 커밋 메시지에 치환 명령 원문) · prd 4줄의 홈 접두 → `~/`(레거시 문서 · 승인 intent 아님 → append-only 대상 아님 · `Evidence-Ref:` 행은 값만) · `dev-package/intent` 는 0건이라 편집 0. ⓑ = `home_path_allow` 에 실행자 홈 등재 → 홈 경로가 `harness.yaml` 에 실린다 → 기각(우려 #7).
- 강제 기제: `harness-contract`(장치 · fail-closed · red(판정) 1) · Claude/Codex 공통 · CI 배선: `harness-contract` 를 CI 에서 도는 자리는 `.github/workflows/agent-bridge.yml:62` 하나인데 그 `paths`(`:5-34`)에 `eval/harness/results/**` · `dev-package/**` 가 없다 → 새 뿌리만 바꾼 PR 에서 CI 가 안 돈다(`:15` 주석 규칙 「뿌리 전체 — 일부만 걸면 CI 가 그 밖을 안 본다」 위반 · 다른 머신 팀원이 같은 보호를 못 받는다). `:17` 뒤에 `'eval/harness/results/**'` · `'dev-package/intent/**'` · `'dev-package/prd/**'` 3줄 · `:15` 주석의 뿌리 목록에 3개 추가(커밋 ⑤).
- 시험:
  - red-first = 뿌리 추가 상태에서 `bash gates/run.sh harness-contract` → red(판정) 16건 로그(레인 보고 인용 · 경로:줄 10개 · results 6줄은 2건씩) → 치환 → green(`home-path roots 9 (scanned N …)`). 로그와 치환은 같은 커밋 ⑤(중간 red 커밋을 남기지 않는다 · 로그는 본문에).
  ...
  - `grep -rnoE '/home/[A-Za-z0-9._-]+/|/Users/[A-Za-z0-9._-]+/' eval/harness/results dev-package/intent dev-package/prd | grep -vc '/home/user/$'` = 0(치환 뒤 · allow `/home/user/` 는 `-v` 로 제외 · V-A7).
  - `grep -c "dev-package/prd/\*\*" .github/workflows/agent-bridge.yml` = 1(CI 배선 · V-A7).
- 병합 조건 = 게이트 green ∧ 치환 diff 가 홈 경로 10줄뿐(`git diff --stat` 10 파일 · `agent-bridge.yml` 은 별도 3줄) · 지우는 산문 = 없음 · 출처 = 권고 5 · E-6 · D4.
...
4. ⑤(A-5) 뿌리 추가 → `harness-contract` RED 16건(10줄) 로그 → 치환 → GREEN(같은 커밋 · 로그는 보고에).
5. 기존 시험 무변경 green · 시험 수 증분 기록(`run-selftest.sh:7,308,311` · `harness-eval-selftest.sh:8,281` · `gates/README.md:295` · `eval/harness/README.md:171` · S-red 뒤 기준 +5 / +1 / 정책 +1 — 실계수는 레인이 적는다) → 커밋 ⑥ 실측.
...
- 커밋 단위 — 각 커밋의 마지막 문단 하나 = `Intent-Ref` 1줄 + 레인 세션의 attribution 줄(memory 「트레일러는 마지막 문단 하나에」 · 두 `-m` 으로 나누면 `intent-ref` 가 못 읽는다):
  ...
  - ② A-1 — `model.txt` · `run.sh`(정본 · `--model` · `record-usage` · 요약줄) · `config_hash.py`(compute · record-usage · verify 2조건) · `harness-eval.sh:79` 정규식 · selftest 2벌(ⓝ–ⓟ · ⓡ · ⓢ · `:150-159,198,201`) · `test_harness_eval_gate.py` ⓐ–ⓓ · `README.md:108,164` · `check.py:79-81`.
  ...
  - ⑤ A-5 — `harness.yaml` 뿌리 3개 · `agent-bridge.yml` `paths` 3줄 · 치환 10줄 · `test_harness_config.py` fixture · 본문에 red 16건 로그와 치환 명령.
...
| V-A7 | `bash gates/run.sh harness-contract` · `grep -rnoE '/home/[A-Za-z0-9._-]+/' eval/harness/results dev-package/intent dev-package/prd \| grep -vc '/home/user/$'` · `grep -c "dev-package/prd/\*\*" .github/workflows/agent-bridge.yml` | green(`home-path roots 9`) · 0(allow `/home/user/` 제외) · 1 | 4.5 |
...
| 1a | #1 의 조건 — 정본 모델 접근 불가(rate limit · 권한) | 러너 78(`run.sh:154-156` · `claude -p` rc≠0) · 대체 모델로 재지 않음 · #1 ⓐ 채택 시 T12 는 Fable 가용 시점까지 대기 · PR 2 회차 순서(머리 「PR 2 회차 앞 병합」)도 같이 밀림 · 모델 접근권이 없는 팀원도 같은 78 | 가용 모델로 정본을 바꿔 회차(= #1 ⓑ 로 전환 · `model.txt` 변경 = 해시 변경 → 재실측) | ⓐ — 정본 고정이 「같은 판정」의 전제 · 대체는 정본 변경 PR 로만 · #1 판정 때 이 대기 조건을 함께 본다 | 〈판정 대기〉 |
