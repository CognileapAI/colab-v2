...
- 우려 항목 판정 = 11라운드(2026-09-26 · Opus blind 판정자 — Fable 주간 한도로 Opus 가 대행 · 교차 점검 3건 반영 · Ted 「권고대로」) · 원문 `~/.claude/reports/harness-state-20260925/team-shared-20260926/grill-AB/`(`B-verdict.md` · `cross.md`). 본문 반영 = #1 보정 ⑴ ⑵(4.1 · V-B5 · 4.2 · §5 · §6 · §9) · #2 형식만(4.1) · #3 ADR-0007 줄 추가 확정(§7) · #5 검사식 · 토큰(V-B9 · 4.4 · §7 · §8 · §9).
...
- V-B5 스냅샷은 도입 뒤 불변 — `LEGACY_SNAPSHOT ∈ changed(fork..head)` ∧ base 에 존재하면 head 블롭이 base 블롭과 바이트 동일해야 한다 · 차이(이름 추가 · 삭제 · 주석 · 파일 부재) = `red(판정)` · 스냅샷을 바꾸지 않은 PR(B 병합 전 분기해 head 에 스냅샷이 없는 열린 PR 포함) · base 부재(도입 PR) = 검사 생략 · base · head 모두 부재 = `red(준비)` 78(형식만으로 떨어지지 않고 legacy 규칙을 전체에 넓히지도 않는다). 이름 추가도 red 인 이유 = 새 intent 를 legacy 표기(「승인」)로 쓰고 같은 PR 에서 스냅샷에 이름 1줄을 더하면 handle 없이 approved · 보호가 된다(Q4 「그 꼴만 승인」 우회).
...
- V-B9 `dev-package/reports/harness/20260925-harness-state/` 에 88 파일 · `check_home_paths` 와 같은 식으로 셌을 때 0건(`/home/user/` 는 allow · `scripts/harness/config.py:26-36` `HOME_PATH` 4꼴 + `home_path_allow` exact 대조 `:283,317-318` · 4.4) · `README.md` 색인 1개 · 비밀 0(사전 스캔 결과 0 · T1 JSON 은 ruleset 스냅샷 · `integration_id: 15368` 은 GitHub Actions 앱 id).
...
- 스냅샷 로더 `legacy_names(root, base, head)`: `show(root, base, LEGACY_SNAPSHOT)`(`:77-79` 재사용)이 있으면 그 내용 · 없으면(도입 PR) `show(root, head, …)` → `#` 주석 · 빈 줄 제외 · 둘 다 없으면 `raise Readiness("legacy 승인 스냅샷이 base · head 모두에 없다 — scripts/harness/intent_legacy_approved.txt")` → 78(`main()` `:187-190` 기존 경로 · 우려 #1 보정 ⑵ · 「형식만」으로 떨어져 63건이 비보호가 되지도 · legacy 규칙을 전체에 넓혀 Q4 「그 꼴만 승인」을 벗어나지도 않는다 · AGENTS 「아무 선언도 없으면 준비 실패」). 발생 구간 = B 코드 ∧ 스냅샷 전 = B 레인 §5 2→3 단계 사이뿐(커밋 ② 는 스냅샷 동반). 합집합 없음(base 우선). `judge()` `:153-160` 은 `legacy = legacy_names(root, base, head)` 로 `is_protected` 를 부른다(스냅샷을 아직 안 가진 열린 PR 도 Update branch 전에 base 쪽 스냅샷으로 보호된다 · base 에 있으면 head 쪽은 판정에 쓰지 않는다 · head 가 스냅샷을 바꿨으면 아래 불변 검사가 red).
- 불변 검사(`:175` 앞 · 우려 #1 보정 ⑴): `LEGACY_SNAPSHOT in changed`(`:115-116` 의 `fork..head` 변경 파일 · 기존 ⑵ 가 `:161` 에서 쓰는 거름과 같다) ∧ base 에 있으면 `blob(root, base, LEGACY_SNAPSHOT) != blob(root, head, LEGACY_SNAPSHOT)`(`:71-74` 원본 바이트 · head 부재 포함) → `red.append("⑵ legacy 승인 스냅샷이 바뀌었다 — 도입 뒤 불변 · 추가 · 삭제 · 주석 모두 금지 · 새 intent 는 승인: @handle 형식")`. 스냅샷을 바꾸지 않은 PR · base 에 없음(도입 PR) → 검사 생략. `changed` 로 거르는 이유 = 대조 대상 = head sha(`ci.yml:676` `COLAB_INTENT_REF_HEAD`) · 실행 코드 = 머지 ref(`actions/checkout` 기본값 · `ci.yml:667-669` ref 미지정) → B 병합 전 분기한 열린 PR 도 B 코드로 돌고 head 에 스냅샷이 없어, 원안(base 에 있으면 무조건 대조)은 그 PR 의 push 마다 거짓 red(팀 원칙 「같은 비용」 위반). 이름 삭제 · 추가 · 주석 변경 · 다른 사본 반입 · 파일 삭제는 모두 `changed` 에 들어가 여전히 red.
...
- 강제 기제: 게이트 `intent-ref`(`gates/run.sh:365-369`) · CI `intent-ref` 잡(`ci.yml:663-677` · base..head) · fail-closed(base · head 모두 스냅샷 부재 = 준비 실패 78 · 도입 뒤 스냅샷 변경 = red) · Claude/Codex 공통(판정기 1개 · 훅 아님 · ADR-0007 `:15` 「훅은 두지 않는다」). 시험(4.2)의 CI 실행처 = `agent-bridge.yml` `compatibility` 잡뿐(필수 check 아님 · 4.2) — 병합 차단 장치는 `intent-ref` 잡 하나.
...
- 출처: intent `:627` Q4 · report.md 권고 6 · D2 · 우려 #1(전환 규칙 · 새 선택지 확정) · #2(handle 검증 · ⓐ 확정 — handle 은 `HANDLE` 정규식 형식 검사만 · 네트워크 조회 · 토큰 없음 · 기계는 형식 · 승인은 사람의 병합 · ADR-0003 `:18-19`).

### 4.2 시험 · 정답표 (단위 B-2 · red-first)
- 파일 `scripts/tests/test_harness_record_gates.py` `IntentRefGateTests`(`:149`) — 상수 `APPROVED`(`:24-26`) 의 `승인: **Ted 2026-01-01**` → `승인: @fixture-approver 2026-01-01 "approve"` · `:245` `:254` 의 `"승인 2026-01-03 Ted"` → `'승인: @fixture-approver 2026-01-03 "ok"'` · `:383` TEMPLATE 단언의 메타를 새 꼴로. `repo()`(`:154-163`)에 legacy 표기 intent `2026-01-03-legacy.md` 와 스냅샷 파일(그 이름 1줄) 추가(⑧ 외 모든 fixture repo 는 base · head 중 한쪽 이상에 스냅샷을 가진다 — 없으면 기존 사례가 78).
- 새 사례: ① 새 꼴 approved · 줄 편집 → red(V-B1) ② `2026-01-03-legacy.md` 를 스냅샷에서 뺀 fixture → 편집 green(V-B2) + base 에 스냅샷이 있는 repo 에서 head 스냅샷의 이름 삭제 · 이름 추가(legacy 표기 새 intent 동반) · 주석 1자 변경 · 파일 삭제 → 각각 red(V-B5) · base 부재 repo 에서 스냅샷 신설 → 검사 생략 green · base 에 스냅샷 ∧ fork · head 부재(B 병합 전 분기한 열린 PR · 스냅샷 무변경) → 불변 검사 생략 · base 스냅샷으로 legacy 보호 green(보정 ⑴) ③ 스냅샷 안 legacy → 편집 red(V-B2·B3) ④ 형식 위반 6꼴 → `classify == "unapproved"`(V-B6) ⑤ 새 꼴 + 같은 줄 「미승인 3건 잔존」 → approved(V-B1) ⑥ `--freeze-legacy HEAD` 출력 = 기대 목록 · 2회 실행 바이트 동일(V-B3) ⑦ 출력 문구 `형식 1 · legacy 스냅샷 1`(V-B4) ⑧ base · head 모두 스냅샷 부재 repo → 종료코드 78 · `red(준비)` 출력(V-B5 · 보정 ⑵ · 형식만 green 도 legacy 전체 적용도 아님).
...
- 치환(순서 고정 · `find <dir> -type f -exec sed -i 's#…#…#g' {} +` 1패턴 1명령 · 리터럴만 · `sed -i` 는 GNU 형식이라 macOS BSD sed 에서 그대로 돌지 않는다 → 1회 반입 명령 · 결과 파일이 정본 · 재현 대상 아님): ⑴ `<home>/workspace/00_Project/00 CoLAB/31 CoLAB-v2/.claude/worktrees/harness-improvement` → `<repo>/.claude/worktrees/harness-improvement`(9곳) ⑵ `<home>/workspace/00_Project/00 CoLAB/31 CoLAB-v2` → `<repo>`(5곳) ⑶ 남은 실행자 홈 절대경로 → `<home>`(`.claude/jobs/…` 13 · `.claude/reports/…` 4 · `workspace/…` 2) — 이 문서의 `<home>` = 실행자 `$HOME`(명령에서는 sed 패턴을 큰따옴표로 써 `$HOME` 을 전개 · 문서에 실제 홈 경로를 적지 않는다). `~/.claude/...`(40곳) 은 절대경로가 아니므로 원문 유지 · `/home/user/`(`harness.yaml:101` `home_path_allow` 값 · 원본 3파일) 유지. ⑷ ⑶ 뒤 아래 검사에 남는 적중(다른 사용자 홈 · `/mnt/<d>/Users/…` · `C:\Users\…` 꼴 포함 · allow 제외)도 전부 `<home>`(저장소 경로면 `<repo>`)으로 치환 — B 병합 뒤 PR A 가 `home_path_roots` 에 이 디렉터리를 넣는 순간 반입물이 곧바로 검사 대상이므로(intent `:629` B 선행) 적중 0 이어야 A 가 green 을 유지한다. 검사(우려 #5) = ad hoc grep 이 아니라 `check_home_paths` 와 같은 식: `PYTHONPATH=scripts/harness python3 -c` 로 `config.load_contract(Path(".agents/harness.yaml"))` 의 `hygiene` 에서 `home_path_allow` 는 그대로 · `home_path_roots` 만 `["dev-package/reports/harness/20260925-harness-state"]` 로 바꾼 `value` 를 만들어 `config.check_home_paths(Path("."), value)` 1회 호출 → `([], None)`(`config.py:26-36` `HOME_PATH` 4꼴 · `:271-323` `git ls-files --others` 라 `git add` 전에도 읽는다 · allow exact 대조 `:283,317-318`) · `grep -rlE 'ghp_|github_pat_|sk-ant-|AKIA|PRIVATE KEY' <dir>` 0(사전 스캔 0).
...
- 게이트 영향: `.gitignore:35` `*.log`(해당 파일 0) · `:83` `gate-summary.json`(0) · `exec-bit` 는 `.sh` 만(`exec-bit.sh:16` · 반입에 `.sh` 0) · `decision-number-guard` 는 `PLAN-SoT.md` 만(`:60`) · `harness-contract` `home_path_roots`(`harness.yaml:100`)에 `dev-package/reports` 없음 → 이 PR 의 검사는 `check_home_paths` 같은 식 1회(위 치환 절 · 우려 #5). 상시 검사는 PR A 소유: A 가 `home_path_roots` 에 `dev-package/reports/harness/20260925-harness-state` 정확히 1토큰 추가(상위 `dev-package/reports/harness` 는 기존 5파일 홈 절대경로 6건 잔존이라 A 가 red · 기존 5파일 정리는 A · B 범위 밖) · B 는 `.agents/harness.yaml` 을 만지지 않는다(해시 집합 · 회차 불요 조건).
...
2. `intent_ref.py` 구현 → ①–⑧ GREEN · v1 시험은 스냅샷 없이 RED 상태 유지 · 이 단계와 3 사이 실 checkout 의 `intent-ref` = 78(base · head 모두 스냅샷 부재 · 보정 ⑵ 의 유일한 발생 구간 · 기대 관측) — 커밋 ② 는 스냅샷 파일을 함께 담으므로 커밋된 상태에서는 발생 0.
...
5. 기존 시험 무변경 green 확인 · 시험 수 증가분 기록(v1 62건 표 + 새 사례 8 + v2 1).
- seam: throwaway `git init` fixture(`:2-4`) · 스냅샷 파일을 fixture repo 안에 쓴다 · 실 checkout 무변경.

## 6. 위험 · 롤백
- 보호 축소 창 없음: 스냅샷 없는 head(B 병합 전 분기한 열린 PR)의 CI 는 대조 대상 = head sha(`ci.yml:676`) · 실행 코드 = 머지 ref(`actions/checkout` 기본값 · `ci.yml:667-669`)라 B 코드로 돌고 base 쪽 스냅샷(base 우선)으로 보호된다 · 그 PR 은 스냅샷을 바꾸지 않았으므로(`changed(fork..head)` 밖) 불변 검사 생략 = 거짓 red 없음(보정 ⑴ · 원안 「head 부재 = 차이 red」는 push 마다 red 였다) · 스냅샷을 다른 내용으로 들여온 경우만 red.
- 준비 실패 78(보정 ⑵): 조건 = B 코드 ∧ base · head 모두 스냅샷 부재 → 발생 구간 = B 레인 §5 2→3 단계 사이뿐. B 병합 뒤 CI 는 base.sha 에 스냅샷이 있어 0 · 갱신 안 한 브랜치의 로컬 실행은 자기 브랜치의 구 `intent_ref.py` 를 돈다(78 아님 · 구 규칙 · 갱신 = `git merge develop` 뒤 새 규칙). 「형식만」 폴백(63건 비보호 · fail-open)과 「legacy 전체 적용」(Q4 초과)을 둘 다 배제한 결과.
...
- 스냅샷 손상 · 우회: 스냅샷이 `changed(fork..head)` 에 있고 base 에 있으면 head 와의 바이트 차이 전부 red(이름 삭제 · 이름 추가 · 주석 · 파일 삭제 · 다른 사본 반입 · V-B5 · 모두 `changed` 에 잡힌다) — 이름 추가를 막는 이유 = legacy 표기 새 intent + 같은 PR 스냅샷 1줄로 형식 없이 보호되는 경로 차단.
...
- begin: `python3 scripts/agent-bridge.py lifecycle begin --role lane-worker --gate harness-contract-selftest --gate harness-contract --gate intent-ref --gate exec-bit --scope 'scripts/harness/intent_ref.py' --scope 'scripts/harness/intent_legacy_approved.txt' --scope 'scripts/tests/test_harness_record_gates.py' --scope 'scripts/tests/fixtures/intent-ref/**' --scope 'dev-package/intent/TEMPLATE.md' --scope 'dev-package/intent/README.md' --scope 'dev-package/reports/harness/20260925-harness-state/**' --scope 'dev-package/intent/2026-09-25-harness-improvement.md' --scope 'dev-package/intent/2026-09-26-human-authorization-for-destructive-ops.md'`(우려 #3 ⓐ 확정 → `--gate adr-records --scope 'docs/decisions/0007-intent-ref-trailer-and-append-only-approved-intents.md'` 추가).
- 반입 명령(한 호출 1명령): `cp -r ~/.claude/reports/harness-state-20260925 dev-package/reports/harness/20260925-harness-state` → `find … -type f -exec sed -i '…' {} +` ×3(4.4 순서) → `check_home_paths` 같은 식 1회(4.4 · 적중이 남으면 ⑷ 치환 뒤 재실행) → README 작성. `git add` 는 디렉터리 단위.
- 커밋 단위(각 트레일러): ① B-2 시험 + v2 정답표(RED) ② B-1 판정기 + `--freeze-legacy` + 스냅샷 파일(GREEN) ③ B-3 TEMPLATE · README ④ B-4 반입 + README 색인 ⑤ B-5 두 intent 줄 추가 + ADR-0007 추가 줄(우려 #3 ⓐ 확정) — 되돌림 단위 = ② · ④.
...
| V-B9 | `find dev-package/reports/harness/20260925-harness-state -type f \| wc -l` · `check_home_paths` 같은 식 1회(`PYTHONPATH=scripts/harness python3 -c …` · roots = 반입 디렉터리 1개 · allow = `harness.yaml` 값 그대로 · 4.4) · `ls …/README.md` | 89(88 + README) · `([], None)` · 존재 | 4.4 |
...
- `Plan-Ref: dev-package/prd/specs/S-HARNESS-TEAM-B-APPROVAL-20260926.md`(총괄 §0 10라운드 · PR A 와 병렬) · 결정 = 형식 OR legacy∧스냅샷 · 스냅샷 도입 뒤 불변(스냅샷을 바꾼 PR 만 base 와 대조 · 차이 red) · base · head 모두 스냅샷 부재 = 준비 실패 78 · 형식 판정만(ADR-0003) · v1 정답표 무변경(해시 집합) · 회차 불요 증명 = V-B11 · 남은 제약 = handle 실재 미검증 · harness-evals 비보호 유지 · 반입 디렉터리 홈 경로 검사는 B 에서 `check_home_paths` 같은 식 1회뿐 · 상시 검사는 PR A 가 `home_path_roots` 에 `dev-package/reports/harness/20260925-harness-state` 1토큰 추가로 소유(우려 #5 · 기존 `dev-package/reports/harness` 5파일 홈 경로 6건은 범위 밖) · `harness-contract-selftest` 의 CI 실행처는 `agent-bridge.yml` `compatibility`(필수 check 아님) · 병합 차단은 `intent-ref` 잡(`ci-required`)뿐 → `compatibility` green 은 병합자가 확인.
...
| # | 항목 | ⓐ | ⓑ | 권고 | 판정 |
|---|---|---|---|---|---|
| 1 | legacy 63건 전환 규칙 | 형식 OR (legacy ∧ 동결 스냅샷 `intent_legacy_approved.txt` · base 1회 생성 · 도입 뒤 불변(차이 red) · 영구) | 형식만 · legacy 63건은 비보호로 두고 필요 시 새 intent 재발행 | ⓐ — ⓑ 는 append-only 를 잃고(ADR-0007 `:14`) 재발행은 승인 원문을 새로 쓸 수 없다 · ⓐ 는 스냅샷 밖 legacy 를 비보호로 만들어 「새 intent 는 새 꼴」을 게이트가 강제 · 만료 없음(파일은 영구 동결) | 「새 선택지: 골격 유지(형식 OR (legacy ∧ 동결 스냅샷) · 영구 동결) + ⑴ 불변 검사는 `LEGACY_SNAPSHOT ∈ changed(fork..head)` ∧ base 존재일 때만 ⑵ base · head 모두 부재 = 준비 실패 78」 확정(11라운드 · Opus blind · Ted 「권고대로」) |
| 2 | handle 검증 | 형식만(GitHub login 정규식 · 네트워크 0 · 토큰 0) | `gh api users/<handle>` 실재 확인 | ⓐ — ⓑ 는 오프라인 기계 · 토큰 없는 CI fork 에서 78 → 기계마다 판정이 갈린다(팀 원칙 위반) · 승인은 사람이 병합으로 한다(ADR-0003 `:18-19`) · 권한(develop 리뷰 권한자)도 기계가 안 본다 | ⓐ 확정(11라운드 · Opus blind · Ted 「권고대로」) |
| 3 | ADR 기록 | ADR-0007 「결과와 감수한 비용」 끝에 날짜 줄 1개 추가(ADR-0003 `:35` 「2026-09-17 추가 —」 선례 · 기존 줄 무수정 · 번호 0011/0012 무영향) | 새 ADR-0013(0011·0012 는 PR 2·4 예약 · 병합 순서와 번호 역전) | ⓐ — 판별식 변경은 ADR-0007 `:30` 재검토 조건 적중이며 같은 결정의 개정 · 번호 역전을 피한다 · 채택 시 §7 gate/scope 추가 | ⓐ 확정(11라운드 · Opus blind · Ted 「권고대로」) |
| 4 | `2026-09-08-harness-evals.md`(실승인 · 메타 「미승인」 · v1 `:13-15` 의도된 비보호) | 그대로(비보호 유지 · 사람이 원하면 새 꼴로 메타 줄 수정 = 미승인이라 편집 가능 · 날짜는 수정 커밋일) | 스냅샷에 예외로 넣어 보호 | ⓐ — 스냅샷은 규칙의 기계 출력이어야 재현된다(`--freeze-legacy` 바이트 동일) · 예외 1건은 손 편집 | ⓐ 확정(11라운드 · Opus blind · Ted 「권고대로」) |
| 5 | 반입 보고서의 홈 경로 검사 | 이 PR = grep 검사 · `home_path_roots` 는 A 의 `.agents/harness.yaml` 몫 → A 에 `dev-package/reports/harness` 1토큰 추가(같은 파일 · 같은 회차 · 단 기존 `dev-package/reports/**` 에 홈 절대경로(`HOME_PATH` 꼴 · `scripts/harness/config.py:29-33`) 잔존 0 확인 뒤) | B 에서 `harness.yaml` 수정 | ⓐ — `.agents/**` 는 해시 집합(`config-paths.txt:12`) 이라 B 가 만지면 회차 조건이 생긴다 · A 는 10라운드 확정 목록(`eval/harness/results` · `dev-package/intent` · `dev-package/prd`) 에 1토큰 추가 = Ted 판정 | 「새 선택지: 소유 분리 유지(B = 1회 검사 · A = `home_path_roots` 소유) · A 토큰 = `dev-package/reports/harness/20260925-harness-state`(정확한 하위 디렉터리) · B 검사 = `check_home_paths` 와 같은 식(ad hoc grep 아님)」 확정(11라운드 · Opus blind · Ted 「권고대로」) |
| 6 | `~/.claude/...` 상대 표기 40곳 | 원문 유지 + README 대응 1줄 | 반입 경로로 재작성 | ⓐ — 증거 무수정 · 절대경로가 아니라 위생 검사 대상도 아님 | ⓐ 확정(11라운드 · Opus blind · Ted 「권고대로」) |
...
## 범위 밖
- ruleset 개정 · T11 PAT · CODEOWNERS(`.github/CODEOWNERS:5` `@sungwooHa`) 교체 · `grill-me`/`researcher.md`/`colab-rules §0` 역할 어휘(PR 4) · `.agents/harness.yaml`(A) · `gates/fixtures/intent-ref/*.json` 수정(해시 집합) · S-auth `authorized_by`(별도 intent ⓒ) · 총괄 §10 의 후속 갱신(PR 2) · E0/S-red spec 의 「근거(저장소 밖)」 문구 · `~/.claude/pr-bodies/` 반입 · handle ↔ GitHub 권한 조회 · 승인 날짜 달력 검증.
