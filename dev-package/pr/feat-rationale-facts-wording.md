...
Head-SHA: 827243ee88ca1a7f26b91a137da5280533eacd32
검증 상태: 부분 검증

<!-- 저장소 파일의 Head-SHA 는 증거 커밋(827243ee · 검토 반영 뒤 게이트를 다시 돈 커밋)이다. 이 요약 파일 커밋이 그 위에 1개 더 있다. PR 본문은 Head-SHA 를 PR head 로 채워 pr_contract --mode draft 를 통과시킨 것이다. -->
...
- core-api(`e75aacc6` · 계약·생성물·frontend 변경 0건)
  ...
  - `app/dataset_search.py`
    ...
    - `rationale_facts`(검토 반영 `827243ee`): 자리 목록에서 「온톨로지 연결 근거」도 뺀다(선행 intent Q7 · intent 범위 밖 절 추기). 파일 근거·온톨로지 자리로만 맞으면 낱말 일치 줄 없이 그 종류 사실만 낸다(온톨로지만이면 「관련 개념」 하나 · 종전 동작).
...
- 시험
  ...
  - `tests/test_search_assembly.py`: 주제 라벨 ⑴ 확장만이면 빠짐 ⑵ 친 라벨은 남음 ⑶ 라벨만이면 남음 ⑷ 뺀 자리에 4번째 낱말 · ③ 자리 목록 · `rationale` 한 줄 일치 · 파일 근거만이면 파일 근거 항목만
    - 검토 반영: 파일 전체 autouse 확인(`_no_mechanism_names`)이 모든 조립 시험의 사실 항목·`rationale` 한 줄에 「온톨로지」「계보」 부재를 단언 · `test_낱말_일치_자리_목록에_온톨로지_연결_근거가_없다` · `test_파일_근거와_온톨로지로만_맞으면_낱말_일치_없이_두_종류가_선다` · 온톨로지만 맞은 경우(낱말이 있어도) `concept` 하나 단언 보강
...
- 커밋 순서
  ...
  3. `70570d97` 이 요약(초판)
  4. `827243ee` 검토 반영 — 「온톨로지 연결 근거」 자리 이름 제거 + 시험 + intent 추기
  5. 이 요약 갱신(PR head)
- 남은 단계
  - advisor ② 수용 검토
...
## 검증
- 게이트(task `4cc2d909bf3e400cafa06dd8c4949d9e` · run `9d5f83b8f4cf4e36a075250fc175771f` · 기준 커밋 `827243ee` · `gates/run.sh task` 1회 · 계 **green 5 / red(판정) 0 / red(준비) 0**)
  - 초판 실행(task `0a6ef136…` · run `efd7cff3…` · 기준 `e75aacc6`)도 green 5 / 0 / 0 이었다.
...
| contract-breaking | 0 | 기준 HEAD 대비 파괴적 변경 없음(계약 변경 0건) · 초판 보충 실행 `COLAB_BREAKING_BASE_REF=origin/develop` exit 0 |
| generated-up-to-date | 0 | 등기부 20건 재생성 일치 |
| service-tests-core-api | 0 | 1916 통과 · skipped 0 · deselected 9 |
| intent-ref | 0 | Intent-Ref 4건 유효(범위 `dcaaf2f6..827243ee` · 커밋 4 · 대상 경로 7) · 승인 intent 58건 대조(추기 = 줄 추가) |
...
- RED 선확인(구현 전 · 시험만 추가한 상태): core-api 11 failed · 1903 passed
  ...
  - 구현 뒤 1914 passed
- RED 선확인(검토 반영 · 시험만 추가한 상태): core-api 2 failed · 1914 passed
  - `AssertionError: ['‘강우’가 이름·주제·요약·온톨로지 연결 근거에 맞았어요']` · `AssertionError: ['‘강우’가 온톨로지 연결 근거에 맞았어요']`
  - 구현 뒤 1916 passed
...
| 자리 목록에 「온톨로지 연결 근거」 없음(선행 Q7 · 추기) | 전: 「‘강우’가 이름·주제·요약·온톨로지 연결 근거에 맞았어요」 → 후: 「‘강우’가 이름·주제·요약에 맞았어요」 · 온톨로지만이면 `concept` 하나 · 파일 근거+온톨로지만이면 `concept`·`evidence` | `test_낱말_일치_자리_목록에_온톨로지_연결_근거가_없다` · `test_파일_근거와_온톨로지로만_맞으면_낱말_일치_없이_두_종류가_선다` · autouse 「온톨로지」「계보」 부재 확인 | 확인됨(시험) · dev 미재캡처 |
...
Evidence-Ref: 로컬 task runtime gate-summary(task `4cc2d909bf3e400cafa06dd8c4949d9e` · run `9d5f83b8f4cf4e36a075250fc175771f`) — CI 증거 아님
...
- 후속(범위 밖)
  ...
  - (해소 · `827243ee`) 자리 목록의 「온톨로지 연결 근거」 노출은 이 PR 에 접어 고쳤다(intent 범위 밖 절 추기).
