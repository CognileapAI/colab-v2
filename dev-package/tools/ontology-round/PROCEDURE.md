...
이 문서가 절차 정본이다. 「회차 돌려」, 「새 자료로 온톨로지 보강」처럼 명시로 요청할 때 따른다.
단순 검색 질문이나 한 건 수정에는 쓰지 않는다. 설치·명령은 같은 폴더의 `README.md`.
...
dev 수집 자격이 없으면 `--database-url` 로 로컬 DB 회차만 가능하다. 자세한 것은 같은 폴더 `README.md`.
