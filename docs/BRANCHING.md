...
| `prod-YYYYMMDD` 태그 | dev 배포 창 N회를 green 으로 넘긴 `main` 커밋 | 복귀 없음 | 없음 | Ted | ⭑ ⟨개정 2026-09-12⟩ prod 는 **실재**하고(`〈395〉` 가 `㊻` 보류를 해제했다) **그 태그에서만 배포**한다 — 집행 자리 = `infra/prod/ship.sh` 의 태그 검사(없으면 exit 65 · **우회 변수 없음**) ／ 종전 ~~prod 는 `PLAN-SoT §9-㊻` 로 보류 · 그 태그에서만 배포~~ |
