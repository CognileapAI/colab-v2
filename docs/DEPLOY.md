...
／ 종전 ~~dev 환경 하나 · **prod 는 아직 없다**(정본 `㊻` 가 ⏸, Ted 판정 선행)~~ — `㊻` 보류는 `〈395〉` 로 해제됐고 prod 는 §5 대로 실재한다.
...
⭑ **⟨증보 2026-09-06 · `〈395〉`⟩ 두 벌이 됐다 — 아래 표는 dev 이고, prod 는 그 아래 절이다.**
...
⭑ **⟨개정 2026-09-06 · `〈395〉`-㉱ · 유료 전환 뒤 실측⟩** ／ 종전 ~~Free Plan 전제의 누적·월간 두 벌~~
...
- **두 겹이다** — ⑴ RDS 자동 백업 — dev **1일**(Free Plan 때의 값 그대로) · **prod 7일**(`〈395〉`-㉯ · 그 기간 안에서 **임의 시점으로 되감을 수 있다**) ⑵ **`backup.sh` 가 하루 1회 `pg_dump` → S3 `_ops/backups/<벌>/`, 30일 보관**(수명 주기가 강제)
...
⭑ **⟨개정 2026-09-06 · `〈395〉`-㉰⟩ 유료로 전환했다 — 「어느 날 그냥 멈춘다」는 사라졌다.**
