...
- 날짜: 2026-09-17
- 대체함: 없음
- 대체됨: [ADR-0005](0005-harness-controls-are-declarative.md)
...
## 재검토 조건
- `gates/run.sh`에 프로세스 간 뮤텍스가 들어가고 `_pg.sh` postgres 슬롯이 레포 루트로 키잉될 때.
  ⚠ 2026-09-17 정정 — 이 두 수정은 **#56·#47 의 범위가 아니다**(#56 = 병렬 안전성 미선언 8건 ＋ 측정 레인의
  생애주기 미결속 · #47 = 워커 간 DB 시드 공유). 담는 이슈가 아직 없다. 이 ADR 은 그 구조 수정 전까지의 운영
  규칙이며, `.agents/rules/colab-rules.md §3-1` 「실행 레인은 한 시점에 하나」(2026-09-05 Ted 결정)와 같은 내용을
  실측 근거와 함께 적은 것이다 — 결정의 원본은 §3-1 이다.
  ⭑ 2026-09-18 — 위 첫 재검토 조건이 발동해 이 ADR 은 **superseded** 다.
  [ADR-0005](0005-harness-controls-are-declarative.md) 의 「2026-09-18 개정」이 `_pg.sh` 슬롯 세 갈래를
  `red(준비 · 78)` 로 승격했고(PR ①), `gates/run.sh` 호스트 전역 잠금을 후속 PR ② 로 담았다.
  ⚠ 「실행 레인은 한 시점에 하나」의 **원본은 `.agents/rules/colab-rules.md §3-1`**(2026-09-05 Ted 결정)이고
  그 규칙은 계속 선다 — 대체되는 것은 이 ADR 이 적은 **한시 운영 근거**뿐이다. 이력은 삭제하지 않는다
  (`docs/decisions/README.md:8`).
- 게이트가 원격 실행기로 옮겨져 호스트 부하와 분리될 때.
- 「판정부 단일화·정본 지역성」 ADR 이 accepted 로 서면 이 ADR 은 그것으로 대체(superseded)한다.
