...
class HarnessSourceLayoutTests(unittest.TestCase):
    ...
    def test_shared_skill_bodies_and_claude_adapters_are_not_duplicated(self):
        ...
        # issue-before / issue-after were added to the shared skills.
        self.assertEqual(len(shared_names), 18)
    ...
    def test_hook_judges_are_shared_and_claude_files_are_adapters(self):
        names = [
            ...
            "ponytail-inject.sh", "test-file-guard.sh", "uncommitted-artifacts.sh",
            "worktree-setup.sh",
        ]
        for name in names:
            with self.subTest(name=name):
