...
class AgentConfigurationTests(unittest.TestCase):
    ...
    SELECTABLE_CODEX_MODELS = frozenset({'gpt-6-astra', 'gpt-5.6-sol'})
    ...
    def test_role_models_are_explicit_and_do_not_claim_parent_inheritance(self):
        ...
        # with the spec's fallback for models the account cannot select.
        expected = {
            'advisor': ('gpt-6-astra', 'high'),
            'lane-worker': ('gpt-5.6-sol', 'high'),
            'researcher': ('gpt-5.6-sol', 'medium'),
            'measurement-lane': ('gpt-5.6-sol', 'low'),
            'gate-runner': ('gpt-5.6-sol', 'low'),
        }
        self.assertEqual(
