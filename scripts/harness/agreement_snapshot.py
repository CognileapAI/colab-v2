...
# Adapted from sungwooHa/ai-sdlc-harness @ fc424e8f21f5d4479a43780e41ce9883ed1dbc7f.
# CoLAB adapter: local CLI only; no automatic approval or lifecycle hooks.
...
def no_symlink_ancestors(path: Path) -> None:
    if any(parent.is_symlink() for parent in (path.absolute(), *path.absolute().parents)):
        raise ValueError("snapshot path must not contain symlink ancestors")


def view_directory(folder: Path, repo: Path | None = None) -> Path:
    name = None
    if repo is not None:
        no_symlink_ancestors(repo)
    for ancestor in ((repo,) if repo is not None else (folder, *folder.parents)):
        manifest = ancestor / ".agents/harness.yaml"
        if manifest.is_file():
            config = json.loads(manifest.read_text(encoding="utf-8"))
            name = config.get("agreement_snapshot", {}).get("deliverables_dir")
            break
    if not isinstance(name, str) or not name:
        raise ValueError('repository agreement_snapshot config required; external drafts need --repo')
    path = Path(name)
...
def freeze(folder: Path, revision: str, approval_ref: str, *, repo: Path | None = None) -> Path:
    no_symlink_ancestors(folder)
    ...
    views = view_directory(folder, repo)
...
def main() -> int:
    ...
    parser.add_argument("--repo", type=Path, help="repository containing .agents/harness.yaml (required for external drafts)")
    ...
    try:
        ...
        path = freeze(args.folder, args.revision, args.approval_ref or "", repo=args.repo)
