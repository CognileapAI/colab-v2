...
if sys.argv[1] == '--ids':
    ...
    Path(sys.argv[2]).write_text('# Auto-generated from core kernel ids; do not edit.\n' + source +
        ...
        # new_ulid is not the ID of anything stored: lineage suggestionId only lets the
        # screen tell items apart (core-ai.yaml suggestLineage prose); suggestions die
        # with the response (d10_suggestion header).
        '\n\n# 저장된 무엇의 ID 가 아니다 — 계보 제안의 suggestionId 는 화면이 항목을 구분하는 값이고\n'
        '# 제안은 응답과 함께 죽는다(core-ai.yaml suggestLineage 산문 · d10_suggestion 머리말).\n'
        'def new_ulid(now_ms: int | None = None) -> str:\n    return str(Ulid.generate(now_ms))\n')
    raise SystemExit(0)
