...
11. **데이터셋 행을 지우는 유일한 자리는 `services/core-api/ops/purge_datasets.py` 다.**
    ...
    **S3 는 exact-key 계획 ＋ sha256 대조로만 지운다** — 접두사·`--recursive` 삭제를 쓰지 않는다(선례 `〈354〉`).
    ...
    `count-before.json` ＋ 원격 challenge nonce(dev 호스트 sudo 로만 읽힌다 — 실행 자리·로그·`reset-ack.json` 에는 nonce 를
    남기지 않고 sha256 만 둔다 · 2026-09-25 dev 검증 보정)로 토큰 로컬 재계산 · env 파일·Write 도구로 값 주입.
