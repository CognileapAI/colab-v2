---
name: dev-reseed
description: dev 환경 초기화와 정본 자료 재적재가 명시적으로 요청됐을 때 `dev-package/tools/dev-reseed/reseed.sh` 10단계를 순서대로 실행한다. staging·prod 와 단순 조회에는 쓰지 않는다.
---

# Codex 연결

먼저 `AGENTS.md`와 `docs/development/dual-agent.md`를 읽고, `.claude/skills/dev-reseed/SKILL.md`를 읽어 수행한다.
