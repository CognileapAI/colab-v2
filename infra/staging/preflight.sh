...
preflight_required() { # $1=env 파일
  ...
  while read -r k; do
    ...
    case "$k" in
      ...
      *_FILE)
        [ -f "${!k}" ] || { fail "$k 가 가리키는 파일이 없다 (경로는 출력하지 않는다)"; continue; }
        if [ "$k" = COLAB_STAGING_ACCOUNT_ADMIN_DB_URL_FILE ]; then
          [ "$(stat -c '%a' "${!k}")" = 600 ] || fail "$k 파일 모드는 0600 이어야 한다"
          [ "$(stat -c '%u' "${!k}")" = 10001 ] || fail "$k 파일 소유 uid는 10001 이어야 한다"
        fi
        ;;
