...
for P in $(backup_profiles); do
  ...
  echo "════════ 프로파일 $P (db=$DB)"
  # ── 프로파일별 최신본을 **각각** 고른다 — 결함이 아니다 (Ted 판정 2026-08-30) ──
  #   지적된 모양: 프로파일마다 독립으로 `ls -1t | head -1` 을 돌리므로, 두 프로파일의
  #   산출물이 **같은 회차의 것이 아닐 수 있다**. 「한 회차로 묶어 고르라」는 제안이 나왔다.
  #   판정: **묶지 않는다.** 프로파일은 회차와 **다른 축**이다 — `platform` 과 `ai` 는
  #   마이그레이션 체인이 분리된 별개 DB 이고(`CLAUDE.md §3-3`), 백업 일정도 각자 돈다.
  #   한 회차로 묶으면 「짝이 맞는 회차가 없다」는 이유로 리허설이 통째로 서고, 그것은
  #   **각 체인의 최신 백업이 실제로 복원되는가**라는 이 스크립트의 목적을 깎는다.
  #   프로파일을 가로질러 각자의 최신본을 검증하는 것이 의도다.
  #   ⚠ 다음 회차가 이 자리를 다시 파지 않도록 적어 둔다. 되짚을 근거가 바뀌면 이 주석을 고친다.
