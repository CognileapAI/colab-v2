#!/usr/bin/env bash
# 백업 스케줄 설치/해제. 경로를 문서에 박지 않고 실행 시점에 결정한다.
#   install-schedule.sh install   현재 crontab 에 CoLAB 백업 블록을 병합
...
#
# 배포 블록(`pipeline/install-schedule.sh`)과 **다른 표식**을 쓴다 — 한쪽을 지울 때
# 다른 쪽이 같이 지워지면 「걸어 뒀다」가 거짓이 된다.
#
# ── 이 파일이 조심하는 두 가지 (배포 쪽에서 먼저 고친 것을 그대로 옮겼다) ─────
# ⑴ **「설치했다」와 「걸려 있다」는 다르다.** `crontab -` 의 종료코드는 앞의 것만 안다.
#    붙였다는 사실이 붙은 자리를 가리킨다는 보증이 아니다. 그래서 설치한 **뒤에**
#    다시 읽어서 판정한다(`verify_installed`).
# ⑵ **통째로 날아가는 모양.** `crontab -l 2>/dev/null` 은 「크론탭이 없다」와
#    「crontab 명령이 실패했다」를 **같은 빈 출력**으로 만든다. 그 빈 출력을 그대로
#    `crontab -` 에 넣으면 형제 블록(배포 스케줄)과 남의 항목까지 사라진 채
#    「설치됨」이 찍힌다. 그래서 ⓐ 읽기 실패와 「없음」을 **가르고**(`read_crontab`),
#    읽지 못했으면 **한 글자도 쓰지 않는다** ⓑ 설치 전 실물을 스냅숏으로 남기고
#    ⓒ 블록 밖 줄 수(`PRE_N`)를 세어 설치 후와 대조한다. 줄이 줄었으면 red 다.
#
# 환경변수
#   COLAB_BACKUP_STATE_DIR  스냅숏 보관 위치 (기본 $HOME/colab-v2-backups)
...
# 걸려 있는지 **읽어서** 판정한다. 넷을 다 본다 — 블록 표식 · 실행 줄 2개 · 블록 밖 줄 수.
