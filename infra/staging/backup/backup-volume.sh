...
#   (저장키는 ULID 라 같은 키가 다른 내용으로 덮어쓰이지 않는다). 그래서 아카이브는 원장을 **덮는다**.
#   ⚠ **「접수는 추가만 한다」는 거짓이다.** `app/routes/ingestion.py` 의 `createDataset`(접수분 → 데이터셋 키 이관) ·
#     `attachUploadGridFiles` · `replaceDatasetGridFile` 이 키를 **이동·재작성**한다. 위 논증은 그래도 선다 —
#     t0 의 원장이 가리키던 키가 t1 에 옮겨져 있으면 오라클이 **RED** 를 내지, 조용히 통과시키지 않는다.
...
#   ⚠ **남는 가장자리 — [t0,t1] 사이의 「삭제」와 「이동(등록 전환)」 둘 다.** 그 사이에 파일이 지워지거나
#     키가 옮겨지면 원장에는 있고 아카이브에는 없어 오라클이 **false RED** 를 낸다. staging 에서 이것을 위해 설계하지 않는다:
#     발생하면 그 회차를 **다시 돌리면 된다**(다음 회차의 원장은 삭제된 행을 안 갖고, 옮겨진 키를 새 자리로 가리킨다). 조용히 통과시키는
