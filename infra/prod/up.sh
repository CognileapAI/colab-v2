...
# 컨테이너를 놓쳤던 교훈). 환경변수는 /opt/colab-v2/prod.env(0600)에서 — 시크릿 **값**(세션·VIZ 토큰·타일 비밀·OPENAI)
# 과 시크릿 **디렉터리**(COLAB_SECRETS_DIR = /etc/colab, 파일 5 + subjects/credentials) 경로다.
# AWS 키는 어디에도 없다 — 인스턴스 프로파일.
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ENV_FILE="${COLAB_PROD_ENV:-$HERE/prod.env}"
...
units=(colab_v2_prod_core_api colab_v2_prod_pipeline_worker colab_v2_prod_viz_render colab_v2_prod_ai_service)
