...
# 상태3 = 0 이 아닌 종료(exit N≥1)면 실패로 인정한다 — 1/78 구분은 H18 이 잰다.
set -uo pipefail
OUT="$(cat)"
FAIL=0
...
printf '%s' "$OUT" | grep -Eq '상태2 명시 면제:.*(건수|N ?건|<N>건|\$\{?N\}?건)' || no "상태2(면제)가 건수 노출을 말하지 않는다."
printf '%s' "$OUT" | grep -Eq '상태3 무언:.*(실패|red|78|exit ?[1-9][0-9]*)' || no "상태3(무언)이 실패가 아니다."
