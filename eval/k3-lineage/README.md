...
| 파일 | 무엇인가 |
|---|---|
| `lineage-cases.json` | 사람이 등록한 실제 계보 엣지. `eval/k4-search/fixtures/reference/dev-data-snapshot-v2.json` 의 `parents` 에서 자식 5건을 골라 그대로 옮긴 것이고(WU4 · 2026-09-25), 옮겨 적다 틀렸는지는 `services/core-api/tests/test_k3_candidate_recall.py::test_계보_정답이_참조_스냅샷과_한_글자도_어긋나지_않는다` 가 **매 게이트에서** 대조한다(측정 표식이 안 붙은 시험이다). |
...
| 모델 절반 | `llm_lineage_probe.py --candidates <그 파일>` — DB 에 닿지 않는다 | 자식 5 × 2회 + 대조군 5 × 2회 = **20회** |
...
같은 자식 5건에서 **정답 부모를 후보에서 뺀다.** 후보에 없는 것을 억지로 고르면 red 다 —
...
- **일반화의 근거가 아니다.** 자식 5 · 엣지 10 · 후보 모집단 28(dev 가시 28건 전부 · 스냅샷 계보는 자식 13 · 엣지 18)다.
  10엣지로 승격·확대를 결정하지 않는다. 미달은 「불충분」이 아니라
  **「판정 보류 · 표본 확장」**으로 적는다.
- **`recent` 전략의 상위 순위는 신호가 아니다.** 참조 코퍼스는 한 번에 적재돼
