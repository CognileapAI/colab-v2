...
종료코드: 0 = 측정 완료 · 78 = 준비 실패(키·입력 파일·출력 충돌·업로드 축 불성립). **판정 게이트가 아니다.**
"""
import argparse
import dataclasses
import hashlib
...
#: 정답 파일 — 러너가 후보 기록의 업로드 축을 대조하는 기준(2026-09-25 사용자 서명 ①ⓐ).
CASES_PATH = HERE / 'lineage-cases.json'
#: `file_name` 말고 근거가 될 수 있는 업로드 축. 이 중 하나도 없으면 측정 불성립이다.
_EVIDENCE_AXES = ('crs', 'grid', 'variables', 'period_start', 'period_end')


def upload_axes_record(file_meta: dict) -> dict:
    """계약 `UploadedFileMeta` → 후보 기록의 `upload_axes` 와 **같은 모양**(`test_k3_lineage_probe._axes_json`).

    제품 함수(`UploadAxes.from_file_meta`)를 통과시킨다 — 여기서 축을 따로 뽑지 않는다.
    """
    row = dataclasses.asdict(signals.UploadAxes.from_file_meta(file_meta))
...
def upload_axes_problems(recorded_cases: list[dict], answer_cases: list[dict]) -> list[str]:
    """⭑ ⟨2026-09-25 intent O2 · 78 표 모델 절반 칸⟩ **측정이 성립하지 않는 업로드 축**을 찾는다.

    ① 후보 기록에 `upload_axes` 가 없다 · ② `file_name` 말고 축이 전부 비었다 ·
    ③ 정답 파일 `upload_meta` 와 후보 기록 `upload_axes` 가 다르다(두 절반이 다른 업로드를 봄).
    하나라도 있으면 부르는 쪽이 78 로 멈춘다 — 수치로 남기지 않는다.
    """
    answers = {c['child_dataset_id']: c for c in answer_cases}
    problems = []
    for case in recorded_cases:
        label = case.get('id') or case.get('child_dataset_id')
        recorded = case.get('upload_axes')
        if not isinstance(recorded, dict):
            problems.append(f'{label}: 후보 기록에 upload_axes 가 없다')
            continue
        if not any(recorded.get(k) for k in _EVIDENCE_AXES):
            problems.append(f'{label}: 업로드 축이 file_name 뿐이다')
        answer = answers.get(case.get('child_dataset_id'))
        if answer is None or not isinstance(answer.get('upload_meta'), dict):
            problems.append(f'{label}: 정답 파일에 upload_meta 가 없다')
            continue
        if upload_axes_record(answer['upload_meta']['file']) != recorded:
            problems.append(f'{label}: 정답 upload_meta 와 후보 기록 upload_axes 가 다르다')
    return problems


def override_upload_meta(recorded_cases: list[dict], override_cases: list[dict]) -> tuple[list[dict], list[str]]:
    """⭑ ⟨2026-09-25 서명 ①ⓑ · 상한 민감도⟩ **같은 후보 JSON** 의 업로드 메타만 갈아 끼운 사본을 만든다.

    후보·대조군·후보 축은 그대로 두고 `file_meta` 만 바꾼다 — 모델 입력·인용 검증·오라클이 모두
    `file_meta` 에서 업로드 축을 다시 만들므로(`upload_axes_of`) 세 자리가 같은 값을 본다.
    원본 기록은 바꾸지 않는다. 자식이 빠졌거나 `file_name` 말고 축이 없으면 문제로 돌려준다(부르는 쪽 78).
    합격선이 아니다 — 판정은 정답 입력(보류 값) 실행이 한다.
    """
    rows = {c.get('child_dataset_id'): c for c in override_cases}
    out, problems = [], []
    for case in recorded_cases:
        label = case.get('id') or case.get('child_dataset_id')
        row = rows.get(case.get('child_dataset_id'))
        if row is None or not isinstance((row.get('upload_meta') or {}).get('file'), dict):
            problems.append(f'{label}: 민감도 입력에 upload_meta 가 없다')
            continue
        file_meta = json.loads(json.dumps(row['upload_meta']['file']))
        if not any(upload_axes_record(file_meta).get(k) for k in _EVIDENCE_AXES):
            problems.append(f'{label}: 민감도 입력의 업로드 축이 file_name 뿐이다')
            continue
        out.append(dict(case, file_meta=file_meta))
    return out, problems
...
def main() -> int:
    ...
    ap.add_argument('--answers', type=Path, default=CASES_PATH,
                    help='업로드 축 대조 기준 정답 파일(기본 lineage-cases.json)')
    ap.add_argument('--upload-meta-override', type=Path, default=None,
                    help='상한 민감도 전용 — 같은 후보 JSON 에 이 파일의 cases[].upload_meta 를 업로드 입력으로 건다'
                         '(합격선 아님 · 후보 기록은 먼저 --answers 와 대조한다)')
    args = ap.parse_args()
    ...
    try:
        ...
        # ⭑ ⟨2026-09-25⟩ 업로드 축이 비었거나 정답과 다르면 측정이 아니다 — 모델을 부르기 전에 78.
        problems = upload_axes_problems(cases, json.loads(args.answers.read_text())['cases'])
        if problems:
            raise ValueError('upload axes not established: ' + '; '.join(problems))
        override = None
        if args.upload_meta_override is not None:
            cases, problems = override_upload_meta(
                cases, json.loads(args.upload_meta_override.read_text())['cases'])
            if problems:
                raise ValueError('upload meta override not established: ' + '; '.join(problems))
            override = dict(source=str(args.upload_meta_override), sha256=_sha256(args.upload_meta_override))
        ...
        result = dict(
            kind='K3 WU-S6 실측 — 기록된 후보·대조군 4종에 두 팔(규칙·모델)을 나란히 건다. '
                 ...
                 + ('' if override is None else
                    ' · ⭑ 상한 민감도 — 업로드 메타를 --upload-meta-override 값으로 갈아 끼운 실행. 합격선 아님'),
            upload_meta_override=override,
