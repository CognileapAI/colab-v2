...
- **대표 제목 규약** — 화면당 `h1` 1개(그 화면의 이름) · 탭 본체·카드 머리 제목은 `h2` · 그 아래 구역이 `h3`.
  대화상자 안 제목은 대화상자 기준으로 세고 화면 단계에 넣지 않는다. 단계를 바꿔도 **보이는 크기는 CSS 가 고정**한다.
  집행 완료 = `/lab-settings` 한 화면(2026-09-13 · `D-6`). 나머지 화면은 후속 항목이다.
