...
//
// ⭑ **⟨개정 2026-09-14 · 기획서 rev2 `#findModal` ＋ 기획자 2026-09-13 구두 피드백⟩**
...
//  - **필터는 검색어 하나**다. 남은 조건은 서버 질의로 그대로 가고, 목록은 고정 높이로 구른다.
//  - **하단 고정 영역** = 고른 후보 요약(이름·단계·분류) ＋ 가공 방식 ＋ 취소／연결. 별도
//    확인 단계를 두지 않는다 — 요약 줄 자체가 확인이다.
...
function periodLabel(period: { start: string; end: string | null } | null | undefined): string | null {
...
export function ParentPicker(props: {
  ...
  /**
   * 서버 질의에 실리는 가공 단계 조건. ⭑ ⟨개정 2026-09-14⟩ **화면 셀렉트는 걷었다**
   * (필터는 검색어 하나) — 값은 부르는 쪽이 쥔 채 질의로만 간다.
   */
  ...
  onLoadMore?: () => void;
  /** 두 번째 인자 = 하단에서 적은 **가공 방식**. 모달로 뜰 때만 실린다. */
  onPick: (row: ParentCandidateRow, method?: string) => void;
  ...
  const content = (
    <div className="lin-picker" data-testid={props.testId}>
      ...
      {props.error ? <div role="alert"><p>{props.error}</p><button type="button" className="btn btn-secondary" onClick={props.onRetry}>다시 시도</button></div> : candidates === null ? (
        ...
        <p className="muted">{query || levelFilter !== null
                ...
                <label className="lin-cand">
                  ...
                  <span className="lin-cand-b">
                    ...
                    <span className="lin-candidate-info">{[
                      ...
                      'period' in row ? periodLabel(row.period) : null,
  ...
  return <div className="modal-back lin-find-back" {...{ [ESC_LAYER_ATTR]: '가공 전 데이터 찾기' }} onMouseDown={event => { downOnBackdrop.current = event.target === event.currentTarget; }} onClick={event => { if (event.target === event.currentTarget && downOnBackdrop.current) props.onClose?.(); }}>
    <div className="modal lin-find" role="dialog" aria-modal="true" aria-label="가공 전 데이터 직접 찾기" ref={body} onKeyDown={event => {
      ...
      {/* ⭑ **하단 고정 영역** — 고른 것을 그대로 되읽어 주고, 가공 방식을 여기서 받는다.
          별도 확인 단계를 두지 않는다(기획자 2026-09-13 「하단 요약이 확인 역할」). */}
      <div className="modal-f lin-find-f">
        ...
        <label className="lin-find-method">
          <span>가공 방식</span>
          <input
            className="inp"
            data-testid="lin-find-method"
            maxLength={120}
        ...
        <div className="lin-find-btns">
          ...
          <button type="button" className="btn btn-primary" disabled={!valid || !!props.error} onClick={() => {
            ...
            // 빈 칸은 **싣지 않는다** — 부르는 쪽의 기존 초기값을 빈 문자열로 덮지 않기 위해서다.
            const typed = method.trim();
            if (typed) props.onPick(selected, typed); else props.onPick(selected);
          }}>이 데이터로 연결</button>
        </div>
      </div>
    </div>
