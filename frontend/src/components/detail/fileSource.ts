// 파일 관리 5 op 의 실서버 구현 (`PLAN-SoT §9 〈339〉`). 타입은 전부 생성물에서 온다.
//
  ...
  return {
    async list(datasetId): Promise<DatasetFile[]> {
      ...
      if (!r.data) {
        ...
        throw new Error(serverMessage(r.error) ?? '파일 목록을 불러오지 못했어요.'); // [정본 무근거 · 〈339〉]
      }
      return r.data.items ?? [];
    },
    ...
    async downloadTicket(datasetId, fileId): Promise<DownloadTicket> {
      ...
      if (r.response.status === 404) throw new FileGone();
      if (r.response.status === 501) throw new NotImplemented();
      if (!r.data) throw new Error(serverMessage(r.error) ?? '다운로드를 시작하지 못했어요.'); // [정본 무근거 · 〈339〉]
      return r.data;
    },
    ...
    async add(datasetId, file, kind, relativePath): Promise<DatasetFile> {
      ...
      if (!r.data) throw new Error(serverMessage(r.error) ?? '파일을 더하지 못했어요.'); // [정본 무근거 · 〈339〉]
      return r.data;
    },
    ...
    async replace(datasetId, fileId, file): Promise<DatasetFile> {
      ...
      if (!r.data) throw new Error(serverMessage(r.error) ?? '파일을 교체하지 못했어요.'); // [정본 무근거 · 〈339〉]
      return r.data;
    },
    ...
    async remove(datasetId, fileId): Promise<void> {
      ...
      if (r.response.status === 409) {
        throw new LastBodyFile(serverMessage(r.error) ?? '마지막 본체 파일은 지울 수 없어요.'); // [정본 무근거 · 〈339〉] 서버 문장이 없을 때만
      }
      if (r.response.status === 501) throw new NotImplemented();
      if (!r.response.ok) throw new Error(serverMessage(r.error) ?? '파일을 지우지 못했어요.'); // [정본 무근거 · 〈339〉]
    },
  };
}
