...
// ── 파일 관리 (`PLAN-SoT §9 〈339〉` — 회의 결정 · 정본 산문 밖) ─────────────────────────

/** 등록된 데이터셋의 조각 하나. `byteSize` 가 `null` 이면 **모르는 것**이지 0 이 아니다 (`〈339〉-(가)`). */
export type DatasetFile = S['DatasetFile'];
export type FileKind = S['FileKind'];
/** 다운로드 티켓 — `url` 이 바이트다. 클라이언트는 그것을 **그대로 내비게이션**한다 (`〈339〉-(다)`). */
...
/**
 * 그 데이터셋의 **마지막 본체 파일**이다 — 본체 ≥ 1 불변식 (`DataModel §4.3` · `〈339〉-(라)` · 계약 409).
