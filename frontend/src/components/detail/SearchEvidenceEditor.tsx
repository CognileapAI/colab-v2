    ...
    <section className="dt-edit" aria-label={`${props.fileName} 검색 근거`}>
      ...
      {item && form && canEdit ? (
        <form className="de-grid" aria-label={`${props.fileName} 검색 근거`} onSubmit={(event) => event.preventDefault()}>
          ...
          <div className="de-row"><label className="de-k" htmlFor={`se-cadence-${props.fileId}`}>주기</label><select className="de-v" id={`se-cadence-${props.fileId}`} value={form.cadence} onChange={(e) => set('cadence', e.target.value as FormState['cadence'])}><option value="">미상</option><option value="15min">15분</option><option value="hourly">매시</option><option value="daily">매일</option><option value="weekly">매주</option><option value="monthly">매월</option></select></div>
