  ...
  return {
    async create(files: PickedFile[], opts?: UploadCreateOptions) {
      ...
      const r = await api.POST('/uploads', {
        headers: opts?.targetLabId ? { 'X-CoLAB-Target-Lab': opts.targetLabId } : {},
      ...
      if (!r.data) throw new Error(requestMessage(r.response.status, r.error, '파일을 올리지 못했어요.'));
      return r.data;
    },
    ...
    async register(body: DatasetCreate, targetLabId?: string) {
      const r = await api.POST('/datasets', { body, headers: targetLabId ? { 'X-CoLAB-Target-Lab': targetLabId } : {} });
      if (r.response.status === 404) throw new UploadGone();
      if (r.response.status === 501) throw new NotImplemented();
