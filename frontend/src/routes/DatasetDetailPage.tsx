...
export function DatasetDetailPage(
  ...
  const targetLabId = account?.canManageServiceAccounts ? shown?.labId : undefined;
  const projectSource = useMemo(() => props.projectSource ?? defaultProjectSource(targetLabId), [props.projectSource, targetLabId]);
      ...
      {detail.status === 'ready' && shown ? (
        ...
        >
              ...
              <>
                <div className="dt-gridact" data-testid="detail-grid-actions">
                  ...
                  {/* **진입점 하나.** 격자 0건은 정상 상태이고(`P2.md §2-21`), 나중에 붙이는 길이
                      ...
                      모달이 그 문장을 그대로 보여 준다. */}
                  <GridAttachEntry
                    targetLabId={account?.canManageServiceAccounts ? shown.labId : undefined}
                    datasetId={datasetId}
          ...
          <UsageSection detail={shown} downloadHidden={edit.editing} projectSource={projectSource}
            canManage={account?.permissions?.['업로드·편집'] === true}
            onChanged={() => setReloadToken((n) => n + 1)} />
          </div>
        </LockedContent>
        </>
