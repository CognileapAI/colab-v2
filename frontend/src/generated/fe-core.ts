...
export interface paths {
    ...
    "/dataset-searches": {
         ...
         *     - **근거는 검색된 이유만 싣는다** — `rationale` 은 이유를 줄바꿈 없이 이은 한 줄이고,
         *       같은 사실의 구조형이 선택 칸 `rationaleFacts`(근거 종류별 항목)다. 한계·부정 문장은
         *       카드 근거에 싣지 않는다. [사용자 승인] intent `2026-09-25-search-rationale-separation.md`
         *       Q6·Q7 — 종전 「한계도 그 한 줄 안에서」는 이 결정으로 개정했다(정본 개정은 별도 절차).
...
export interface components {
    schemas: {
        ...
        SearchResults: components["schemas"]["ListEnvelope"] & {
            ...
            /**
             * @description [사용자 승인] 이번 검색이 좁혀 뒤진 **주제**. 주제로 좁혔을 때만 싣는다 —
             *     화면은 범위 줄 뒤에 「주제 ‘X’로 좁혀 뒤졌어요.」를 한 번 적는다. 종전에는 카드
             *     근거마다 반복됐다(intent `2026-09-25-search-rationale-separation.md` Q8).
             */
            topic?: string;
        ...
        SearchResultRow: components["schemas"]["DatasetRow"] & {
            ...
            relevanceBar: number;
            /**
             * @description 이 결과가 검색된 이유 **한 줄**(줄바꿈 없음). **이유만 싣고 한계·부정 문장은
             *     싣지 않는다** — `rationaleFacts` 의 항목을 순서대로 이은 문장이다(구 화면 호환 ·
             *     화면과 같은 사실을 말한다). 뒤진 범위·주제 같은 질의 공통부는 여기 넣지 않고
             *     응답 머리(`scope`·`topic`)가 한 번 말한다.
             *     [사용자 승인] intent `2026-09-25-search-rationale-separation.md` Q6·Q8 ·
             *     미해결 질문 ⒝ — 종전 「한계도 이 한 줄 안에서 함께 밝힌다」를 개정했다
             *     (정본 `Policy_데이터_찾기 §4·§8` 개정은 별도 절차).
             */
            rationale: components["schemas"]["AiRationale"];
            /**
             * @description [사용자 승인] 검색된 이유의 **구조형** — 근거 종류별 사실 목록. 화면은 이것을
             *     「AI」 패널의 2단계 목록으로 그리고, 없으면 `rationale` 한 줄로 대신 그린다
             *     (intent `2026-09-25-search-rationale-separation.md` Q1b·Q7).
             *     종류 순서는 서버가 정한다: `term`(낱말 일치) → `concept`(관련 개념) →
             *     `linked`(연결된 자료) → `evidence`(파일 근거). **사실이 없는 종류는 싣지 않는다.**
             *     항목은 이유만 담는다 — 한계·부정 문장은 싣지 않는다.
             */
            rationaleFacts?: {
                /** @enum {string} */
                kind: "term" | "concept" | "linked" | "evidence";
                items: components["schemas"]["AiRationale"][];
            }[];
            /**
