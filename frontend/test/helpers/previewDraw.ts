...
const DRAW_TEST_ID = 'up-preview-draw';
...
export async function clickPreviewDrawWhenReady(
  ...
  const button = await waitFor(() => {
    const el = screen.getByTestId(DRAW_TEST_ID) as HTMLButtonElement;
    if (el.disabled) throw new Error(`${DRAW_TEST_ID} is still disabled (palettes not ready)`);
    return el;
  }, options.wait);
