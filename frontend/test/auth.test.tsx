...
describe('문지기 — 인증 세부는 한 곳에만 있다', () => {
  it('토큰이 없으면 로그인 화면이고 GNB 가 없다', () => {
    ...
    renderApp();
    expect(screen.getByTestId('login-access-code')).toBeInTheDocument();
  ...
  it('/me 가 401 이면 토큰을 버리고 로그인 화면으로 돌아간다', async () => {
    ...
    renderApp();
    expect(await screen.findByTestId('login-access-code')).toBeInTheDocument();
...
describe('요청 첨부 — 화면이 헤더를 손으로 붙이지 않는다', () => {
  ...
  it('로그인 op 에는 붙지 않는다 — 계약이 `security: []` 다', async () => {
    ...
    await api.POST('/sessions', { body: { accessCode: 'code' } });
...
describe('로그인 화면', () => {
  it('성공하면 토큰을 보관하고 앱으로 넘어간다', async () => {
    ...
    fireEvent.change(screen.getByTestId('login-access-code'), { target: { value: '심어둔-코드' } });
  ...
  it('실패하면 로그인 자리에 남고 사유를 그대로 말한다 — 가짜 진행이 없다', async () => {
    ...
    renderApp();
    fireEvent.change(screen.getByTestId('login-access-code'), { target: { value: '아무-코드' } });
    fireEvent.click(screen.getByTestId('login-submit'));
