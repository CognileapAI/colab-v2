...
#      판정·비고 머리가 모두 맞는 행만 면제하고(파생 포함) 그 밖의 불일치는 종전대로 실패하며, 목록 부재·깨짐·칸 오류는
#      실패(fail-closed)다. `--verify-from` 은 상세 화면 순회 없이 앞 실행의 판정표·계수로 대조해 계정 최종화로 가고,
#      파일 부재·대상 sha·데이터셋 수·판정표 이름 불일치와 `--from verify` 가 아닌 실행을 거부한다.
#      왜 = 2026-09-24 재시드가 28/28 적재 뒤 열린 제품 결함(#133·#134) 세 행으로 verify 에서 멈춰 계정 최종화가
#      돌지 않았다. 사용자 결정(2026-09-25) = 실패한 꼬리만 마친다(reset·전수 재순회 없음).
#
