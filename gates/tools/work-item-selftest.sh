...
# ㈐ 정밀도 — 식별자 토큰을 못 읽으면 그 행은 「대조 대상 밖」으로 빠지고 어긋남이 숨는다.
# `DR-1a` 처럼 하이픈 뒤에 숫자＋소문자가 오는 id 가 그 모양이다(`D2b` 와 달리 하이픈이 앞에 온다).
expect red "㈐" "㈐ 하이픈 뒤 숫자＋소문자 접미 id(DR-1a) 행도 대조한다 (토큰 누락 → 조용한 통과 금지)" run red-m-suffix-id
