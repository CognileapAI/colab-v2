...
| `selftest` | **위 게이트들이 실제로 red를 낼 수 있는지** (contract · event · boundary · db-boundary · db · rls-effect · seam-consistency · generated · work-item · e2e-format-coverage 증명 열). ⚠ **`stage2-markers-selftest` 는 여기 없다** — pipeline-worker 런타임 의존(rasterio 등)이 필요해 `contract-gates` 잡 환경에서 못 돈다. CI 는 `dormant-tests` 잡에서 따로 부른다 |
...
| `preview-tile-slot-selftest` | **12** | docker(postgres) — 일회용 DB 에 `preview.cog-built` 픽스처를 넣고, 픽스처 TIFF 를 **바이트로 지어**(라이브러리를 들이면 판정이 라이브러리로 옮겨간다) 10 케이스를 돈다: ⓐ 적용 DB 미지정 · ⓑ 면제 선언 부재 · ⓑ' 면제 항목 부재 · ⓒ 자리 경로 미선언 · ⓓ 없는 디렉터리 · ⓔ **자리에 타일 0건** · ⓕ 못 쓰는 타일 · ⓖ 발행 있음·쓸 수 있는 타일 0 이 전부 red, ⓗ 이름으로 면제 · ⓘ 쓸 수 있는 타일 1건 이 green 이며 **ⓗ 는 면제 건수 노출까지, ⓐ 는 원인 표식(`cause=입력미선언`)까지 본다** |
