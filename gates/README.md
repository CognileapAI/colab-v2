...
| **`adr-records`** ⭑신설 | `docs/decisions/*` 전 기록의 파일명·제목·상태·날짜·필수 절·대체 연결 누락(`scripts/harness/adr_gate.py --all --base "${COLAB_ADR_BASE:-HEAD}"` · ADR-0003 대로 훅 없이 게이트로만 붙인다 · 의미 승인은 판정하지 않는다). 기준 = `COLAB_ADR_BASE`(CI 는 PR base sha) — 승인·대체된 ADR 의 삭제·되돌림을 그 기준 대비로 본다. 기록 0건은 red(판정) · 통과 줄에 기록 수를 찍는다. CI = `planning-gates` 잡 · 필터 `dev-package` 에 `docs/decisions/**` 포함(대조 = `ci-filter-check.py` ㈕). 입력을 못 읽으면 red(준비 · 78) |
| **`intent-ref`** ⭑신설 | ⑴ 범위 안 커밋에 `Intent-Ref: dev-package/intent/<파일>.md` 트레일러가 **하나도 없거나** 그 파일이 head 에 없으면 red(판정) — 범위가 `services/**`·`frontend/src/**`·`contracts/**`·`db/**`·`scripts/**`·`gates/**`·`.agents/**`·`.claude/**`·`.codex/**` 를 바꿀 때만(아니면 「대상 밖」과 건수를 출력하고 green). 소급 = 트레일러만 담은 `git commit --allow-empty` 1개. ⑵ 기준 시점 **또는 분기 시점**(merge-base)에 **승인 표기가 있는 intent**(메타 줄에 `승인` · `미승인` 없음 · `TEMPLATE.md` 제외)의 기존 줄이 분기점..head 에서 삭제·변경되면 red(판정) — 줄 추가만 허용 — 기존 줄(원본 바이트 · `git cat-file blob`)이 새 내용에 순서대로 모두 남아 있어야 한다(diff 출력을 해석하지 않으므로 `---` 줄 · 끝 줄바꿈 · `-diff` 속성 · 색상 설정이 판정을 바꾸지 못한다). 분기 뒤 develop 에서 승인된 intent 도 보호한다. 기준 = `COLAB_INTENT_REF_BASE`(CI 는 PR base sha · head 는 `COLAB_INTENT_REF_HEAD`) · 미선언이면 `merge-base(HEAD, origin/develop)` 를 쓰고 그 사실을 출력한다 · 그 ref 도 없거나 기준을 못 풀면 red(준비 · 78). CI = 독립 잡 `intent-ref`(`pull_request` · base ≠ `product` · `fetch-depth: 0`). 판별식 정답표 = `gates/fixtures/intent-ref/intent-meta-classification.json`(62건 · 승인 44 · 미승인 13 · 메타 없음 5 · 의도된 비보호 1) |
...
| **`region-within-drift`** ⭑신설 | **검색 계약의 지역 포함 표(`contracts/search/semantics.json` `regionWithin`)가 D9 그래프와 같은가** — intent `dev-package/intent/2026-09-26-region-containment-expansion.md` 결정 1 ㈎ 는 그래프 한 홉(`한반도` → `남한`·`충청권`)을 계약에 한 번 더 적는다. 그래프를 고치고 계약을 잊으면 조건 검색이 옛 포함 관계로 조용히 답하므로 이 대조가 **이중 기재의 유일한 방어선**이다. **보는 것** — ① 부모 식별자가 `regions` 에 있고 `place` 가 그 질의 표기 중 하나 ② `place` 가 지명 노드로 정확히 1개 · expandable=t ③ 표의 하위 = 그래프의 `<하위> 안에 있다 <place>` 지명 노드 전부(빠짐·남음 red · 남는 하위가 그래프상 상위면 **상향 엣지**로 이름 붙인다) ④ 그래프 팬아웃 > 6 이면 부모 전체를 표에서 뺀다(`d9_ontology.MAX_FANOUT`) ⑤ 하위 별칭·`placeAliases` = `d9_place_alias`(자기 자신 제외) ⑥ `regions` 식별자가 가리키는 펼 수 있는 지명 노드가 표에 없으면 red · 표가 비면 red(대상 0건). 입력은 커밋된 파일뿐(DB·네트워크 0). **red(준비 · 78)** = 입력 부재·파싱 불가 · `regionWithin` 미선언(입력미선언) · 별칭 행 0건 |
| **`region-within-drift-selftest`** ⭑신설 | 위 게이트가 red fixture 로 fail-closed 임을 증명한다 — 케이스마다 `mktemp -d` 안에 **실제 입력의 사본**(semantics.json · k2b-graph-standard.tsv · db/ai/seed/*.sql)을 두고 한 곳만 비튼다. **ⓐ 실제 입력 green · ⓑ 그래프에서 `남한 안에 있다 한반도` 삭제 red(판정 · 「남는 하위」) · ⓒ 그래프에만 있는 한반도 하위 red(판정 · 「빠진 하위」) · ⓓ 표에 상향 하위(그래프는 `한반도 안에 있다 동아시아`) red(판정 · 「상향 엣지」) · ⓔ 팬아웃 7 red(판정) · ⓕ 하위 별칭 누락 red(판정 · 「빠진 별칭」) · ⓖ 그래프 파일 부재 red(준비 · 78) · ⓗ `regionWithin` 미선언 red(준비 · 입력미선언)**. red 는 사유를 이름으로 내야 통과다. 끝에 레포 원본 ↔ 사본 동일성을 본다 |
...
- **전수 회차의 정본 호출문은 아래 한 줄이다.** `seed-plan-drift`·`frontend-visual`·`harness-eval` 세 게이트는 운영자 입력이 없으면 red(준비 · 입력미선언 · 78)이고, 셋 다 **선언이든 명시 면제든 말을 해야** 병합 진입 조건 `red_준비 == 0`(`:137`)을 채운다. 변수를 아무것도 주지 않은 호스트에서는 어떤 브랜치도 그 조건을 만족할 수 없다. **명시 면제도 병합 진입 조건 충족이다**(`harness-eval` 은 해시 일치 결과 전제 · intent `dev-package/intent/2026-09-17-gate-input-env-vars-block-merge-gate.md` 결정 · 2026-09-18). 면제 건수는 전수 요약 줄이 아니라 **해당 게이트 자신의 출력**에 드러난다 — `frontend-visual` 은 페이지 건수, `harness-eval` 은 과제 건수다.
...
| `planning-gates` | `dev-package`(⭑ `docs/decisions/**` · `scripts/harness/adr_gate.py` · `.agents/harness.yaml` 포함) | `adr-records`·`work-item-consistency`·`seed-plan-drift`(명시 면제 모드) · `planning-freshness` 는 CI 에 원본이 없어 안내만 내고 로컬 실행 결과가 따로 필요하다 |
| **`intent-ref`** ⭑신설 | **`pull_request` 이고 base ≠ `product`**(경로 필터 없음 · `changes` 를 거치지 않는다) | `intent-ref`(base..head · `fetch-depth: 0`). push 에서는 skipped 이고 `ci-required` 가 경로 필터 skip 과 같이 센다 |
...
| `harness-eval-selftest` ⭑신설 | **18** | 없음(bash + python3 + PyYAML) — `claude` 는 `mktemp -d` 안의 스텁이고 과제 뿌리도 임시 자리다(**모델 호출 0회**). ⓐ 과제 0건 red(판정) · ⓑ 면제 건수 노출 green · ⓒ 상한 초과 red(준비) · ⓓ 미선언 red(준비·입력미선언) · ⓔ–ⓚ 실행 모드 요약 대조 · ⓛ–ⓡ 면제의 설정 해시 결합 ＋ CI 필터 대조 |
...
| `service-tests-selftest` | 실행 요약 참조 | 서비스 venv와 Docker — 기존 계수·병렬도 음성 시험에 AI DB 부재(78)·AI schema fixture 실패(1)를 추가. 픽스처 = `gates/fixtures/service-tests/` |
