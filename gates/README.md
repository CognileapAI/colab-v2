...
| **`intent-ref`** ⭑신설 | ⑴ 범위 안 커밋에 `Intent-Ref: dev-package/intent/<파일>.md` 트레일러가 **하나도 없거나** 그 파일이 head 에 없으면 red(판정) — 범위가 `services/**`·`frontend/src/**`·`contracts/**`·`db/**`·`scripts/**`·`gates/**`·`.agents/**`·`.claude/**`·`.codex/**` 를 바꿀 때만(아니면 「대상 밖」과 건수를 출력하고 green). 소급 = 트레일러만 담은 `git commit --allow-empty` 1개. ⑵ 기준 시점에 **승인 표기가 있는 intent**(메타 줄에 `승인` · `미승인` 없음 · `TEMPLATE.md` 제외)의 기존 줄이 삭제·변경되면 red(판정) — 줄 추가만 허용. 기준 = `COLAB_INTENT_REF_BASE`(CI 는 PR base sha · head 는 `COLAB_INTENT_REF_HEAD`) · 미선언이면 `merge-base(HEAD, origin/develop)` 를 쓰고 그 사실을 출력한다 · 그 ref 도 없거나 기준을 못 풀면 red(준비 · 78). CI = 독립 잡 `intent-ref`(`pull_request` · base ≠ `product` · `fetch-depth: 0`). 판별식 정답표 = `gates/fixtures/intent-ref/intent-meta-classification.json`(62건 · 승인 44 · 미승인 13 · 메타 없음 5 · 의도된 비보호 1) |
...
| `planning-gates` | `dev-package`(⭑ `docs/decisions/**` 포함) | `adr-records`·`work-item-consistency`·`planning-freshness` |
